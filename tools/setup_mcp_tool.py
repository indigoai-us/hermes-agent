...
``hqr mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the HQ Runtime desktop app. Use the "
            "terminal instead: `hqr mcp install <name>` for catalog entries, "
            "`hqr mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "HQ Runtime desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `hqr mcp catalog` "
