"""On-demand supply-chain audit for HQ Runtime Agent installs.

Scans three surfaces a HQ Runtime user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The HQ Runtime venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.hqr/plugins``
...
def _discover_plugins(hqr_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.hqr/plugins``.
...
def _discover_components(
    *,
    ...
    home = hqr_home or Path(get_hqr_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `hqr security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, hqr_home=home
    )
