...
These exercise the real store against a temp HQR_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated HQR_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("HQR_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_hqr_home() reads the env live.
    yield tmp_path
