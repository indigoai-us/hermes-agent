...
Maps each role archetype to the HQ Runtime skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public hqr-agent
...
## HQ Runtime skills relevant to video production

### Visual / rendering skills (`hqr-agent/skills/creative/`)
...
### Design / pre-production skills (`hqr-agent/skills/creative/`)
...
### Audio / media skills (`hqr-agent/skills/creative/` + `skills/media/`)
...
These are **not** HQ Runtime skills but external CLIs / APIs that profiles invoke.
...
## Built-in HQ Runtime tools for media review

These are native HQ Runtime tools — not invoked via terminal but through their own
...
key is present in `${HQR_HOME:-~/.hqr}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every HQ Runtime profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HQR_HOME:-~/.hqr}/.env` → environment variable.
...
When a new HQ Runtime-public video skill ships:
