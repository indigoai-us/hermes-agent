"""Shared constants for HQ Runtime Agent.
...
_HQR_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_HQR_HOME_OVERRIDE", default=_UNSET
)
...
def set_hqr_home_override(path: str | Path | None) -> Token:
    """Set a context-local HQ Runtime home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _HQR_HOME_OVERRIDE.set(value)


def reset_hqr_home_override(token: Token) -> None:
    """Restore the previous context-local HQ Runtime home override."""
    _HQR_HOME_OVERRIDE.reset(token)


def get_hqr_home_override() -> str | None:
    """Return the active context-local HQ Runtime home override, if any."""
    override = _HQR_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_hqr_home() -> Path:
    """Return the platform-native default HQ Runtime home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "hqr"
    return Path.home() / ".hqr"


def _hqr_home_from_env() -> Path:
    """Resolve HQR_HOME from the process environment only.

    Reads the ``HQR_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_hqr_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_hqr_home`
    and :func:`get_process_hqr_home` so the two never drift.
    """
    val = os.environ.get("HQR_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_hqr_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``HQR_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_hqr_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[HQR_HOME fallback] HQR_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass HQR_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_hqr_home() -> Path:
    """Return the HQ Runtime home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_hqr_home_override`) → ``HQR_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``HQR_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``HQR_HOME`` explicitly (see the systemd
    template in ``hqr_cli/gateway.py`` and the kanban dispatcher in
    ``hqr_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    override = get_hqr_home_override()
    if override:
        return Path(override)

    if not os.environ.get("HQR_HOME", "").strip():
        _warn_profile_fallback_once()

    return _hqr_home_from_env()


def hqr_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a HQ Runtime home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_hqr_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_hqr_home() -> Path:
    """Return the HQ Runtime home for the running process, ignoring task overrides.

    Unlike :func:`get_hqr_home`, this never follows the context-local
    override set by :func:`set_hqr_home_override`.  It resolves only the
    process ``HQR_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_hqr_root(). The function resolves
# HQR_HOME against the native home on every call (~80us of path
...
# Its result depends only on (HQR_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates HQR_HOME mid-process.
_default_hqr_root_memo: "tuple[str, str, Path] | None" = None


def get_default_hqr_root() -> Path:
    """Return the root HQ Runtime directory for profile-level operations.

    In standard deployments this is the platform-native HQ Runtime home
    (``~/.hqr`` on POSIX, ``%LOCALAPPDATA%\\hqr`` on native Windows).

    In Docker or custom deployments where ``HQR_HOME`` points outside
    ``~/.hqr`` (e.g. ``/opt/data``), returns ``HQR_HOME`` directly
    — that IS the root.

    In profile mode where ``HQR_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.hqr/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_hqr_root_memo
    native_home = _get_platform_default_hqr_home()
    env_home = os.environ.get("HQR_HOME", "")
    if _default_hqr_root_memo is not None:
        memo_native, memo_env, memo_result = _default_hqr_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # HQR_HOME is under ~/.hqr (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — HQR_HOME itself is the root
                result = env_path
    _default_hqr_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``HQR_OPTIONAL_SKILLS``.
    """
    override = os.getenv("HQR_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_hqr_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``HQR_OPTIONAL_MCPS``.
    """
    override = os.getenv("HQR_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_hqr_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``HQR_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<HQR_HOME>/skills`` last-resort
    """
    override = os.getenv("HQR_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_hqr_home() / "skills"
...
def get_hqr_dir(
    ...
    *,
    ...
    """Resolve a HQ Runtime subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to HQR_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to HQR_HOME (e.g. ``"image_cache"``).
        home: Optional explicit HQ Runtime home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local HQR_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_hqr_home()
...
def iter_hqr_node_dirs(home: Path | None = None) -> list[Path]:
    """Return HQ Runtime-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\hqr\\node``. POSIX installs use
    ``$HQR_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_hqr_home()
    dirs = [root / "node"]
...
_HQR_NODE_TARGET_MAJOR = int(os.environ.get("HQR_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    HQ Runtime-managed Node trees live under ``$HQR_HOME/node`` (or a profile's
    ``HQR_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_hqr_node_executable``
    used to trust file presence alone, so ``hqr update`` would pick that
...
def hqr_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any HQ Runtime-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_hqr_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ HQ Runtime-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `hqr update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%HQR_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the hqr.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_HQR_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_HQR_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$HQR_HOME/node`` on POSIX.
    ...
    ``HQR_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_hqr_managed_node() -> str | None:
    """Install a HQ Runtime-managed Node tree and return its npm path.
    ...
    HQ Runtime never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$HQR_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_hqr_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_hqr_node_dirs():
...
def heal_hqr_managed_node() -> bool:
    """Redownload HQ Runtime-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not hqr_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HQR_HOME": str(get_hqr_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_hqr_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_HQR_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_hqr_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _HQR_NODE_TARGET_MAJOR
    return False


def find_hqr_node_executable(command: str) -> str | None:
    """Return a HQ Runtime-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_HQR_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_hqr_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for HQ Runtime-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy HQ Runtime-managed installs.

    This is for HQ Runtime-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_hqr_node_executable(command)
    if managed:
        return managed
    if hqr_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_hqr_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with HQ Runtime-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_hqr_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``hqr update`` — leaving a **dangling symlink** that ``which`` still
...
def display_hqr_home() -> str:
    """Return a user-friendly display string for the current HQR_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.hqr``
        profile:  ``~/.hqr/profiles/coder``
        custom:   ``/opt/hqr-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.hqr``.  For code that needs a real ``Path``, use
    :func:`get_hqr_home` instead.
    """
    home = get_hqr_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``HQR_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{HQR_HOME}/home`` when the profile-home directory exists."""
    hqr_home = get_hqr_home_override() or (env or {}).get("HQR_HOME") or os.getenv("HQR_HOME")
    if not hqr_home:
        return None
    profile_home = os.path.join(hqr_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("HQR_REAL_HOME") or os.getenv("HQR_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding HQ Runtime profile HOME.

    ``HQR_HOME`` scopes HQ Runtime state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={HQR_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{HQR_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{HQR_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply HQ Runtime' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["HQR_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when HQ Runtime itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/hqr-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under HQR_HOME.

    Replaces the ``get_hqr_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, hqr_logging.py, hqr_time.py, etc.).
    """
    return get_hqr_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under HQR_HOME."""
    return get_hqr_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under HQR_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_hqr_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._hqr_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``hqr_cli`` modules using three different Windows
    ...
    A few sites outside ``hqr_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``hqr_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of HQ Runtime itself. An ImportError
...
# `hqr_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with HQ Runtime.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("hqr_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `hqr update` is exactly
