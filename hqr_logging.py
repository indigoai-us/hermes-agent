"""Centralized logging setup for HQ Runtime Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.hqr/logs/`` (profile-aware via ``get_hqr_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``hqr_cli.web_server``,
    ``hqr_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in HQ Runtime (TUI, gateway, ``hy_memory`` server, MCP
...
from hqr_constants import get_config_path, get_hqr_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_hqr_session_injector", False):
    ...
    _session_record_factory._hqr_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``hqr logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``hqr logs --component gateway``.
    "gateway": ("gateway", "hqr_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the HQ Runtime logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hqr_home
        Override for the HQ Runtime home directory.  Falls back to
        ``get_hqr_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_hqr_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._hqr_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the hqr group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from hqr_cli.config import is_managed
...
# the module header): when several HQ Runtime processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._hqr_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="hqr-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse hqr_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when hqr_cli.config isn't importable (bare
        # hqr_logging consumers).
        try:
            from hqr_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
