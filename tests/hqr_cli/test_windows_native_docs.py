...
def test_windows_native_install_path_docs_match_installer() -> None:
    ...
    assert "%LOCALAPPDATA%\\hqr\\hqr-agent\\bin" in doc
    assert (
        "Get-Command hqr        # should print "
        "C:\\Users\\<you>\\AppData\\Local\\hqr\\hqr-agent\\bin\\hqr.exe"
    ) in doc
    # Installer exposes $InstallDir\bin, and must copy the launchers into it.
    assert '$hermesBin = "$InstallDir\\bin"' in install
    assert "hqr.exe" in install and "hqr-acp.exe" in install
