...
def backup_env(monkeypatch, tmp_path):
    """Isolate HQR_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".hqr"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_hqr_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd hqr_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under HQR_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level HQR_DIR picks up the tmp HOME."""
    import hqr_constants
    importlib.reload(hqr_constants)
