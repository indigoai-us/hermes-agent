"""Projects codex app-server events into HQ Runtime' messages list.

The translator that lets HQ Runtime' memory/skill review keep working under the
...
HQ Runtime' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape HQ Runtime
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way HQ Runtime' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but HQ Runtime stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way HQ Runtime already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — HQ Runtime' messages store text only.
        text_parts: list[str] = []
