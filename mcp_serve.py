"""
HQ Runtime MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (HQ Runtime-specific extra)

Usage:
    hqr mcp serve
    hqr mcp serve --verbose
...
logger = logging.getLogger("hqr.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using HQR_HOME."""
    try:
        from hqr_constants import get_hqr_home
        return get_hqr_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("HQR_HOME", Path.home() / ".hqr")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from hqr_constants import get_hqr_home
        directory_file = get_hqr_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("HQR_HOME", Path.home() / ".hqr")
...
class EventBridge:
    ...
    This is the HQ Runtime equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from hqr_constants import get_hqr_home
            db_file = get_hqr_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("HQR_HOME", Path.home() / ".hqr")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the HQ Runtime MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "hqr",
        instructions=(
            "HQ Runtime Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the HQ Runtime MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
