...
- Skill helper paths:
  - `${HQR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HQR_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HQR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client hqr
```
...
```bash
PUBLISH="${HQR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client hqr
```
...
```bash
DRIVE="${HQR_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `hqr`)    |
