#!/bin/bash
# repro.sh -- reproduce desktop-update paths against a sandboxed HQR_HOME.
#
# Nothing here touches your real ~/.hqr or checkout. Each mode builds (or
# reuses) a disposable install under /tmp and drives the REAL code path --
# the actual installer, the actual orchestrator, the actual `hqr update`.
#
#   repro.sh shim          shim UI only: success event after 6s
#   repro.sh shim-fail     shim UI only: error event after 6s
#   repro.sh fresh         fresh install into a sandbox HQR_HOME
...
SANDBOX="${HQR_UPDATE_REPRO_HOME:-/tmp/hqr-update-repro}"
SANDBOX_ROOT="$SANDBOX/hqr-agent"

say() { printf '\n\033[1m== %s ==\033[0m\n' "$1"; }

ensure_sandbox_install() {
  if [ -x "$SANDBOX_ROOT/venv/bin/hqr" ]; then
  ...
  HQR_HOME="$SANDBOX" bash "$SANDBOX_ROOT/scripts/install.sh" --non-interactive --skip-setup --hqr-home "$SANDBOX"
}

case "$MODE" in
  shim)
    HQR_SELFTEST_HOLD_SECONDS="${HQR_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  shim-fail)
    HQR_SELFTEST_FAIL=1 HQR_SELFTEST_HOLD_SECONDS="${HQR_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  fresh)
    rm -rf "$SANDBOX"
    ensure_sandbox_install
    say "fresh install OK: $("$SANDBOX_ROOT/venv/bin/hqr" --version 2>/dev/null || echo '?')"
    ;;
  behind)
    N="${2:-25}"
    ensure_sandbox_install
    ...
    HQR_HOME="$SANDBOX" bash "$SCRIPT_DIR/posix.sh" \
      --install-root "$SANDBOX_ROOT" --branch main --desktop-pid 0 || true
    say "result file:"
    cat "$SANDBOX/.hqr-update-result.json" 2>/dev/null || echo "(none written)"
    echo
  ...
  gate)
    ...
    G="/tmp/hqr-gate-test.$$"
    UNPACKED="$G/hqr-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    touch "$UNPACKED/hqr" && chmod +x "$UNPACKED/hqr"
    ...
    decide() { bash "$SCRIPT_DIR/posix.sh" --self-test-gate --install-root "$G/hqr-agent" "$@" | cut -d: -f1; }

    expect "appimage (not under unpacked)"      skew     "$(decide --relaunch-target /opt/HQ Runtime/hqr)"
    expect "sibling-prefix dir not fooled"      skew     "$(decide --relaunch-target "$UNPACKED-evil/hqr")"
    expect "no chrome-sandbox (namespace)"      relaunch "$(decide --relaunch-target "$UNPACKED/hqr")"

    touch "$UNPACKED/chrome-sandbox"
    expect "sandbox not root/setuid"            manual   "$(decide --relaunch-target "$UNPACKED/hqr")"
    expect "opt-out: --sandbox-fallback"        relaunch "$(decide --relaunch-target "$UNPACKED/hqr" --sandbox-fallback)"
    expect "opt-out: --no-sandbox launch arg"   relaunch "$(decide --relaunch-target "$UNPACKED/hqr" -- --no-sandbox)"
    expect "opt-out: ELECTRON_DISABLE_SANDBOX"  relaunch "$(ELECTRON_DISABLE_SANDBOX=1 decide --relaunch-target "$UNPACKED/hqr")"
    ...
    QHOME="$G/qhome"; mkdir -p "$QHOME/hqr-agent"
    bash "$SCRIPT_DIR/posix.sh" --no-ui --no-marker-cleanup --desktop-pid 0 \
      --install-root "$QHOME/hqr-agent" --branch 'evil"branch\n$(x)' >/dev/null 2>&1 || true
    if python3 -c "import json,sys; d=json.load(open('$QHOME/.hqr-update-result.json')); sys.exit(0 if d['branch']=='evil\"branch\\\\n\$(x)' and d['ok']==False else 1)"; then
  ...
  launch)
    ...
    # a fake install whose `hqr` stub exits 0 instantly, so the flow
    # reaches finish() with FINAL_CODE=0 and exercises the launch leg.
    L="/tmp/hqr-launch-test.$$"
    fails=0
    expect_msg() { # name python-expr
      if python3 -c "import json,sys; d=json.load(open('$L/.hqr-update-result.json')); sys.exit(0 if ($2) else 1)"; then
        printf 'ok   %s\n' "$1"
      else
        printf 'FAIL %s -> %s\n' "$1" "$(cat "$L/.hqr-update-result.json" 2>/dev/null)"; fails=$((fails+1))
      fi
    }
    stub_install() { # creates a fake install whose hqr update succeeds
      rm -rf "$L"; mkdir -p "$L/hqr-agent/venv/bin"
      printf '#!/bin/sh\nexit 0\n' > "$L/hqr-agent/venv/bin/hqr"
      chmod +x "$L/hqr-agent/venv/bin/hqr"
    }

    # 1. linux relaunch target dies instantly -> manual downgrade in result
    stub_install
    UNPACKED="$L/hqr-agent/apps/desktop/release/linux-unpacked"
    mkdir -p "$UNPACKED"
    printf '#!/bin/sh\nexit 1\n' > "$UNPACKED/hqr"; chmod +x "$UNPACKED/hqr"
    if [ "$(uname)" != "Darwin" ]; then
      bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/hqr-agent" \
        --relaunch-target "$UNPACKED/hqr" >/dev/null 2>&1 || true
      expect_msg "instant-exit relaunch downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen HQ Runtime' in d['message']"
    else
      ...
      expect_msg "missing bundle downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen HQ Runtime' in d['message']"
    fi
    ...
    stub_install
    bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/hqr-agent" \
      --relaunch-target /opt/HQ Runtime/hqr >/dev/null 2>&1 || true
    if [ "$(uname)" != "Darwin" ]; then
