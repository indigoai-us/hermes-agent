"""Pydantic request/response models for the HQ Runtime dashboard web server.

Extracted verbatim from ``hqr_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from hqr_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``hqr model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `hqr import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`hqr -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the HQR_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under HQR_HOME.
    output: str = ""
