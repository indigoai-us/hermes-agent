"""Regression tests for #34107 — Docker UID/GID handling in ensure_hqr_home.

When HQ Runtime runs in Docker with ``HQR_UID=1000`` / ``HQR_GID=911``,
the entrypoint chowns the top-level ``HQR_HOME`` once at startup. But
...
The fix is a ``_chown_to_hqr_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_hqr_uid_gid
# ---------------------------------------------------------------------------


class TestResolveHqrUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("HQR_UID", "1000")
        monkeypatch.setenv("HQR_GID", "911")
        from hqr_cli.config import _resolve_hqr_uid_gid
...
class TestChownToHqrUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_hqr_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("HQR_UID", raising=False)
        monkeypatch.delenv("HQR_GID", raising=False)
        from hqr_cli import config as cfg
