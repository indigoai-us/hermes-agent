"""
Tests for timezone support (hqr_time module + integration points).
...
def _reset_hqr_time_cache():
    """Reset the hqr_time module cache (replacement for removed reset_cache)."""
    hqr_time._cached_tz = None
    hqr_time._cached_tz_name = None
    hqr_time._cache_resolved = False


# =========================================================================
# hqr_time.now() — core helper
# =========================================================================

class TestHqrTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_hqr_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["HQR_TIMEZONE"] = "Asia/Kolkata"
        result = hqr_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["HQR_TIMEZONE"] = "Europe/London"
        tz = hqr_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When HQR_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) HQR_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("HQR_TIMEZONE=" + os.environ.get("HQR_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "HQR_TIMEZONE=NOT_SET" in result["output"], (
            "HQR_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When HQR_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("HQR_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=hqr_tz) which shifted
        absolute time when system-local tz != HQ Runtime tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when HQ Runtime tz
        ...
        # Use a HQ Runtime timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _hqr_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["HQR_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hqr_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of HQ Runtime timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["HQR_TIMEZONE"] = "US/Eastern"
        _reset_hqr_time_cache()
