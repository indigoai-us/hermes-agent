...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("hqr_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("HQR_SESSION_KEY", session_key)
        monkeypatch.setenv("HQR_EXEC_ASK", "1")
        monkeypatch.delenv("HQR_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("hqr-verify-", "hqr-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "hqr-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/hqr-verify-example.py",
            "rm -f /tmp/hqr-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../hqr-verify-example.py",
            "rm -f /tmp/a/../../tmp/hqr-verify-example.py",
            "rm -f /var/tmp/hqr-verify-example.py",
            "rm -f /tmp/hqr-verify-*",
            "rm -f /tmp/hqr-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/hqr-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/hqr-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\hqr-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\hqr-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\hqr-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"HQR_SESSION_KEY": "bob"}, clear=False):
...
class TestHqrConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.hqr/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.hqr/config.yaml",
            "echo '  mode: off' >> ~/.hqr/config.yaml",
            "echo x | tee ~/.hqr/config.yaml",
            "echo x | tee $HQR_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.hqr/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-HQ Runtime absolute config.yaml is
        # handled by the project patterns, not the HQ Runtime-home rule.
        for cmd in (
            "cat ~/.hqr/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.hqr/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / HQ Runtime-home prefixes must fold to ~/ and
    ~/.hqr/ in dangerous-command detection.
    ...
    SSH, and HQ Runtime config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_hqr_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/HQR_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("HQR_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.hqr/hqr-agent && source venv/bin/activate && python -m hqr_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m hqr_cli.main gateway run --replace &",
            "nohup python -m hqr_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart hqr-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the HQ Runtime security config via IFS.
            "sed${IFS}-i ~/.hqr/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep hqr) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof hqr)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof hqr_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof hqr`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the HQ Runtime service
    label achieves the same effect as `hqr gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_hqr_label_detected(self):
        for cmd in (
            "launchctl stop ai.hqr.gateway",
            "launchctl kickstart -k system/ai.hqr.gateway",
            "launchctl bootout system/ai.hqr.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.hqr.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-HQ Runtime labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("HQR_GATEWAY_SESSION", "HQR_CRON_SESSION",
                      "HQR_YOLO_MODE",
                      "HQR_SESSION_KEY", "HQR_INTERACTIVE")
        }
        os.environ.pop("HQR_YOLO_MODE", None)
        os.environ.pop("HQR_INTERACTIVE", None)
        # HQR_CRON_SESSION takes priority over HQR_GATEWAY_SESSION in
        ...
        os.environ.pop("HQR_CRON_SESSION", None)
        os.environ["HQR_GATEWAY_SESSION"] = "1"
        os.environ["HQR_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("hqr_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"HQR_INTERACTIVE": "1"}, clear=False):
