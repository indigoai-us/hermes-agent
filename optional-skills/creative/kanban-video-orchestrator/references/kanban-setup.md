...
the actual `setup.sh` that creates the project workspace, configures HQ Runtime
...
2. **Create profiles** — `hqr profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.hqr/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `hqr kanban create` assigned to the director
...
```bash
hqr profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.hqr/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hqr/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most HQ Runtime
...
Each profile gets a `SOUL.md` at `~/.hqr/profiles/<name>/SOUL.md` that
...
```bash
hqr kanban create "Direct production of <video title>" \
    --assignee director \
...
the HQ Runtime `.env` (`${HQR_HOME:-$HOME/.hqr}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hqr_env="${HQR_HOME:-$HOME/.hqr}/.env"
    if grep -q "^${var}=" "$_hqr_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_hqr_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_hqr_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY hqr ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY hqr OPENROUTER_API_KEY || exit 1
# ...
```
