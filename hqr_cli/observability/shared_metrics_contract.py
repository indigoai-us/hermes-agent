"""Bounded product contract for the first HQ Runtime shared-metrics slice."""
...
SCHEMA_KEY = "hqr.metrics.schema_version"
SCHEMA_VERSION = "hqr.metrics.event.v2"
MODEL_CALL_SCOPE = "hqr.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "hqr.task_run"
TOOL_CALL_SCOPE = "hqr.tool_call"
CLIENT_ACTIVE_MARK = "hqr.client.active"
TOOL_APPROVAL_MARK = "hqr.tool_approval"
SKILL_LIFECYCLE_MARK = "hqr.skill.lifecycle"
SKILL_LOAD_MARK = "hqr.skill.load"
SUBSCRIBER_NAME = "hqr.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "hqr.client.active"
LEGACY_MODEL_CALL_METRIC = "hqr.model_call.count"
MODEL_ROUTE_METRIC = "hqr.model_route.count"
TASK_STARTED_METRIC = "hqr.task_run.started"
TASK_FINISHED_METRIC = "hqr.task_run.finished"
TOOL_CALL_METRIC = "hqr.tool_call.count"
TOOL_APPROVAL_METRIC = "hqr.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "hqr.skill.lifecycle.count"
SKILL_LOAD_METRIC = "hqr.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted HQ Runtime installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(hqr_version or "").strip()
    ...
    return {
        ...
        "hqr_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("hqr_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its HQ Runtime logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("hqr.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from hqr_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map HQ Runtime terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map HQ Runtime registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "hqr-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal HQ Runtime tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to HQ Runtime."""
