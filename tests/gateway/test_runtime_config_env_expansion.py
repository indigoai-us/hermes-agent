...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("HQR_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("HQR_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("HQR_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("HQR_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("HQR_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
