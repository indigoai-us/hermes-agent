...
developer's REAL ~/.hqr/state.db, and a pytest-spawned process flipped
...
# Must match the root the guard itself computes.  Hardcoding ``~/.hqr``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\hqr``: the paths under test were then *correctly*
...
REAL_ROOT = hqr_state._real_platform_state_root()
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.hqr/state.db must fail hard."""
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            hqr_state, "DEFAULT_DB_PATH", hqr_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_hqr_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic HQR_HOME must succeed."""
        monkeypatch.setenv("HQR_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            hqr_state, "DEFAULT_DB_PATH", hqr_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        hqr_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(hqr_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_hqr_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no HQR_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("HQR_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_hqr_home_succeeds(self, tmp_path):
        """Same child, hermetic HQR_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["HQR_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from hqr_state import SessionDB\n"
            "db = SessionDB()\n"
