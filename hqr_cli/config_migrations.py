...
``hqr_cli.config.migrate_config``. Each step is a function
...
``hqr_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``hqr_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``hqr_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``hqr_cli.config`` (e.g.
``patch("hqr_cli.config.read_raw_config", ...)``) keep working, because
...
#: HQR_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hqr_home()}/config.yaml and run `hqr setup` to "
...
def _cfg():
    """Return the live ``hqr_cli.config`` module (lazy, cycle-free)."""
    from hqr_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$HQR_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `hqr plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `hqr curator status` has no
    ...
    #   3. Creates `~/.hqr/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hqr_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_hqr_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `hqr config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `hqr config set`"
                )
...
# behavior set it to true explicitly via `hqr config set`.
