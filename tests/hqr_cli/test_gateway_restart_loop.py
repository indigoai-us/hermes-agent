...
Covers:
- Defense 1: gateway stop/restart refuse when _HQR_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "hqr gateway restart",
        "hqr gateway stop",
        "hqr  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "HQR GATEWAY RESTART",           # uppercase
    ])
    def test_hqr_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.hqr.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.hqr/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l hqr-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.hqr.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.hqr.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.hqr/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated hqr note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "hqr cron list",
        "hqr update",
        "hqr config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `hqr gateway start` is benign — starting a gateway from inside a
        ...
        "hqr gateway start",
        "hqr gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway hqr
        # services must not be falsely blocked (the old `.*hqr` matched any
        # hqr token).
        "launchctl unload ai.hqr.update-checker.plist",
        "launchctl restart ai.hqr.daemon",
        ...
        "systemctl restart hqr-meta.service",
        "systemctl restart hqr-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hqr_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade hqr then run hqr gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_hqr_gateway_once.sh). The script must live under
        # HQR_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("HQR_HOME", str(tmp_path / ".hqr"))
        scripts_dir = tmp_path / ".hqr" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify hqr gateway stop/restart refuse when _HQR_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_HQR_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_HQR_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _HQR_GATEWAY=1.

    Issue #37453: systemctl --user restart hqr-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart hqr-gateway",
        "systemctl --user restart hqr-gateway",
        "systemctl stop hqr-gateway.service",
        "hqr gateway restart",
        "launchctl kickstart gui/501/ai.hqr.gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart hqr-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.hqr.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-hqr label: label-independent detection is the point
        # (#62891 second reproduction used `ai.hqr.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.hqr.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.hqr.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.hqr.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-hqr systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run hqr gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("hqr gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under HQR_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.hqr.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/hqr\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 hqr gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_hqr_home()` falls back to Path.home(), which raises
        RuntimeError when neither HQR_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    hqr_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run hqr gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HQR_HOME", str(tmp_path / ".hqr"))
        (tmp_path / ".hqr").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run hqr gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop hqr-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart hqr-gateway' /var/log/syslog",
        "rg 'hqr gateway restart' /home/user/.hqr/logs/",
        "journalctl -u hqr-gateway --grep 'systemctl restart hqr-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'hqr_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell hqr gateway restart"',
        'psql -c "\\! systemctl restart hqr-gateway"',
        "grep 'systemctl restart hqr-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart hqr-gateway",
        'grep "$(systemctl restart hqr-gateway)" f',
        "grep 'restart' log; systemctl restart hqr-gateway",
        'sqlite3 db "SELECT 1"; hqr gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "hqr gateway restart",
        "sudo systemctl stop hqr-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart hqr-gateway%'\""
        )
