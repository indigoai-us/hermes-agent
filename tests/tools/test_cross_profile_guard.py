...
def fake_hqr(tmp_path, monkeypatch):
    """Build a two-profile HQ Runtime layout and point HQR_HOME at
    the hqr-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("HQR_HOME", str(sec_home))

    import hqr_constants
    monkeypatch.setattr(hqr_constants, "get_default_hqr_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_hqr_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_hqr):
        from tools.file_tools import write_file_tool
        target = fake_hqr["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hqr):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_hqr["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "hqr-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_hqr_path_unaffected(self, fake_hqr, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_hqr):
    ...
    def test_cross_profile_patch_bypass(self, fake_hqr):
        from tools.file_tools import patch_tool
        target = fake_hqr["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_hqr):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_hqr, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_hqr["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up HQR_HOME (set in
        ...
        assert "not found in active profile 'hqr-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.hqr/profiles/<name>/."""
        # Don't set HQR_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_hqr_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_hqr_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hqr):
        """When active profile is 'hqr-security', the prompt warns
        ...
        assert "Active HQ Runtime profile" in src
        assert "cross_profile=True" in src
        assert "~/.hqr/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active HQ Runtime profile: default" in src
        assert "Active HQ Runtime profile: {active_profile}" in src
