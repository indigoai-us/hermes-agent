...
profile's HQR_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.hqr/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active HQR_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever HQR_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    HQR_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import hqr_constants

    monkeypatch.setattr(
        hqr_constants, "_get_platform_default_hqr_home", lambda: root
    )
    monkeypatch.setenv("HQR_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile HQR_HOME (<root>/profiles/<name>), the cron store
    ...
    assert hqr_constants.get_hqr_home().resolve() == profile_home.resolve()
    assert hqr_constants.get_default_hqr_root().resolve() == root.resolve()

    # cron/jobs.py computes HQR_DIR from get_hqr_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.HQR_DIR.resolve() == profile_home.resolve()
        assert (
