"""Unified provider-credential lifecycle across every store HQ Runtime reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.hqr/.env``                     — the canonical secret store
    2. ``~/.hqr/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.hqr/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `hqr auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``hqr auth add``.
    """
    from hqr_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from hqr_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from hqr_cli.config import load_env, remove_env_value
