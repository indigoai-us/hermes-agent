...
def auth_json_path():
    """Return the HQ Runtime auth store path, respecting HQR_HOME overrides."""
    return get_hqr_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`hqr tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a HQ Runtime release, and
