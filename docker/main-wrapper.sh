#!/bin/sh
# shellcheck shell=sh
# /opt/hqr/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching HQR_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `hqr` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `hqr <args>` (subcommand passthrough)
#
# Drop to hqr via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${HQR_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${HQR_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export HQR_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset HQR_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid hqr "$@"; exec "$@"; }
...
# an arbitrary non-root, non-hqr UID, the bootstrap was skipped and the
# baked image dirs (owned by the hqr build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hqr)" ]; then
    cat >&2 <<EOF
[hqr] ERROR: container started with --user $cur_uid (an arbitrary, non-hqr UID) — not supported.
...
# to the hqr user's home before dropping privileges so libraries that
...
_hqr_orig_cwd="${HQR_ORIG_CWD:-$PWD}"
...
# the user's command so `hqr chat` starts in the Docker -w
...
# HQ Runtime subcommand pass-through.
drop hqr "$@"
