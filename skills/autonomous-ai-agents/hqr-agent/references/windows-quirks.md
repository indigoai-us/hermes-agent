...
HQ Runtime runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`hqr config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually HQ Runtime's env scrubber dropping
...
HQ Runtime-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every HQ Runtime tool and most Windows APIs. Prefer forward slashes in code
