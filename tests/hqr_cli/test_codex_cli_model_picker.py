...
Covers:
 - Normal path (tokens already in HQ Runtime auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — HQ Runtime
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `hqr auth openai-codex`. The old
...
@pytest.fixture()
def hqr_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in HQ Runtime auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(hqr_auth_only_env):
    """openai-codex appears when tokens are already in HQ Runtime auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or HQ Runtime
    auth store."""
    hqr_home = tmp_path / ".hqr"
    hqr_home.mkdir()
