"""
Backup and import commands for hqr CLI.

`hqr backup` creates a zip archive of the entire ~/.hqr/ directory
(excluding the hqr-agent repo and transient files).

`hqr import` restores from a backup zip, overlaying onto the current
HQR_HOME root.
"""
...
from hqr_constants import get_default_hqr_root, get_hqr_home, display_hqr_home
from utils import (
    _preserve_file_mode,
...
from hqr_cli.sizefmt import format_bytes as _format_size
...
# ``hqr-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/hqr-agent/``
...
# HQR_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "hqr-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under HQR_HOME) —
...
# File names that ``hqr import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE HQR_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the HQ Runtime backup slot."""
...
@contextmanager
def _backup_operation_lock(hqr_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = hqr_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another HQ Runtime backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside HQR_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to hqr root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``hqr-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/hqr-agent/`` — must be preserved.
        if part == "hqr-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside HQR_HOME.
...
# to the user as a hung `hqr update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # hqr_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the HQ Runtime home directory."""
    hqr_root = get_default_hqr_root()

    if not hqr_root.is_dir():
        print(f"Error: HQ Runtime home directory not found at {hqr_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(hqr_root):
            _run_backup_locked(args, hqr_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, hqr_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"hqr-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"hqr-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_hqr_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(hqr_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(hqr_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``hqr-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "hqr-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(hqr_root)
    ...
    # outside HQR_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_hqr_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: hqr import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a HQ Runtime backup.
    ...
    # Look for telltale files that a hqr home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a HQ Runtime backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.hqr/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a hqr dir name
        if prefix in {".hqr", "hqr"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``hqr import``, which is the
    ...
    so without the chown a ``sudo hqr import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``hqr import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to HQ Runtime' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_hqr_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (hqr_root / "config.yaml").exists()
        has_env = (hqr_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has HQ Runtime configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under HQR_HOME.
            ...
            try:
                target.resolve().relative_to(hqr_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_hqr_home()}."
            )
        ...
        profiles_dir = hqr_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # hqr_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: hqr profile list  (after installing hqr)")

        # Guidance
        print()
        if not (hqr_root / "hqr-agent").is_dir():
            print("Note: The hqr-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: hqr update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  hqr -p {pname} gateway install")
        ...
        try:
            from hqr_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your HQ Runtime configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and hqr backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to HQR_HOME).
...
# — `hqr update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $HQR_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside HQR_HOME and the entry is silently
...
def _quick_snapshot_root(hqr_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: hqr snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(hqr_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside HQR_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``hqr update``.
    ...
    Args:
        ...
        hqr_home: Override for the HQ Runtime home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(hqr_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for hqr backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_hqr_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, hqr_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(hqr_root):
            return _write_full_zip_backup_locked(out_path, hqr_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, hqr_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``hqr_root`` to ``out_path``.
...
def _pre_update_backup_dir(hqr_home: Optional[Path] = None) -> Path:
    home = hqr_home or get_hqr_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of HQR_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<HQR_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``hqr update``) should continue even if the backup fails.
    """
    hqr_root = hqr_home or get_default_hqr_root()
    if not hqr_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(hqr_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, hqr_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `hqr claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of HQR_HOME under ``backups/`` before a
    ``hqr claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``hqr import <archive>``.  Writes to
    ``<HQR_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `hqr import` and the
