"""Per-terminal session breadcrumbs for ``hqr -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$HQR_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``hqr -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_hqr_home() / "terminal-sessions"
