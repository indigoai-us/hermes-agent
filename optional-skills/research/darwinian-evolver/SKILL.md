...
version: 0.1.0
author: Bihruze (Asahi0x), HQ Runtime Agent
license: MIT
...
import upstream classes into HQ Runtime itself.
...
```bash
mkdir -p ~/.hqr/cache/darwinian-evolver && cd ~/.hqr/cache/darwinian-evolver
...
```bash
cd ~/.hqr/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hqr/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hqr/skills/research/darwinian-evolver
DE_DIR=~/.hqr/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside HQ Runtime core.
   Custom driver scripts under `~/.hqr/skills/...` are user-side and fine.
