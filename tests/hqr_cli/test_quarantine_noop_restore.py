...
``hqr*.exe`` shims aside (``hqr.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``hqr`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("hqr", "hqr-agent", "hqr-acp", "hqr-gateway"):
...
# ---------------------------------------------------------------------------
# hqr_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "hqr.exe" in names, "hqr.exe must be restored after a no-op install"
    assert "hqr-acp.exe" in names
    assert "hqr-gateway.exe" in names
    assert (scripts / "hqr.exe").read_bytes() == b"MZ-old-hqr"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "hqr.exe").read_bytes() == b"MZ-new-hqr"
...
# ---------------------------------------------------------------------------
# hqr_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "hqr.exe" in names, "hqr.exe must be restored after a no-op recovery install"
    assert (scripts / "hqr.exe").read_bytes() == b"MZ-old-hqr"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "hqr.exe").write_bytes(b"MZ-new-hqr")
