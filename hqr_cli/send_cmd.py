"""CLI subcommand: ``hqr send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"hqr send: {file_path} is not a text file. --file reads the "
                ...
                f'  hqr send --to telegram "MEDIA:{file_path}"\n'
                f'  hqr send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  hqr send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"hqr send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"hqr send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"hqr send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"hqr send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `hqr send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"hqr send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `hqr gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.hqr/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hqr_env() -> None:
    """Populate ``os.environ`` from ``~/.hqr/.env`` AND bridge top-level
    ...
    startup that ``hqr send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.hqr/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.hqr/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``hqr send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.hqr/.env and ~/.hqr/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "hqr send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  hqr send --to telegram \"hello\"\n"
            "  hqr send --to discord:#ops --file report.md\n"
            "  hqr send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "hqr send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `hqr send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform HQ Runtime "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.hqr/.env + ~/.hqr/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  hqr send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | hqr send --to telegram:-1001234567890\n"
            "  hqr send --to discord:#ops --file /tmp/report.md\n"
            "  hqr send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  hqr send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  hqr send --list                  # all platforms\n"
            "  hqr send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `hqr send --list telegram`.",
    )
