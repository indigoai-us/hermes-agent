...
When HQ Runtime runs in TUI mode, the gateway child process communicates with
...
# ``get_hqr_home() / "plugins"`` (user) and ``./.hqr/plugins/`` (project,
# gated behind ``HQR_ENABLE_PROJECT_PLUGINS``) — see
# ``hqr_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``hqr_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import hqr_constants (this script
    ...
    # plugins in get_hqr_home()/plugins/ and project plugins in
    # ./.hqr/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_hqr_home() / "plugins"]
    if os.environ.get("HQR_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".hqr" / "plugins")
