...
## 5. HQ Runtime Configuration

### Config location

`$HQR_HOME/config.yaml` (defaults to `~/.hqr/config.yaml` when `HQR_HOME` is unset)
...
Restart the HQ Runtime session for changes to take effect. The MCP connection is
