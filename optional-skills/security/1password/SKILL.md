...
version: 1.0.0
author: arceus77-7, enhanced by HQ Runtime Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during HQ Runtime terminal calls (desktop app flow only)
...
### Service Account (recommended for HQ Runtime)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HQR_HOME:-~/.hqr}/.env` (the skill will prompt for this on first load).
...
## HQ Runtime Execution Pattern (desktop app flow)

HQ Runtime terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hqr-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hqr-op.sock"
