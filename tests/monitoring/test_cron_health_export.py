...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("hqr.gateway.up"), _M("hqr.gateway.active_agents"),
        _M("hqr.gateway.busy"), _M("hqr.gateway.drainable"),
        _M("hqr.gateway.restart_requested"),
        _M("hqr.platform.up"), _M("hqr.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("hqr.cron.scheduler.heartbeat_age_seconds"),
        _M("hqr.cron.scheduler.last_success_age_seconds"),
        _M("hqr.cron.scheduler.catch_up_occurrences"),
        _M("hqr.cron.jobs.enabled"), _M("hqr.cron.jobs.running"),
        _M("hqr.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "HQ Runtime Agent-owned Relay transport health" in text
