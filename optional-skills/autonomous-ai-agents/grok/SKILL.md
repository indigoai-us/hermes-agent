...
version: 0.1.1
author: Matt Maximo (MattMaximo), HQ Runtime Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hqr:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, hqr-agent]
---

# Grok Build CLI — HQ Runtime Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the HQ Runtime terminal. Grok
...
1. Prepare stable input files first with HQ Runtime tools (`read_file`,
...
2. **Don't conflate HQ Runtime' xAI auth with the `grok` CLI's auth.** HQ Runtime'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume HQ Runtime' xAI auth carries
   over.
