"""
Configuration management for HQ Runtime Agent.

Config files are stored in ~/.hqr/ for easy access:
- ~/.hqr/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.hqr/.env         - API keys and secrets

This module provides:
- hqr config          - Show current configuration
- hqr config edit     - Open config in editor
- hqr config get      - Print a resolved configuration value
- hqr config set      - Set a specific value
- hqr config unset    - Remove a user configuration value
- hqr config wizard   - Re-run setup wizard
"""
...
from hqr_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``hqr config set`` (which rewrites
    ...
    ``config.yaml`` in place: hqr never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.hqr/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``hqr logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  hqr config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` HQ Runtime makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. HQ Runtime itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``hqr update``, the TUI build.
...
#   on every plugin install / ``hqr update``.
...
# * ``HQR_HOME`` / ``HQR_PROFILE`` / ``HQR_CONFIG`` /
#   ``HQR_ENV`` — HQ Runtime runtime location flags. Writing these into
...
#
# IMPORTANT: ``HQR_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (HQR_LANGFUSE_PUBLIC_KEY,
# HQR_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # HQ Runtime runtime location — never via dashboard env writer.
    # NOT a HQR_* blanket: integration credentials (HQR_GEMINI_*,
    # HQR_LANGFUSE_*, HQR_SPOTIFY_*, ...) ARE allowed.
    "HQR_HOME", "HQR_PROFILE", "HQR_CONFIG", "HQR_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or HQ Runtime runtime location "
            "(HQR_HOME, HQR_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # HQR_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `hqr plugins enable
    # observability/langfuse` or `hqr tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "HQR_LANGFUSE_ENV",
    "HQR_LANGFUSE_RELEASE",
    "HQR_LANGFUSE_SAMPLE_RATE",
    "HQR_LANGFUSE_MAX_CHARS",
    "HQR_LANGFUSE_CAPTURE",
    "HQR_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    ...
    "HQR_ACP_AUTH_METHOD",
    "HQR_ACP_AUTO_APPROVE",
    "HQR_COPILOT_ACP_COMMAND",
    "HQR_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from hqr_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set HQR_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("HQR_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_hqr_home() / ".managed"
...
def is_managed() -> bool:
    """Check if HQ Runtime is running in package-manager-managed mode.

    Two signals: the HQR_MANAGED env var (set by the systemd service),
    or a .managed marker file in HQR_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update HQ Runtime through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``hqr_cli/`` — i.e. the git checkout for source
    installs, ``/opt/hqr`` inside the published image. It is a property of
    the running interpreter, NOT of ``$HQR_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how HQ Runtime was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$HQR_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. HQR_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.hqr``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$HQR_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.hqr:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$HQR_HOME``, a
    ...
    ``docker`` and ``hqr update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/hermes-agent`` image bakes a ``docker``
        stamp into ``/opt/hqr`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $HQR_HOME.
    try:
    ...
    #    host install whose shared $HQR_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``hqr update``.
    try:
        method = (
            (get_hqr_home() / ".install_method")
    ...
    # detect Nix installs that don't set HQR_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``hqr_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HQR_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/hqr`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``hqr update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# hqr_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side HQ Runtime, it doesn't update
...
✗ ``hqr update`` doesn't apply inside the Docker container.
...
pulling a fresh image and restarting your container instead:
  ...
  docker compose up -d --force-recreate hqr-agent
...
Notes:
  ...
  • Your config and session history live under ``$HQR_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``hqr update`` inside Docker.
...
def format_managed_message(action: str = "modify this HQ Runtime installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("HQR_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this HQ Runtime installation is managed by NixOS "
            f"(HQR_MANAGED={env_hint}).\n"
            "Edit services.hqr-agent.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this HQ Runtime installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall HQ Runtime."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HQR_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hqr_bin
    or None if container mode is not active, we're already inside the
    container, or HQR_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("HQR_DEV") == "1":
        return None
    ...
    container_mode_file = get_hqr_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "hqr-agent")
    exec_user = info.get("exec_user", "hqr")
    hqr_bin = info.get("hqr_bin", "/data/current-package/bin/hqr")
...
# Re-export from hqr_constants — canonical definition lives there.
from hqr_constants import get_hqr_home, get_process_hqr_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_hqr_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_hqr_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_hqr_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the HQR_UID / HQR_GID env vars set by Docker deployments.

    Docker containers running HQ Runtime commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level HQR_HOME once, but
    subdirectories created at runtime by ``ensure_hqr_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("HQR_UID", "").strip()
    gid_str = os.environ.get("HQR_GID", "").strip()
    try:
...
def _chown_to_hqr_uid(path) -> None:
    """Chown ``path`` to ``HQR_UID:HQR_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_hqr_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_hqr_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the hqr group can
    share state with the gateway service.

    The mode can be overridden via the HQR_HOME_MODE environment variable
    (e.g. HQR_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HQR_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``HQR_UID``/``HQR_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HQR_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_hqr_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When HQ Runtime runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("HQR_CONTAINER") or os.environ.get("HQR_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set HQR_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HQR_HOME, upgrading legacy empty templates.
...
# ensure_hqr_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_HQR_HOME_ENSURED: set = set()


def ensure_hqr_home():
    """Ensure ~/.hqr directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``HQR_HOME`` we only re-run
    ...
    ``get_hqr_home()`` and therefore re-run for the new path.
    """
    home = get_hqr_home()
    key = str(home)

    if key in _HQR_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``hqr profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old HQR_HOME would resurrect an empty skeleton
    ...
    _HQR_HOME_ENSURED.add(key)


def _ensure_hqr_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HQR_HOME {home} does not exist. "
...
from hqr_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `hqr config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.hqr.config`` entries, then checks
    ...
    try:
        ...
        # should never break `hqr update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but HQ Runtime' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older HQ Runtime versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``hqr_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (hqr_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (hqr_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (hqr_cli/setup.py)
    "known_plugin_toolsets", # written/read by hqr_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'hqr setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — HQ Runtime won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, hqr send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'hqr doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "hqr update / hqr -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from hqr_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ hqr config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in hqr_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from hqr_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from hqr_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: hqr config set <key> <value>")
    ...
    # `hqr update`, but only the version bump is persisted.
    ...
    # metadata.hqr.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.hqr/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_hqr_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``HqrCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``hqr config set``
    ...
    backends) — while display paths (``hqr status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _hqr_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in hqr_cli.personality.
from hqr_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`hqr_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from hqr_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from hqr_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``HQR_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`hqr_cli.personality` (single owner).
    """
    from hqr_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.hqr/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_hqr_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.hqr/config.yaml.
    ...
    (which change ``HQR_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the HQ Runtime host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/hqr/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_hqr_dotenv()
...
#   openai-codex (OAuth — hqr auth) — OpenAI Codex
#   nous         (OAuth — hqr auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.hqr/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset HQ Runtime writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.hqr/.env.
    ...
    interactive menu render (`hqr tools`, `hqr setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `hqr tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.hqr/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.hqr/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``HQR_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"HQR_CUSTOM_{slug}_API_KEY" if slug else "HQR_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.hqr/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.hqr/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to HQ Runtime — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known HQ Runtime vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.hqr/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.hqr/.env`` over ``os.environ``.

    Used for HQ Runtime-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ HQ Runtime Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from hqr_cli.auth import get_anthropic_key
    ...
    # Warn on stale HQR_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("HQR_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale HQR_MAX_ITERATIONS={_env_ghost} "
                f"(run 'hqr doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from hqr_cli.personality import active_personality_name
    ...
    print(color("  hqr config edit     # Edit config file", Colors.DIM))
    print(color("  hqr config set <key> <value>", Colors.DIM))
    print(color("  hqr setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``HQR_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("HQR_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `hqr cron list`, then pin the "
        "intended values with `hqr cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (hqr_cli/plugins_cmd.py, hqr_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `hqr config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``hqr config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``hqr config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``hqr config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (hqr config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        if value.lower() in {'true', 'yes', 'on'}:
            ...
            #   hqr config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   hqr config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``hqr config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``hqr config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    hqr config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    hqr config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `hqr config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_hqr_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `hqr config set model.api_key cfut_...` routes to config.yaml
    ...
    if not is_known and not force:
        print(color(
            ...
            "but HQ Runtime may not read it.",
            Colors.YELLOW,
        ))
        if suggestion:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `hqr config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: hqr config get <key> [--json]")
            print()
            print("Examples:")
            print("  hqr config get model")
            print("  hqr config get terminal.backend")
            print("  hqr config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: hqr config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  hqr config set model anthropic/claude-sonnet-4")
            print("  hqr config set terminal.backend docker")
            print("  hqr config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: hqr config unset <key>")
            print()
            print("Examples:")
            print("  hqr config unset model")
            print("  hqr config unset terminal.backend")
            print("  hqr config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'hqr config migrate' to add them")
        ...
        print("  hqr config           Show current configuration")
        print("  hqr config edit      Open config in editor")
        print("  hqr config get <key>          Print a resolved config value")
        print("  hqr config set <key> <value>   Set a config value")
        print("  hqr config unset <key>        Remove a config value")
        print("  hqr config check     Check for missing/outdated config")
        print("  hqr config migrate   Update config with new options")
        print("  hqr config path      Show config file path")
        print("  hqr config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``hqr config`` UI so users
