"""disk_cleanup — ephemeral file cleanup for HQ Runtime Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under HQR_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly HQR_HOME and /tmp/hqr-*
Never touches: ~/.hqr/logs/ or any system directory.
"""
...
try:
    ...
    def get_hqr_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("HQR_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".hqr").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$HQR_HOME/logs/``."""
    return get_hqr_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$HQR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HQR_HOME or ``/tmp/hqr-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    hqr_home = get_hqr_home()
    try:
        path.resolve().relative_to(hqr_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/hqr-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("hqr-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "hqr-agent", "backups", "profiles", ".worktrees",
...
# Paths under $HQR_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so HQR_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        hqr_home = get_hqr_home()
        for parent in ("cron", "cronjobs"):
            base = hqr_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside HQR_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HQR_HOME, but never recurse into known
    # durable state trees.  Some installs place the HQ Runtime checkout, venv,
    # and desktop build under HQR_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    hqr_home = get_hqr_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in hqr_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    hqr_home = get_hqr_home()
    try:
        rel = path.resolve().relative_to(hqr_home)
        ...
        # Path isn't under HQR_HOME (e.g. /tmp/hqr-*) — fall through.
        pass
