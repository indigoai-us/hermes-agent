"""``hqr logs`` — view and filter HQ Runtime log files.
...
Usage examples::
    ...
    hqr logs                    # last 50 lines of agent.log
    hqr logs -f                 # follow agent.log in real time
    hqr logs errors             # last 50 lines of errors.log
    hqr logs gateway -n 100    # last 100 lines of gateway.log
    hqr logs gui -f            # follow gui.log (dashboard/pty/ws)
    hqr logs desktop -f        # follow desktop.log (Electron app boot/backend)
    hqr logs --level WARNING    # only WARNING+ lines
    hqr logs --session abc123   # filter by session ID substring
    hqr logs --component tools  # only tool-related lines
    hqr logs --since 1h         # lines from the last hour
    hqr logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_hqr_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when HQ Runtime runs — try 'hqr chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_hqr_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_hqr_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_hqr_home()}/logs/")
        return

    print(f"Log files in {display_hqr_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'hqr chat' to generate logs)")
