"""Regression tests for #76414: `hqr honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("hqr.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("hqr_work"). The lookup
...
a real honcho.json (temp HQR_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "hqr": {"peerName": "alice", "aiPeer": "hqr"},
            "hqr_work": {"peerName": "alice", "aiPeer": "hqr"},
            "hqr_my_profile": {"peerName": "bob", "aiPeer": "hqr"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "hqr_cli.profiles.list_profiles",
        ...
        assert host == "hqr_work"  # not "hqr.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'hqr_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("hqr.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["hqr_work"]
        cfg["hosts"]["hqr.work"] = {"peerName": "carol", "aiPeer": "hqr"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "hqr_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'hqr.work' (or '(not set)')."""
        monkeypatch.setattr(
            "hqr_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "hqr.work" not in out
        ...
        assert "alice" in work_line and "hqr" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "hqr.new" not in out  # well-formed key, no dot-form leak
