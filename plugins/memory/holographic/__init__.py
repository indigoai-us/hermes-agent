"""hqr-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $HQR_HOME/config.yaml (profile-scoped):
  plugins:
    hqr-memory-store:
      db_path: $HQR_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "hqr-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hqr_home):
        """Write config to config.yaml under plugins.hqr-memory-store."""
        from pathlib import Path
        config_path = Path(hqr_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["hqr-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_hqr_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _hqr_home = str(get_hqr_home())
        _default_db = _hqr_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $HQR_HOME in user-supplied paths so config values like
        # "$HQR_HOME/memory_store.db" or "~/.hqr/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$HQR_HOME", _hqr_home)
            db_path = db_path.replace("${HQR_HOME}", _hqr_home)
