"""
Single source of truth for provider identity in HQ Runtime Agent.
...
2. **HQ Runtime overlays** — transport type, auth patterns, aggregator flags,
...
# -- HQ Runtime overlay ----------------------------------------------------------
# HQ Runtime-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HqrOverlay:
    """HQ Runtime-specific provider metadata layered on top of models.dev."""
...
HQR_OVERLAYS: Dict[str, HqrOverlay] = {
    "moa": HqrOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": HqrOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": HqrOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": HqrOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": HqrOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": HqrOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HQR_QWEN_BASE_URL",
    ),
    "lmstudio": HqrOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": HqrOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": HqrOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": HqrOverlay(
    ...
    "alibaba-coding-plan": HqrOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "hqr", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. HQ Runtime overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + HQ Runtime overlay
    ...
    overlay = HQR_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + hqr extra
    ...
    if overlay is not None:
        # HQ Runtime-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The HQ Runtime overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in HQR_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact HQ Runtime provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from hqr_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
