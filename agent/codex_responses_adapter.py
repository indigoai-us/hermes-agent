...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``hqr-tools`` server already overflows 64 chars,
...
# output items.  HQ Runtime injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while HQ Runtime-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on HQ Runtime threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: HQ Runtime'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a HQ Runtime-side metadata key and not part
