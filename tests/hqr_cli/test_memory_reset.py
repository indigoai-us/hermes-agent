"""Tests for the `hqr memory reset` CLI command.
...
- Profile-scoped reset (uses HQR_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake HQR_HOME with memory files."""
    hqr_home = tmp_path / ".hqr"
    memories = hqr_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("HQR_HOME", str(hqr_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHQ Runtime repo is at ~/.hqr/hqr-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `hqr memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        hqr_home = tmp_path / ".hqr"
        (hqr_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("HQR_HOME", str(hqr_home))
