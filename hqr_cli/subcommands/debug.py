...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for HQ Runtime Agent. Use 'hqr debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hqr debug share              Upload debug report (asks for confirmation)
    hqr debug share --yes        Skip confirmation (for scripts/CI)
    hqr debug share --lines 500  Include more log lines
    hqr debug share --expire 30  Keep paste for 30 days
    hqr debug share --local      Print report locally (no upload)
    hqr debug share --no-redact  Disable upload-time secret redaction
    hqr debug share --nous       Upload to Nous-internal storage (private)
    hqr debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'hqr debug share'",
    )
