...
HQR_HOME_DIR="${HQR_HOME:-$HOME/.hqr}"
HQR_CFG="${HQR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure HQ Runtime config has twozero_td MCP entry ──
if [[ ! -f "$HQR_CFG" ]]; then
    echo -e " ${FAIL} HQ Runtime config not found at ${HQR_CFG}"
    manual_steps+=("Create ${HQR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HQR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in HQ Runtime config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to HQ Runtime config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${HQR_CFG} manually"); }
    manual_steps+=("Restart HQ Runtime session to pick up config change")
fi
