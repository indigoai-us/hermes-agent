...
All per-subject data lives under PDD_DATA_DIR (default: $HQR_HOME/unbroker),
which is the same trust boundary HQ Runtime uses for .env and OAuth tokens.
"""
...
def hqr_home() -> Path:
    return Path(os.environ.get("HQR_HOME") or (Path.home() / ".hqr"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else hqr_home() / "unbroker"
