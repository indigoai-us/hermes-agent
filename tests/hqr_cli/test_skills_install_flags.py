"""
Tests for --yes / --force flag separation in `hqr skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("hqr_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["hqr", "skills", "install", "official/email/agentmail", "--yes"],
    )
