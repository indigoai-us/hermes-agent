...
_PRIVATE_BROWSER_SENTINEL = "_HQR_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# hqr: pin this named session to its own tab (once per daemon process)
def _hqr_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "hqr-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from HQ Runtime's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at HQ Runtime's venv site-packages, and a
    ...
    # needs HQ Runtime's import path.
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_hqr_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `hqr tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """HQ Runtime' own bin dir ($HQR_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from hqr_constants import get_hqr_home

        return str(Path(get_hqr_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: HQ Runtime' own ``$HQR_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    HQ Runtime-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: HQ Runtime' managed uv (bootstrapped on demand via
    ``hqr_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$HQR_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical HQ Runtime-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_hqr_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `hqr tools` → Browser Automation."
        )
