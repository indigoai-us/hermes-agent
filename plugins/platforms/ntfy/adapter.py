"""ntfy platform adapter (HQ Runtime plugin).
...
a HQ Runtime dependency.

This adapter ships as a HQ Runtime platform plugin under
``plugins/platforms/ntfy/``. The HQ Runtime plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "hqr-in"              # subscribe topic (incoming)
          publish_topic: "hqr-out"     # optional — defaults to topic
...
_ECHO_TAG = "hqr-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``hqr cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a HQ Runtime dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `hqr gateway status` without
