...
from hqr_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every HQ Runtime secret
...
def _build_browser_env() -> dict:
    ...
    Strips HQ Runtime-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``hqr_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = hqr_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    hqr_node_bin = str(hqr_home / "node" / "bin")
    hqr_node_root = str(hqr_home / "node")
    hqr_nm_bin = str(hqr_home / "node_modules" / ".bin")
    return [hqr_node_bin, hqr_node_root, hqr_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``hqr doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.hqr/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active HQ Runtime profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.hqr/plugins/browser/<vendor>/``) participate
...
# comparison sites (four here, plus hqr_cli/tools_config.py and
# hqr_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when HQ Runtime silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — HQ Runtime-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the HQ Runtime CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-hqr_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a hqr
...
# Hard ceiling for a daemon whose owning hqr process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed hqr processes — this way every clean hqr exit sweeps
    ...
    # Sweep orphans from other crashed hqr processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live hqr processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current hqr PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live hqr process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    hqr restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning hqr process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced hqr PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent hqr instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-hqr_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live hqr process and must not be touched — but "owner alive"
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the HQ Runtime-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy HQ Runtime-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_hqr_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, HQ Runtime-managed
    ...
    # node_modules binary, which disappears on the next ``hqr update`` and
    ...
    # Build an extended search PATH including HQ Runtime-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_hqr_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_hqr_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_hqr_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``hqr update`` (or ``hqr doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential HQ Runtime holds, while running registry-fetched
    npm code on every ``hqr update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `hqr update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this hqr PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_hqr_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = hqr_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise HQ Runtime
    ...
    from hqr_constants import get_hqr_dir
    screenshots_dir = get_hqr_dir("cache/screenshots", "browser_screenshots")
