...
_HQR_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (HQR_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `hqr tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (HQR_KANBAN_TASK env "
    ...
    # Coding posture (base HQ Runtime — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in hqr_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full HQ Runtime toolsets (CLI + messaging platforms)
    #
    ...
    # the `hqr send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "hqr-cli": {
        ...
        "tools": _HQR_CORE_TOOLS,
        "includes": []
    },

    "hqr-cron": {
        # Mirrors hqr-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `hqr tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hqr-cli; gated by `hqr tools`",
        "tools": _HQR_CORE_TOOLS,
        "includes": []
    },
    ...
    "hqr-email": {
        "description": "Email bot toolset - interact with HQ Runtime via email (IMAP/SMTP)",
        "tools": _HQR_CORE_TOOLS,
        "includes": []
    },
    ...
    "hqr-sms": {
        "description": "SMS bot toolset - interact with HQ Runtime via SMS (Twilio)",
        "tools": _HQR_CORE_TOOLS,
        "includes": []
    },
    ...
    "hqr-gateway": {
        ...
        "tools": [],
        "includes": ["hqr-telegram", "hqr-discord", "hqr-whatsapp", "hqr-slack", "hqr-signal", "hqr-bluebubbles", "hqr-homeassistant", "hqr-email", "hqr-sms", "hqr-mattermost", "hqr-matrix", "hqr-dingtalk", "hqr-feishu", "hqr-wecom", "hqr-wecom-callback", "hqr-weixin", "hqr-qqbot", "hqr-webhook", "hqr-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``hqr-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_HQR_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``hqr-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (hqr-<name>).
        # Gives them _HQR_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("hqr-"):
            platform_name = name[len("hqr-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_HQR_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
