...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad HQR_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: HQR_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hqr_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "HQR_HOME": "/h", "HQR_PROFILE": "p",
        "HQR_CONFIG": "/c.yaml", "HQR_ENV": "/e",
        "HQR_DELEGATED_CHILD_CONTEXT": "1",
        # other HQR_* → dropped (broad prefix removed)
        "HQR_BASE_URL": "https://x", "HQR_INTERACTIVE": "1",
        "HQR_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "HQR_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "HQR_BASE_URL", "HQR_INTERACTIVE", "HQR_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped HQR_* vars."""
    import logging
