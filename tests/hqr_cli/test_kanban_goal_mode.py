...
2. Spawn: _default_spawn sets the HQR_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (hqr kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """hqr kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from hqr_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("hqr_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("hqr_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("hqr_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("hqr_cli.kanban._worker_run_id_for", lambda _: None)
