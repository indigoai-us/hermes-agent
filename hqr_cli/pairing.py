...
Usage:
    hqr pairing list              # Show all pending + approved users
    hqr pairing approve <platform> <request-id|code>  # Approve a pairing request
    hqr pairing revoke <platform> <user_id> # Revoke user access
    hqr pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle hqr pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: hqr pairing {list|approve|revoke|clear-pending}")
        print("Run 'hqr pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: hqr pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.hqr/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'hqr pairing list' to see pending requests.\n")
