"""Guard: HQ Runtime-owned subprocesses must not resolve managed runtimes by bare PATH.

HQ Runtime installs runtimes for itself — ``uv`` at ``$HQR_HOME/bin/uv``, Node at
``$HQR_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in HQ Runtime's own
...
* when a system copy also exists, the one HQ Runtime does not own wins — which is
...
``iter_hqr_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes HQ Runtime provisions into HQR_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not HQ Runtime-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("hqr_cli/gateway.py", "node"): (
        "Fallback rung of _append_node_dir_for_service(), after the managed "
        "dirs from iter_hqr_node_dirs() are already appended."
    ),
    ("hqr_cli/main.py", "node"): (
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a HQ Runtime-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$HQR_HOME/bin (uv) and $HQR_HOME/node are not on an "
        ...
        "  node/npm -> hqr_constants.find_node_executable()\n"
        "  PATH env -> hqr_constants.iter_hqr_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(hqr_constants, helper))


def test_managed_uv_helpers_exist():
    from hqr_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
