...
def _default_registry_path() -> Path:
    return get_hqr_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_hqr_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_hqr_home = expected_hqr_home if expected_hqr_home is not None else str(get_hqr_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["HQR_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("hqr_home") or "")
        if got_home and got_home != self.expected_hqr_home:
            raise RuntimeError(f"compute host HQR_HOME mismatch: {got_home} != {self.expected_hqr_home}")
