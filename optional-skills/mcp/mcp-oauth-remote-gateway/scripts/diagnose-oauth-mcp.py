...
Usage:
  ...
  <server_name> matches the files in $HQR_HOME/mcp-tokens/<server>.json etc.
...
def _hqr_home():
    # Prefer HQ Runtime' own resolver (profile-safe); fall back to env then ~/.hqr.
    try:
        from hqr_constants import get_hqr_home
        return str(get_hqr_home())
    except Exception:
        return os.environ.get("HQR_HOME") or os.path.expanduser("~/.hqr")


def _tokens_dir():
    return os.path.join(_hqr_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "hqr-diag", "version": "1.0"}},
        },
        headers={
