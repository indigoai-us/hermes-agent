...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/hqr>
...
```bash
hqr memory setup honcho   # configure Honcho directly (works on a fresh install)
hqr memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
hqr config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hqr/.env
```

> `hqr honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `hqr memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `hqr model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HQR_HOME/honcho.json` | Profile-local (isolated HQ Runtime instances) |
| 2 | `~/.hqr/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active HQ Runtime profile: `hqr` (default) or `hqr_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `hqr honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | HQ Runtime session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`hqr-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`hqr`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-hqr-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening hqr in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — HQ Runtime session ID (timestamp + hex). Every `hqr` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple HQ Runtime profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`hqr`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active HQ Runtime profile: `hqr` (default) or `hqr_<profile>` (e.g. `hqr -p coder` -> host key `hqr_coder`). Older `hqr.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HQR_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `hqr-agent`) |
...
|---------|-------------|
| `hqr memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `hqr honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `hqr memory setup`) |
| `hqr honcho status` | Show resolved config for active profile |
| `hqr honcho enable` / `disable` | Toggle Honcho for active profile |
| `hqr honcho mode <mode>` | Change recall or observation mode |
| `hqr honcho peer --user <name>` | Update user peer name |
| `hqr honcho peer --ai <name>` | Update AI peer name |
| `hqr honcho tokens --context <N>` | Set context token budget |
| `hqr honcho tokens --dialectic <N>` | Set dialectic max chars |
| `hqr honcho map <name>` | Map current directory to a session name |
| `hqr honcho sync` | Create host blocks for all HQ Runtime profiles |
