...
cache problem must never block HQ Runtime startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the HQ Runtime home used for cache paths.

    ``home_path`` is whatever ``load_hqr_dotenv()`` already resolved;
    falling back to ``$HQR_HOME`` / ``~/.hqr`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from hqr_constants import get_hqr_home

        home_path = get_hqr_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<hqr_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if HQR_HOME is ever made traversable.
            try:
