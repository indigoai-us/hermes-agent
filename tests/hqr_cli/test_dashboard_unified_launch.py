...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "hqr_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # HQR_HOME.  For a standard install (HQR_HOME unset) that root is
        # the platform-native default (~/.hqr), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from hqr_constants import get_default_hqr_root
        assert env.get("HQR_HOME") == str(get_default_hqr_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (HQR_DESKTOP=1) must NOT
