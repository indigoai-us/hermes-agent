...
Usage (via cron with --no-agent):
    ...
    hqr cron create hqr-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HQR_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hqr-issues --repo NousResearch/hqr-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the HQ Runtime .env file
(``${HQR_HOME:-~/.hqr}/.env``) to avoid the 60 req/hr
