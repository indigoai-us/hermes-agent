...
`hqr skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.hqr.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.hqr.blueprint`` is a mapping containing
    ...
    hqr = meta.get("hqr") if isinstance(meta, dict) else None
    blueprint = hqr.get("blueprint") if isinstance(hqr, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.hqr.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.hqr.blueprint`` block) they can hand
    to ``hqr skills publish`` to share. ``body`` is the plain-language
