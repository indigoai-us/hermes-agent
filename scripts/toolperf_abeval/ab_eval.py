...
measured in the production session DB) through `hqr chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    HQR_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured HQ Runtime home with credentials for the
...
(tracker: NousResearch/hqr-agent#77056).
"""
import json
import os
import re
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "HQR_HOME": str(HOME),
                "HQR_NEMO_RELAY_ATOF_ENABLED": "1",
                "HQR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "HQR_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "HQR_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "hqr_cli.main", "chat", "--query", q,
