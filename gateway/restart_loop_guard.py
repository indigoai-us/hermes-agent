...
Defenses 1 and 2 (the ``_HQR_GATEWAY`` guard on ``hqr gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.hqr.gateway")``,
...
State lives in ``<HQR_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_hqr_home() / "gateway" / "restart_loop.json"
