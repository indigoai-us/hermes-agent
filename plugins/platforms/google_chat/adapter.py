...
# ``model_tools`` import time, so ``hqr status``, ``hqr chat``, etc.
...
# ``hqr_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "HQ Runtime is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a hqr MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "hqr-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${HQR_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _hqr_home = _get_hqr_home()
        except (ModuleNotFoundError, ImportError):
            _hqr_home = _Path.home() / ".hqr"
        self._thread_count_store = _ThreadCountStore(
            _hqr_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "HQ Runtime is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HQR_HOME", str(_Path.home() / ".hqr"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${HQR_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a hqr MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'HQ Runtime is thinking…' marker message.
        ...
        "HQ Runtime is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "HQ Runtime is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "HQ Runtime is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "HQ Runtime is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``hqr setup``.

    The setup wizard at ``hqr_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.hqr/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. hqr-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.hqr/.env")
    print_info("Restart the gateway: hqr gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``hqr cron`` running as a
    separate process from ``hqr gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the HQ Runtime plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `hqr setup` to install Google Chat support.",
        ...
        # the "HQ Runtime is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'HQ Runtime is thinking…' marker message "
