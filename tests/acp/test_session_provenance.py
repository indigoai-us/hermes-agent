...
def test_root_session_no_compression(db):
    ...
    assert prov["currentHqrSessionId"] == "root1"
    assert prov["rootHqrSessionId"] == "root1"
    assert prov["parentHqrSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_hqr_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentHqrSessionId"] == "old"
    assert prov["rootHqrSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousHqrSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootHqrSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"hqr"}
    assert "sessionProvenance" in meta["hqr"]
    assert meta["hqr"]["sessionProvenance"]["currentHqrSessionId"] == "root1"
