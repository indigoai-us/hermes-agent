...
appropriate.  All installs go to a HQ Runtime-owned bin staging dir,
``<HQR_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``hqr lsp
  status``.
...
# ``<HQR_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `hqr lsp status` reports the host's presence.
...
def hqr_lsp_bin_dir() -> Path:
    """Return the HQ Runtime-owned bin staging dir for LSP servers."""
    from hqr_constants import get_hqr_home

    p = get_hqr_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(hqr_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $HQR_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that HQ Runtime installed and
    ...
    if npm is None:
        ...
        return None
    staging = hqr_lsp_bin_dir().parent  # <HQR_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a hqr-owned target dir.
    ...
    console script.
    """
    pip_target = hqr_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = hqr_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``hqr lsp status`` CLI to give users a quick
