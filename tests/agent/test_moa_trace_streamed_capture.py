...
path via ``hqr chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp HQR_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point HQR_HOME at a temp dir and turn moa.save_traces on."""
    hqr_home = tmp_path / ".hqr"
    hqr_home.mkdir()
    monkeypatch.setenv("HQR_HOME", str(hqr_home))

    # save_moa_turn reads config via hqr_cli.config.load_config; stub it to
