...
HQ Runtime Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``HQR_NIX_BUILD=1`` on the
HQ Runtime package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("HQR_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for hqr-agent is not supported.\n"
    "HQ Runtime is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the HQ Runtime Nix derivation sets HQR_NIX_BUILD=1. If it does, file a bug."
)
