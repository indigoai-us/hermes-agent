...
Uses a file-based lock (~/.hqr/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `hqr update` reloads
# the module) fail with ModuleNotFoundError for hqr_time et al.
...
from hqr_time import now as _hqr_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `hqr fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `hqr cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running HQ Runtime, pin it explicitly: "
                f"`hqr cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy hqr_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from hqr_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``hqr tools`` config for the ``cron`` platform.
    ...
    try:
        from hqr_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``HQR_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("HQR_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid HQR_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _hqr_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_hqr_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_hqr_home())
...
# (HQR_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse HQR_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("HQR_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid HQR_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_hqr_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_hqr_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.hqr/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``hqr update`` / ``hqr gateway stop`` / systemd restart, or an
...
_hqr_home: Path | None = None


def _get_hqr_home() -> Path:
    """Resolve HQ Runtime home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active HQR_HOME at call time
    ...
    """
    return _hqr_home or get_hqr_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hqr_home = _get_hqr_home()
    lock_dir = hqr_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"HQ Runtime — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("HQR_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid HQR_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within HQR_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other HQ Runtime-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against HQR_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_hqr_home() / "scripts"
    ...
    # escape — scripts MUST reside within HQR_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("HQR_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `hqr setup`), or pin a "
            "working provider via `hqr cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`hqr setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        now_iso = _hqr_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _hqr_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    #
    # Bounded with its own timeout (separate from HQR_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("HQR_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid HQR_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_hqr_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_hqr_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed HQR_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads HQR_SESSION_PLATFORM
    #     and HQR_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via HQR_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     HQR_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read HQR_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the HQR_CRON_AUTO_DELIVER_* vars set
    # below, so clearing HQR_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # HQR_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ HQR_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "HQR_CRON_AUTO_DELIVER_PLATFORM",
        "HQR_CRON_AUTO_DELIVER_CHAT_ID",
        "HQR_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["HQR_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `hqr chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with HQR_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $HQR_KANBAN_TASK -- letting
        ...
        # load_hqr_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this HQR_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_hqr_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from hqr_cli.env_loader import (
            load_hqr_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["HQR_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["HQR_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["HQR_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > HQR_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``hqr cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_hqr_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"HQR_MODEL={os.getenv('HQR_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`hqr cron edit {job_id} --model <name>` or set a "
                "default with `hqr model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from hqr_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("HQR_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject HQR_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running HQ Runtime "
                        "pin it explicitly: "
                        f"`hqr cron edit {job_id} --provider <provider> "
        ...
        agent = AIAgent(
            ...
            # HQR_HOME. When a workdir is configured, also inject project
        ...
        # override via HQR_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from hqr_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_hqr_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_hqr_now().strftime('%b %d %H:%M')}"
...
def run_one_job(
    ...
    *,
    ...
    profile_home = _get_hqr_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_hqr_home())
        )
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`hqr pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `hqr resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `hqr cron run` that claimed a job
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _hqr_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _hqr_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set HQR_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("HQR_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid HQR_CRON_MAX_PARALLEL value; defaulting to unbounded")
