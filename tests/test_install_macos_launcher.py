"""Regression coverage for the user-facing macOS HQ Runtime launcher."""
...
def test_venv_launcher_bypasses_uv_console_script_that_requires_realpath(tmp_path: Path) -> None:
    """Stock macOS must start HQ Runtime even when its uv console script needs realpath."""
    ...
    (install_dir / "hqr").write_text("# source entrypoint\n", encoding="utf-8")
    _make_executable(
        venv_bin / "hqr",
        "#!/bin/sh\n"
        f'PATH="{minimal_path}"\n'
