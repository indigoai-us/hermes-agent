...
($INSTALL_DIR), not into the shared $HQR_HOME.

Background (shared-$HQR_HOME bug)
------------------------------------
$HQR_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.hqr:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$HQR_HOME the two installs would fight over one slot — and the container,
...
``~/.hqr/hqr-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$HQR_HOME/\.install_method"', text), (
        "install.sh must not stamp $HQR_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `hqr update`"
    )
