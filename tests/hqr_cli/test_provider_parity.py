...
the SAME provider universe as ``hqr model`` (the CLI/TUI picker).
...
def test_every_hqr_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `hqr model` universe."""
    ...
    assert not missing, (
        "providers shown in `hqr model` but not configurable in the desktop "
