...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time HQ Runtime starts and finds ``~/.openclaw/``.

    Points users at ``hqr claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``hqr claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to HQ Runtime, run "
        "`hqr claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `hqr claw cleanup` (renames it to ~/.openclaw.pre-migration — "
