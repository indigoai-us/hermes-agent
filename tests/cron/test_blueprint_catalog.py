...
the shared /blueprint command handler (hqr_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated HQR_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("hqr://blueprint/morning-brief?")
