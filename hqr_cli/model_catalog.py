...
The HQ Runtime docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.hqr/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_hqr_home() / "cache" / "model_catalog.json"
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model HQ Runtime silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`hqr update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``hqr update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.hqr/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``hqr model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
