...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from hqr_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_hqr_home), "display": display_hqr_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _hqr_home / "config.yaml"
        try:
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No HQ Runtime provider is configured.",
                },
            )
