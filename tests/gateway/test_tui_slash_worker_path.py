...
that omits the HQ Runtime venv bin dir and ~/.local/bin. The spawned
tui_gateway.slash_worker then cannot resolve HQ Runtime-managed CLIs such as
...
class TestPrependToolPaths:
    def test_prepends_managed_venv_and_user_bin(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HQR_HOME", str(tmp_path / "hh"))
        env = {"PATH": "/usr/bin"}
