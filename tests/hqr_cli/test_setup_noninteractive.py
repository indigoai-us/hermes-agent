...
import pytest
from hqr_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `hqr` should not prompt for input when no provider exists and stdin is headless."""
        from hqr_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("hqr_cli.main._has_any_provider_configured", return_value=False),
            patch("hqr_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "hqr config set model.provider custom" in out
