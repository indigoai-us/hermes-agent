...
the `cronjob` tool, the `hqr cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `hqr curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `hqr curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.hqr/skills/.usage.json` holds
...
Users drive it via `hqr kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `HQR_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `HQR_KANBAN_BOARD` pinned in env); tenant is a soft namespace
