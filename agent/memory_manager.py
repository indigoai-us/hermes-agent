...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_HQR_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, HQ Runtime expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile HQR_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped HQR_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``hqr_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_hqr_home()`` themselves.
        """
        if "hqr_home" not in kwargs:
            from hqr_constants import get_hqr_home
            kwargs["hqr_home"] = str(get_hqr_home())
        for provider in self._providers:
            try:
