...
def isolated_home(monkeypatch):
    """Temp HQR_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="hqr_test_31179_")
    hqr_home = os.path.join(test_home, ".hqr")
    os.makedirs(hqr_home)
...
def _fresh_modules():
    """Drop cached hqr modules so each test reloads against current env."""
    _drop_reload_targets()
