"""Tests for hqr_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import hqr_logging
# Use whatever RotatingFileHandler class hqr_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. hqr_logging
...
from hqr_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    hqr_logging._logging_initialized = False
    ...
    hqr_logging._reset_queued_handlers()
    ...
    hqr_logging._install_session_record_factory()
    yield
    ...
    hqr_logging._logging_initialized = False
    hqr_logging.clear_session_context()


@pytest.fixture
def hqr_home(tmp_path, monkeypatch):
    """Provide an isolated HQR_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_hqr_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["HQR_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, hqr_home):
        log_dir = hqr_logging.setup_logging(hqr_home=hqr_home)
        assert log_dir == hqr_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in hqr_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home)
        hqr_logging.setup_logging(hqr_home=hqr_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home)

        test_logger = logging.getLogger("test_hqr_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        hqr_logging.flush_log_queue()

        agent_log = hqr_home / "logs" / "agent.log"
    ...
    def test_explicit_params_override_config(self, hqr_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (hqr_home / "config.yaml").write_text(yaml.dump(config))

        hqr_logging.setup_logging(hqr_home=hqr_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, hqr_home):
        ...
        gw_log = hqr_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, hqr_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home, mode="gui")

        logging.getLogger("hqr_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        hqr_logging.flush_log_queue()

        gui_log = hqr_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, hqr_home):
        """When session context is set, log lines include [session_id]."""
        hqr_logging.setup_logging(hqr_home=hqr_home)
        hqr_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = hqr_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home)
        hqr_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_hqr_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        hqr_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in hqr_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        hqr_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("hqr_cli.config.is_managed", return_value=True):
                hqr_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = hqr_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert hqr_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not hqr_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, hqr_home):
        level, max_size, backup = hqr_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, hqr_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> hqr_logging._ManagedRotatingFileHandler:
        handler = hqr_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that hqr_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        hqr_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        hqr_logging.setup_logging(hqr_home=hqr_home, mode="gateway")
        gw_path = hqr_home / "logs" / "gateway.log"
        rotated = hqr_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = hqr_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, hqr_home):
        hqr_logging.setup_logging(hqr_home=hqr_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_hqr_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in hqr_logging.rotating_file_handlers()
        )
