...
`mcp_tool.py`, and `hqr_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# HqrMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_hqr_provider_class() -> Optional[type]:
    ...
    class HqrMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._hqr_server_name = server_name
            self._hqr_home = ""
            ...
            self._hqr_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`HqrTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, HqrTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not HQ Runtime' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import HqrTokenStorage
                        if isinstance(storage, HqrTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._hqr_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import HqrTokenStorage
            if not isinstance(storage, HqrTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``hqr mcp reauth``.
            ...
            back to ``hqr mcp reauth``.
            """
            try:
                if self._hqr_preregistered:
                    return
    ...
    return HqrMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_HQR_PROVIDER_CLS: Optional[type] = _make_hqr_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._hqr_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`HqrMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `hqr mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``hqr mcp remove <name>`` and (indirectly) by
        ``hqr mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, hqr_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, hqr_home=hqr_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, hqr_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, hqr_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, hqr_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(hqr_home) / f"{_safe_filename(server_name)}.json"
            try:
