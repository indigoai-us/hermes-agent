...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    hqr_state = ModuleType("hqr_state")
    hqr_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "hqr_state", hqr_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "HQ Runtime", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "HQ Runtime"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "HQ Runtime", "detail": "full"}),
        ),
    )
