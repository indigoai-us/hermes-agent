...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        with patch.object(HqrCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(HqrCLI, "_is_session_yolo_active", return_value=False):
            text = HqrCLI._build_status_bar_text(host, width=width)
