...
When ``HQR_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.hqr/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.hqr            (has its own auth.json fixture)
    * Profile     -> tmp_path/.hqr/profiles/coder   (active, HQR_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"HQ Runtime is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (HQR_HOME != ~/.hqr, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    hqr_home = tmp_path / "classic"
    hqr_home.mkdir()
