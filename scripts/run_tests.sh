#!/usr/bin/env bash
# Canonical test runner for hqr-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.hqr/...)
#
# Usage:
...
# (HQR_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.hqr/hqr-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.hqr/hqr-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${HQR_PYTHON:-}" ] && [ -x "$HQR_PYTHON" ] \
    && "$HQR_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: HQR_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `hqr` binary rather
  # than the devShell hook.
  PYTHON="$HQR_PYTHON"
  echo "▶ no local venv — using Nix dev venv via HQR_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and HQR_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.hqr/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.hqr"
...
#
#   * HQR_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * HQR_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     HQR_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# HQR_RUN_SLOW_PET_TESTS / HQR_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no HQR_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in HQR_TEST_IMAGE HQR_TEST_WORKERS HQR_TEST_PATHS \
  HQR_TEST_FILE_TIMEOUT HQR_TEST_FILE_RETRIES HQR_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${HQR_RUN_SLOW_PET_TESTS:+HQR_RUN_SLOW_PET_TESTS="$HQR_RUN_SLOW_PET_TESTS"} \
  ${HQR_E2E_BROWSER:+HQR_E2E_BROWSER="$HQR_E2E_BROWSER"} \
