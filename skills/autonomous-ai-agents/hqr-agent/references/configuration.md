...
Edit with `hqr config edit` or `hqr config set section.key value`.
...
`hqr config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `hqr tools` (interactive) or `hqr tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_HQR_CORE_TOOLS` is the default bundle most platforms inherit).
