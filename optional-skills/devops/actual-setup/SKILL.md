---
name: actual-setup
description: Set up Actual Computer (actual.inc) inference in HQ Runtime.
version: 2.0.0
author: shl0ms + HQ Runtime Agent
license: MIT
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a HQ Runtime inference
...
- User has an `ac_` key and wants HQ Runtime routed through their Actual cluster.
...
- HQ Runtime has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current HQ Runtime — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.hqr/.env`.
...
3. Select provider + model:
   ```bash
   hqr config set model.provider actual
   hqr config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   hqr chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point HQ Runtime at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.hqr/.env`, then:
   ```bash
   hqr config set model.provider actual
   hqr config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   hqr chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by HQ Runtime since the first-class provider).**
   ...
   request still 400s this way on an old HQ Runtime, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** HQ Runtime' default toolset
   ...
   servers emit a bare `data: [DONE]` — HQ Runtime reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current HQ Runtime the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
hqr chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
hqr chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
