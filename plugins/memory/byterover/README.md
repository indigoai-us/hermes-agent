...
```bash
hqr memory setup    # select "byterover"
```

Or manually:
```bash
hqr config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.hqr/.env
```
...
Working directory: `$HQR_HOME/byterover/` (profile-scoped).
