#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the HQ Runtime Skills Hub.
...
Used by hqr_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _hqr_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hqr_meta = metadata.get("hqr", {})
            if isinstance(hqr_meta, dict):
                tags = hqr_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/hqr-agent"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``hqr update``. Rather than telling them to update first, resolve
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``hqr_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# HQ Runtime centralized index source
# ---------------------------------------------------------------------------

HQR_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
HQR_INDEX_TTL = 6 * 3600  # 6 hours


def _hqr_index_cache_file() -> Path:
    return _index_cache_dir() / "hqr-index.json"


def _load_hqr_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for HQR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    hqr_index_cache_file = _hqr_index_cache_file()
    if hqr_index_cache_file.exists():
        try:
            age = time.time() - hqr_index_cache_file.stat().st_mtime
            if age < HQR_INDEX_TTL:
                return json.loads(hqr_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("HQ Runtime index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "HQ Runtime index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("HQ Runtime index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        hqr_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        hqr_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class HqrIndexSource(SkillSource):
    """Skill source backed by the centralized HQ Runtime Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "hqr-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "hqr-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        HqrIndexSource(auth=auth), # Centralized index (search + resolved install paths)
