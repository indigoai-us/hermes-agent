...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite HQ Runtime's live source checkout "
        ...
        "checkout, stop HQ Runtime, run the command externally, then restart "
        "HQ Runtime."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(hqr_home).expanduser() if hqr_home else Path.home() / ".hqr"
