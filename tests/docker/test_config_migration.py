...
Build the real image and verify: a config.yaml present in $HQR_HOME
is migrated by docker_config_migrate.py on boot, running as the hqr
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $HQR_HOME must be migrated on boot by
    docker_config_migrate.py, running as the hqr user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $HQR_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/hqr/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by hqr (migration ran as hqr)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "hqr", (
        f"config.yaml not owned by hqr (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
