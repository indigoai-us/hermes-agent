...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HQR_HOME", os.path.expanduser("~/.hqr")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HQR_HOME", Path.home() / ".hqr")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# HQ Runtime config paths
# ═══════════════════════════════════════════════════════════════════

HQR_HOME = Path(os.getenv("HQR_HOME", Path.home() / ".hqr"))
CONFIG_PATH = HQR_HOME / "config.yaml"
PREFILL_PATH = HQR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/HQ Runtime models — already uncensored, just needs clean prompt
    "hqr": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "hqr" in model_lower or "nous" in model_lower:
        return "hqr"
...
def _get_current_model() -> tuple:
    """Read current model and provider from HQ Runtime config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hqr/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart HQ Runtime for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart HQ Runtime for changes to take effect.")
