...
def cron_store(tmp_path, monkeypatch):
    ...
    (hqr_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("HQR_HOME", str(hqr_home))
    monkeypatch.setattr(J, "HQR_DIR", hqr_home)
    monkeypatch.setattr(J, "CRON_DIR", hqr_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", hqr_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", hqr_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_hqr_now", lambda: now):
            due = J.get_due_jobs()
