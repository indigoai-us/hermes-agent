"""Regression test: ``hqr mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``hqr_cli/_parser.py``).  The dispatcher in ``hqr_cli/main.py``
...
result, ``hqr mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the hqr parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`hqr mcp add foo --url ...` must keep args.command == "mcp".
