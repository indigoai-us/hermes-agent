...
On hosts with many profile-backed ``hqr-gateway*.service`` units,
``hqr update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "hqr-gateway-xiaomo5":
        ...
        assert failed == ["hqr-gateway-xiaomo5"]
        assert restarted == [
            "hqr-gateway-xiaomo1",
            "hqr-gateway-xiaomo2",
            "hqr-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "hqr-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "hqr-gateway-coder.service loaded active running",
        ...
        assert seen == ["hqr-gateway-coder"]

    def test_hqr_serve_units_are_included(self):
        # #83438 — hqr update restarted hqr-gateway* units but left
        # hqr-serve* (the Desktop app's backend) on stale pre-update code.
        seen: list[str] = []
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    "ssh.service loaded active running",
                    "hqr-serve.service loaded active running",
                    "hqr-serve-work.service loaded active running",
                    "hqr-gateway.service loaded active running",
                    "",
                ]
            ),
            process_unit=seen.append,
        ...
        assert seen == ["hqr-serve", "hqr-serve-work", "hqr-gateway"]

    def test_hqr_server_near_prefix_is_rejected(self):
        # Review on #83595: a bare ``startswith("hqr-serve")`` gate also
        # accepts the unrelated ``hqr-server.service``. Only the exact
        # base unit or the hyphenated profile family should pass.
        seen: list[str] = []

        _for_each_systemd_gateway_unit(
            _list_units_stdout(["hqr-server"]),
            process_unit=seen.append,
            on_unit_timeout=lambda *_: pytest.fail("unexpected timeout"),
        )
    ...
    def test_hqr_gateway_near_prefix_is_rejected(self):
        # Same strict shape on the gateway side: profile units are
        # ``hqr-gateway-<profile>``, so a hypothetical
        # ``hqr-gatewayd.service`` must not enter the restart path.
        seen: list[str] = []

        _for_each_systemd_gateway_unit(
            _list_units_stdout(["hqr-gatewayd", "hqr-gateway-coder"]),
            process_unit=seen.append,
            on_unit_timeout=lambda *_: pytest.fail("unexpected timeout"),
        )
...
class TestGracefulSigusr1Eligibility:
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["hqr-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["hqr-gateway-xiaomo5", "hqr-gateway-xiaomo6", "hqr-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("hqr-gateway-xiaomo5") == 1
        assert "hqr-gateway-xiaomo6" in out
