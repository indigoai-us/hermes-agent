"""Tests for ``hqr doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from hqr_cli import doctor_live
from hqr_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `hqr doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("hqr_cli.doctor.HQR_HOME", tmp_path / "home")
        monkeypatch.setattr("hqr_cli.doctor.PROJECT_ROOT", tmp_path / "root")
