...
Uses a throwaway HQR_HOME so it never touches ~/.hqr. No real names.
"""
...
def _fresh_home():
    """Point HQR_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hqr_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["HQR_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER HQR_HOME is set.
...
# a different get_hqr_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"HQR_HOME (throwaway): {HOME}")
