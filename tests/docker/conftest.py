...
Override the image with ``HQR_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("HQR_TEST_IMAGE", "hqr-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``HQR_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("HQR_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged hqr user
# ---------------------------------------------------------------------------
#
# Background: every HQ Runtime runtime path inside the container drops to UID
# 10000 (the ``hqr`` user) via ``s6-setuidgid hqr``. ``docker exec``
...
# fails (PermissionError swallowed) for hqr, so a test that ran as root
...
# helpers below run every probe under ``-u hqr`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: hqr).
