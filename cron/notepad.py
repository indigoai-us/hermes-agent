...
Write path is the CLI (``hqr cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_hqr_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`hqr cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `hqr cron notepad {job_id} delete "
