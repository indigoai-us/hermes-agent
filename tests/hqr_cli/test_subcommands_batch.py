...
def test_config_get_unset_subcommands_parse():
    """`hqr config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="hqr")
...
# ── deprecated `hqr login` fails gracefully, not with argparse error ────
#
# `hqr login` is a removed command; its handler (`login_command` in
# `hqr_cli/auth.py`) prints a deprecation notice pointing at `hqr auth` /
# `hqr model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `hqr --help`.
