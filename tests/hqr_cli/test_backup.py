"""Tests for hqr backup and import commands."""
...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import hqr_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew hqr_cli.backup's datetime forward instead of sleeping.
    ...
    import hqr_cli.backup as _backup

    shim = getattr(_backup.datetime, "_hqr_test_shim", None)
    if shim is None:
...
def _make_hqr_tree(root: Path) -> None:
    """Create a realistic ~/.hqr directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "hqr_state.db"):
    ...
    # hqr-agent repo (should be EXCLUDED)
    (root / "hqr-agent").mkdir(exist_ok=True)
    (root / "hqr-agent" / "run_agent.py").write_text("# big file\n")
    (root / "hqr-agent" / ".git").mkdir()
    (root / "hqr-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hqr_agent(self):
        from hqr_cli.backup import _should_exclude
        assert _should_exclude(Path("hqr-agent/run_agent.py"))
        assert _should_exclude(Path("hqr-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import hqr_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = hqr_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, hqr_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside HQR_HOME."""
        hqr_home = tmp_path / ".hqr"
        hqr_home.mkdir()
        _make_hqr_tree(hqr_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(hqr_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid HQ Runtime backup."""
        from hqr_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your HQ Runtime configuration has been restored." in out
        assert "hqr gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        hqr_home = tmp_path / ".hqr"
        (hqr_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("HQR_HOME", str(hqr_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (hqr_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (hqr_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            hqr_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (hqr_home / "gateway.pid").write_text("4242")
        (hqr_home / "processes.json").write_text('{"live": true}')
        ...
        assert (hqr_home / "gateway.pid").read_text() == "4242"
        assert (hqr_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (hqr_home / "cron.pid").exists()
        assert not (hqr_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (hqr_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".hqr"
        src_home.mkdir(parents=True)
        _make_hqr_tree(src_home)

        monkeypatch.setenv("HQR_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from hqr_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".hqr"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("HQR_HOME", str(dst_home))
        ...
        # hqr-agent should NOT be present
        assert not (dst_home / "hqr-agent").exists()
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from hqr_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".hqr/", "")
            zf.writestr(".hqr/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_hqr_home(self, tmp_path, monkeypatch):
        """Backup handles empty hqr home (no files to back up)."""
        hqr_home = tmp_path / ".hqr"
        hqr_home.mkdir()
        # Only excluded dirs, no actual files
        (hqr_home / "__pycache__").mkdir()
        (hqr_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        hqr_home = tmp_path / ".hqr"
        hqr_home.mkdir()
        (hqr_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = hqr_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        hqr_home = tmp_path / ".hqr"
        hqr_home.mkdir()
        (hqr_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("HQR_HOME", str(hqr_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (hqr_home / "config.yaml").exists()
        assert (hqr_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`hqr import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the HQR_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (hqr_home / "config.yaml").write_text(original)
        monkeypatch.setenv("HQR_HOME", str(hqr_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (hqr_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(hqr_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside HQR_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        hqr_home = dst_home / ".hqr"
        hqr_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo hqr import`` onto a user-owned (or Docker/NAS volume-owned)
        HQR_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "hqr_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``hqr import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from hqr_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from hqr_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from hqr_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def hqr_home(self, tmp_path):
        """Create a fake HQR_HOME with critical state files."""
        home = tmp_path / ".hqr"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, hqr_home):
        from hqr_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(hqr_home=hqr_home)
        db_copy = hqr_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, hqr_home, monkeypatch, capsys):
    ...
    def test_snapshot_includes_pairing_directories(self, hqr_home):
        ...
        (hqr_home / "platforms" / "pairing").mkdir(parents=True)
        (hqr_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (hqr_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (hqr_home / "pairing").mkdir()
        (hqr_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (hqr_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = hqr_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (hqr update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, hqr_home, capsys):
        ...
        """
        import json
        from hqr_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", hqr_home=hqr_home)
        assert first_id is not None
        first_dir = hqr_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", hqr_home=hqr_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = hqr_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, hqr_home=hqr_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, hqr_home):
        ...
        from hqr_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = hqr_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(hqr_home=hqr_home)
        copy = (
            hqr_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, hqr_home=hqr_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, hqr_home, monkeypatch):
        ...
        import hqr_cli.backup as bk
        from hqr_cli.backup import create_quick_snapshot

        board = hqr_home / "kanban" / "boards" / "work"
        ...
        copy = hqr_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``hqr update``
    ...
    def test_backup_contents_match_full_backup(self, hqr_home):
        """Pre-update backup should include the same user data that
        ``hqr backup`` would, and should exclude the same directories."""
        from hqr_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hqr_home=hqr_home)
        ...
        # hqr-agent repo excluded
        assert not any(n.startswith("hqr-agent/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, hqr_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(hqr_home=hqr_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (hqr_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, hqr_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside HQR_HOME."""
        from hqr_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def hqr_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".hqr"
        root.mkdir()
        _make_hqr_tree(root)
        # Point HQR_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("HQR_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve HQR_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(hqr_home, value):
        import yaml
        (hqr_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
    ...
    @staticmethod
    def _snaps(hqr_home):
        d = hqr_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, hqr_home, capsys):
        ...
        self._set_mode(hqr_home, "off")
        from hqr_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(hqr_home)
        assert not self._zips(hqr_home)



    def test_config_full_mode(self, hqr_home, capsys):
        self._set_mode(hqr_home, "full")
        from hqr_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(hqr_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (hqr claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``hqr claw migrate`` runs before mutating ~/.hqr/."""
    ...
    def test_restorable_with_hqr_import(self, hqr_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid HQ Runtime
        backup — `hqr import` should accept it."""
        from hqr_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(hqr_home=hqr_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, hqr_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from hqr_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(hqr_home=hqr_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(hqr_home=hqr_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`hqr update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, hqr_home: Path, label="pre-update"):
        from hqr_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, hqr_home=hqr_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from hqr_cli.backup import restore_cron_jobs_if_emptied
        hqr_home = tmp_path / ".hqr"
        jobs_path = hqr_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(hqr_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, hqr_home=hqr_home)
        assert result is not None
...
# location, NOT under HQR_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, hqr_home: Path) -> None:
        hqr_home.mkdir(parents=True, exist_ok=True)
        (hqr_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (hqr_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (hqr_home / "state.db").write_bytes(b"x")
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under HQR_HOME,
        ...
        # External state did NOT leak into HQR_HOME.
        assert not (hqr_home / "_external").exists()
