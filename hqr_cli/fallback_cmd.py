"""
hqr fallback — manage the fallback provider chain.
...
Subcommands:
  hqr fallback [list]   Show the current fallback chain (default when no subcommand)
  hqr fallback add      Pick provider + model via the same picker as `hqr model`,
                           then append the selection to the chain
  hqr fallback remove   Pick an entry to delete from the chain
  hqr fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.hqr/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from hqr_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `hqr model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  hqr fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `hqr model`, then append the selection to the chain."""
    from hqr_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `hqr model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `hqr fallback list` to view, or `hqr fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``hqr fallback [subcommand]``."""
