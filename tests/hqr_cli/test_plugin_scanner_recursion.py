...
def _enable(hqr_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<hqr_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        hqr_home = Path(os.environ["HQR_HOME"])  # set by hermetic conftest fixture
        user_plugins = hqr_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(hqr_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        hqr_home = Path(os.environ["HQR_HOME"])  # set by hermetic conftest fixture
        _write_plugin(hqr_home / "plugins", ["p1"])
        _enable(hqr_home, "p1")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(hqr_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(hqr_home, "bad-img-plugin")
