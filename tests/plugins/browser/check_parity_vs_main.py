...
Run from the PR worktree:
    ...
    cd ~/.hqr/hqr-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.hqr/hqr-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.hqr/hqr-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a hqr-agent checkout"
)
...
# Isolated HQR_HOME for the config write.
