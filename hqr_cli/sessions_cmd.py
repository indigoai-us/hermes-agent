"""``hqr sessions`` command — extracted from ``hqr_cli/main.py``.
...
Helpers that stay in ``hqr_cli.main`` (``get_hqr_home``,
...
``hqr_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `hqr sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`hqr sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from hqr_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    hqr sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from hqr_cli.session_recovery import (
    ...
    if action == "import":
        from hqr_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from hqr_state import workspace_key as _ws_key
        ...
        from hqr_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from hqr_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_hqr_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from hqr_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_hqr_home() / "session-exports"
        ...
        # `hqr sessions prune`: no time window and no filters at all
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `hqr sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: hqr sessions pin <session_id>"
            )
            return
        ...
        # Launch hqr --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`hqr sessions optimize` later to reclaim freed space.)")
