...
def _isolate(tmp_path, monkeypatch):
    """Isolate HQR_HOME so tests don't touch real config."""
    home = tmp_path / ".hqr"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal HqrCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = HqrCLI.__new__(HqrCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["hqr-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below HQ Runtime' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly HQ Runtime' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above HQ Runtime' minimum context length."""
