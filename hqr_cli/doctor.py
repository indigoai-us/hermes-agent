"""
Doctor command for hqr CLI.

Diagnoses issues with HQ Runtime Agent setup.
"""
...
from hqr_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
HQR_HOME = get_hqr_home()
_DHH = display_hqr_home()  # user-facing display path (e.g. ~/.hqr or ~/.hqr/profiles/coder)

# Load environment variables from ~/.hqr/.env so API key checks work
_env_path = get_env_path()
load_hqr_dotenv(hqr_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from hqr_cli.colors import Colors, color
from hqr_cli.models import _HQR_USER_AGENT
from hqr_cli.vercel_auth import describe_vercel_auth
from hqr_constants import OPENROUTER_MODELS_URL
...
from hqr_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all HQ Runtime containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `hqr update`"
    return (
...
def _hqr_database_paths(hqr_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for HQ Runtime-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from hqr_cli.backup import _QUICK_STATE_FILES
    ...
    for board_db in sorted((hqr_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(hqr_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``hqr_cli.sqlite_safe_read``).
    ...
    """
    from hqr_cli.sqlite_safe_read import (
        has_live_connection,
...
def _format_db_size(db_path: Path) -> str:
    ...
    from hqr_cli.backup import _format_size
...
def _report_database_journal_modes(
    hqr_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from hqr_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = hqr_home if hqr_home is not None else HQR_HOME
    try:
        databases = _hqr_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list HQ Runtime databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.hqr/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("HQR_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from hqr_cli.auth import get_minimax_oauth_auth_status
...
from hqr_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'hqr sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. HQR_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("HQR_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("HQR_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches hqr_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``hqr --version`` reporting a stale version while
    ...
    """
    try:
        from hqr_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != hqr_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hqr update', or set "
            "hqr_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-hqr and dashboard static services are up
    ...
    for static in ("main-hqr", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `hqr profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `hqr doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from hqr_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the HQR_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("HQR_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via HQR_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `hqr`.
    os.environ.setdefault("HQR_INTERACTIVE", "1")

    # Handle `hqr doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.hqr/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 HQ Runtime Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`hqr doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from hqr_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: HQ Runtime already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and hqr_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.hqr/.env (primary location for user config)
    env_path = HQR_HOME / '.env'
    if env_path.exists():
        ...
        # matches hqr_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'hqr setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'hqr setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'hqr setup' to create one")
                issues.append("Run 'hqr setup' to create .env")
    
    # Check ~/.hqr/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = HQR_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from hqr_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from hqr_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'hqr config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from hqr_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hqr/.env or run 'hqr setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'hqr setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'hqr config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from hqr_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from hqr_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'hqr setup' to migrate config")
                else:
                    issues.append("Run 'hqr doctor --fix' or 'hqr setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from hqr_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'hqr doctor --fix'")
        except Exception:
            pass

        # Detect stale HQR_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives HQR_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from hqr_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("HQR_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"HQR_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `hqr setup` run)",
                )
                if should_fix:
                    if remove_env_value("HQR_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale HQR_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove HQR_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the HQR_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale HQR_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'hqr doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from hqr_cli.config import load_env as _load_env_depr
            from hqr_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses HQ Runtime' own device-code flow — the Codex CLI is
    ...
    if hqr_home.exists():
        ...
        hqr_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = hqr_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give HQ Runtime a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# HQ Runtime Agent Persona\n\n"
                "<!-- Edit this file to customize how HQ Runtime communicates. -->\n\n"
                "You are HQ Runtime, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = hqr_home / "memories"
    ...
    state_db_path = hqr_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from hqr_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'hqr doctor --fix' "
                        "(or 'hqr sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from hqr_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'hqr doctor --fix' "
                        "(or 'hqr sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every HQ Runtime
        ...
        try:
            from hqr_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'hqr sessions optimize-storage' "
    ...
    wal_path = hqr_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'hqr doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "hqr"
        ...
        _cmd_link = _cmd_link_dir / "hqr"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(hqr not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/hqr → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/hqr points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/hqr → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/hqr — run 'hqr doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/hqr exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/hqr not found",
                    "(hqr command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/hqr → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hqr symlink — run 'hqr doctor --fix'")
    ...
    try:
        from hqr_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'hqr-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'hqr-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/HQ Runtime-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `hqr update` does, so a session's first browser call
            ...
            # `hqr update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `hqr doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable HQR_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a HQ Runtime one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'hqr config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``hqr doctor``.
    ...
    # never the legitimate source for `hqr doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'hqr setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = HQR_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: hqr skills list)")
    ...
    try:
        from hqr_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = HQR_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: hqr memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: hqr memory setup",
                    "No Honcho API key — run 'hqr memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run hqr memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: hqr memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: hqr memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from hqr_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "hqr -p" in content:
                            _m = _re.search(r"hqr -p (\S+)", content)
    ...
    # `hqr doctor --live` (real network calls; bounded + read-only).
    try:
        from hqr_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'hqr doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
