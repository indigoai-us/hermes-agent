...
HQ Runtime pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.hqr/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hqr_home>/bin/bws`` on
  first use.  HQ Runtime pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.hqr/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``hqr`` invocations don't
  hammer the API.
* Failures NEVER block HQ Runtime startup.  Missing binary, no network,
...
# In-process cache so repeated load_hqr_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `hqr chat -q ...`
...
# <hqr_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.hqr/.env (which
...
_ENCRYPTED_CACHE_INFO = b"hqr-bws-encrypted-cache-v1"
...
def _hqr_bin_dir() -> Path:
    """Where HQ Runtime stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<hqr_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``hqr secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="hqr-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a HQ Runtime
...
# ---------------------------------------------------------------------------
# Public entry point — called from hqr_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_hqr_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `hqr secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `hqr secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `hqr secrets bitwarden setup` to install."
        )
        return result
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`hqr secrets bitwarden token`) so the
