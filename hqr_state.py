#!/usr/bin/env python3
"""
SQLite State Store for HQ Runtime Agent.
...
from hqr_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from hqr_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from hqr_state_portability import SessionPortabilityMixin
from hqr_state_schema import SessionSchemaMixin
from hqr_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``hqr -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_hqr_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.hqr`` even when a test fixture later
    redirects ``HQR_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_hqr_home()`` fresh so a runtime
       ``HQR_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_hqr_home() / "state.db"
...
# ~/.hqr/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects HQR_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without HQR_HOME, a stale worktree
# without the re-pin, or a developer shell that exports HQR_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "HQR_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.hqr``).  The test conftest injects the pre-sandbox production
#: root here so custom-``HQR_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default HQ Runtime root for the guard.

    Deliberately avoids ``Path.home()`` / ``hqr_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``hqr_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "hqr"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "hqr"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".hqr"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``HQR_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under HQ Runtime test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``HQR_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "HQR_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``HQR_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``hqr`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real HQ Runtime home *root*.
    ...
    under ``~/.hqr/hqr-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``HQR_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``HQR_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real HQ Runtime root {root}). "
                "Tests must run against a temporary HQR_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "HQR_HOME. If this test genuinely needs the live "
...
# hqr_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``hqr sessions optimize`` (FTS merge + VACUUM)
    ...
    ``hqr_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``hqr_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``hqr update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"HQ Runtime-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart HQ Runtime"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a HQ Runtime-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `hqr doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see hqr_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with hqr_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``hqr doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal HQ Runtime host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``hqr serve`` backend, interactive CLI sessions,
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``hqr_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the HQ Runtime home tree (``get_hqr_home()``) — the safe repair scope:
      HQ Runtime owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``hqr_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_hqr_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. HQ Runtime needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (hqr_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_hqr_state.py:600-632)
        ...
        probe_session_id = f"_hqr_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.hqr/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``hqr sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("HQR_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_hqr_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("HQR_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship hqr_state without hqr_cli). A
    ...
    try:
        ...
        logger.debug(
            "hqr_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch hqr_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``hqr_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `hqr snapshot list` / "
                "`hqr snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (hqr doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple hqr processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling HQ Runtime
    ...
    # `hqr update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `hqr snapshot list` / "
                    f"`hqr snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`hqr db optimize`),
            ...
            # ``hqr_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `hqr update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `hqr sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another HQ Runtime process held the "
    ...
    #
    # `optimize_fts_storage()` (the `hqr sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`hqr sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``hqr sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``hqr --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: hqr_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``hqr -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``hqr sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``HQR_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade HQ Runtime, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one HQ Runtime session.

        A HQ Runtime session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a HQ Runtime session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `hqr sessions vacuum` runs in a transient
