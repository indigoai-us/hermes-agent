"""Runtime smoke test for Docker $HQR_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the hqr user after container boot.
...
registered later runs its log service as the dropped hqr user and
...
def test_logs_gateways_seeded_and_hqr_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by hqr after boot."""
    ...
    assert "logs=hqr" in r.stdout, (
        f"logs/ not owned by hqr: {r.stdout}"
    )
    assert "gateways=hqr" in r.stdout, (
        f"logs/gateways/ not owned by hqr: {r.stdout}"
    )
