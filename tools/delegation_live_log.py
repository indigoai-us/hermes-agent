...
per child under::
    ...
    <hqr_home>/cache/delegation/live/<delegation_id>/task-<n>.log
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.hqr)."""
    from hqr_constants import get_hqr_dir

    return get_hqr_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        try:
            ...
            header = [
                "=== HQ Runtime subagent live transcript ===",
