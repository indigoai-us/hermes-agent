...
This is an optional third-party workflow, not a HQ Runtime core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with HQ Runtime native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer HQ Runtime native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer HQ Runtime native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended HQ Runtime workflow:
...
## Recommended HQ Runtime usage patterns
...
- Do not silently choose Parallel when HQ Runtime native tools are already sufficient.
