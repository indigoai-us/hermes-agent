...
unprivileged `hqr` user, giving the layout most people have —
$HQR_HOME/hqr-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  HQR_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .hqr-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `hqr setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${HQR_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/hermes-agent.git}"
...
GIT_ROOT="${HQR_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${HQR_DEV_SANDBOX_DIR:-.hqr-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t hqr-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t hqr-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    ...
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      ...
      if git --git-dir=/work/repos/hqr-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `hqr` launcher to ~/.local/bin and the
# checkout to $HQR_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/hqr-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q hqr-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t hqr-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'HQ Runtime sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/hqr-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=HQ Runtime dev sandbox CA' \
