"""
HQ Runtime Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.hqr/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove HQ Runtime PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing hqr-agent or hqr PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# HQ Runtime Agent" comment and following line
                if '# HQ Runtime Agent' in line or '# hqr-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('hqr' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hqr
                if 'hqr' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a HQ Runtime-owned file, and
...
def remove_wrapper_script():
    """Remove the hqr wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "hqr",
        Path.home() / ".local" / "bin" / "hqr-acp",
        Path.home() / ".local" / "bin" / "hqr-agent",
        Path("/usr/local/bin/hqr"),
        Path("/usr/local/bin/hqr-acp"),
        Path("/usr/local/bin/hqr-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains hqr_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'hqr_cli' in content or 'hqr-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hqr_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``hqr`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this HQ Runtime home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (hqr_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``hqr gateway run`` processes
    ...
    try:
        from hqr_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from hqr_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``HQR_HOME`` and ``HQR_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\hqr\git\cmd``, ``%LOCALAPPDATA%\hqr\git\bin``,
#      ``%LOCALAPPDATA%\hqr\git\usr\bin``, ``%LOCALAPPDATA%\hqr\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\hqr\git\`` and Node to
#      ``%LOCALAPPDATA%\hqr\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``hqr dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\hqr\gateway-service\`` and sometimes
...
def _hqr_path_markers(hqr_home: Path) -> list[str]:
    """Path-entry substrings that identify HQ Runtime-owned User-PATH entries."""
    root = str(hqr_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare hqr-agent install dir.
    markers = [root + "\\hqr-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HQR_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "hqr".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-hqr as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hqr_home: Path) -> list[str]:
    """Strip HQ Runtime-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hqr_path_markers(hqr_home)
            kept: list[str] = []
...
def remove_hqr_env_vars_windows() -> list[str]:
    """Delete HQR_HOME and HQR_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("HQR_HOME", "HQR_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(hqr_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\hqr\\``.  Only called on full uninstall; they're
...
def _is_default_hqr_home(hqr_home: Path) -> bool:
    """Return True when ``hqr_home`` points at the default (non-profile) root."""
    try:
        from hqr_constants import get_default_hqr_root
        return hqr_home.resolve() == get_default_hqr_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its HQR_HOME directory.

    We shell out to ``hqr -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current HQR_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m hqr_cli.main` so we don't depend on a `hqr`
    #    wrapper that may be half-removed mid-uninstall.
    hqr_invocation = [_sys.executable, "-m", "hqr_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                hqr_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's HQR_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``hqr uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$HQR_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from hqr_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(hqr_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ HQ Runtime Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No HQ Runtime Chat GUI installation was found.")
        print(f"  Checked: {hqr_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The HQ Runtime agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(hqr_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The HQ Runtime agent at {hqr_home / 'hqr-agent'}")
        print(f"  • Your config, sessions, and secrets under {hqr_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The HQ Runtime agent is still installed. Run 'hqr' to use the CLI,")
    print("or 'hqr uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.hqr/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.hqr/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie HQR_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_hqr_home(hqr_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.hqr data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ HQ Runtime Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {hqr_home / 'config.yaml'}")
    print(f"  Secrets: {hqr_home / '.env'}")
    print(f"  Data:    {hqr_home / 'cron/'}, {hqr_home / 'sessions/'}, {hqr_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their HQR_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL HQ Runtime data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the HQ Runtime code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hqr_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • HQ Runtime PATH entries from shell configs / Windows User PATH")
    print("  • HQ Runtime wrapper scripts and HQ Runtime-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • HQ Runtime config/data: {hqr_home}")
        if _is_default_hqr_home(hqr_home):
        ...
        print(f"  • Keep HQ Runtime config/data: {hqr_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``hqr`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$HQR_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in hqr_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\hqr\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(hqr_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No HQ Runtime-owned PATH entries in User environment")

        log_info("Removing HQR_HOME / HQR_GIT_BASH_PATH User env vars...")
        removed_env = remove_hqr_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No HQ Runtime-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing hqr command...")
    ...
    #     (only when they still point into this HQ Runtime home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing HQ Runtime-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(hqr_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No HQ Runtime-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(hqr_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE HQR_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from hqr_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(hqr_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.hqr/, just remove the hqr-agent subdir
            if hqr_home in project_root.parents or project_root.parent == hqr_home:
    ...
    #     under HQR_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(hqr_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(hqr_home)
    ...
    # 5. Optionally remove ~/.hqr/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile HQR_HOME dirs live under
        ...
        try:
            if hqr_home.exists():
                shutil.rmtree(hqr_home)
                log_success(f"Removed {hqr_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {hqr_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {hqr_home}")
    ...
    print()
    print("Thank you for using HQ Runtime Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m hqr_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import hqr_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``hqr_constants`` + ``hqr_cli.colors``
    (and lazily ``hqr_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m hqr_cli.uninstall")
    parser.add_argument(
        "--mode",
