"""CLI entry point for the hqr-agent ACP adapter.

Loads environment variables from ``~/.hqr/.env``, configures logging
...
try:
    ...
    # shadowing HQ Runtime's own modules — ``hqr acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from HQR_HOME (default ``~/.hqr``)."""
    from hqr_cli.env_loader import load_hqr_dotenv

    hqr_home = get_hqr_home()
    loaded = load_hqr_dotenv(hqr_home=hqr_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", hqr_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="hqr-acp",
        description="Run HQ Runtime Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print HQ Runtime version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive HQ Runtime provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.hqr/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import HqrACPAgent  # noqa: F401

    print("HQ Runtime ACP check OK")


def _run_setup() -> None:
    from hqr_cli.main import main as hqr_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "hqr", "model"]
        hqr_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting hqr-agent ACP adapter")
    ...
    import acp
    from .server import HqrACPAgent
    ...
    if os.environ.get("HQR_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
