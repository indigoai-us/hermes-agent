...
## Layer 1 — Is this thing a HQ Runtime plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the HQ Runtime process**. Its LLM calls go through HQ Runtime's already-configured provider. | Already uses Portal if user's HQ Runtime is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside HQ Runtime. That LLM call is already authenticated
via HQ Runtime's provider config. The plugin's own server auth (e.g.
...
What HQ Runtime Agent has that *feels* like OAuth — `hqr login --provider nous`
opening a browser, user signs in, token lands in `~/.hqr/auth.json` — is a
**HQ Runtime-specific browser flow**. Under the hood it produces a credential
HQ Runtime uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read HQ Runtime's existing Portal credential out of `~/.hqr/auth.json`.
...
rides on the credential HQ Runtime already holds.

Where this could live in HQ Runtime:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for HQ Runtime itself.
...
1. First decide: HQ Runtime plugin (runs inside HQ Runtime) or separate app? If plugin,
   it already uses Portal via HQ Runtime's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   HQ Runtime agent itself") is fully supported. HQ Runtime treats agent
...
pitch in the Tool Gateway announcement is specifically about HQ Runtime Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when HQ Runtime is configured to use Portal as its provider. It is
