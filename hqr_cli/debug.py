"""``hqr debug`` debug tools for HQ Runtime Agent.

Currently supports:
    hqr debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.hqr/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[hqr debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.hqr/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``hqr debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``hqr debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `hqr debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "hqr-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``hqr debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.hqr/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``hqr debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "hqr-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from hqr_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_hqr_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `hqr debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by HQ Runtime Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``hqr dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``hqr dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "hqr-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``hqr debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "hqr debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `hqr debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  hqr debug delete <url>")

    print("\nShare these links with the HQ Runtime team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/HQ Runtime version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``hqr debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from hqr_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "hqr debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `hqr debug share --local` to print the report instead, "
            "or `hqr debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: hqr debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'hqr debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``hqr debug`` call.
    ...
    # best-effort — any failure is swallowed so ``hqr debug`` stays
