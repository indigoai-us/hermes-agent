...
#
# Installs a real, earlier HQ Runtime the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `hqr`.
#
...
# Keep sandbox state out of the default .hqr-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".hqr-sandbox-e2e-$ROUTE"
export HQR_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/hqr/.hqr/hqr-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/hqr-agent.git"
...
LOG_DIR="${HQR_E2E_LOG_DIR:-$(mktemp -d -t hqr-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED hqr accept FLAG on `hqr update`?
#
...
# that silently rots. `hqr update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "hqr update --help 2>&1" | grep -qF -- "$flag"
}
...
require_hqr_works() {
  local when="$1" out
  out="$(in_sandbox "hqr --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "hqr --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "hqr runs $when"
}

# ── install the earlier HQ Runtime ─────────────────────────────────────────────
...
require_hqr_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: hqr update'
    ...
    # hqr which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="hqr update --yes"
    else
      update_cmd="hqr update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "hqr update failed ($update_cmd)"
    fi
    require_landed_on_target 'hqr update'
    require_hqr_works 'after hqr update'
    ;;
  installer)
    ...
    require_hqr_works 'after installer re-run'
    ;;
esac
