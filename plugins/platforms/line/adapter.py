"""
LINE Messaging API platform adapter for HQ Runtime Agent.
...
to HQ Runtime Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``HQR_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hqr_home = Path(get_hqr_home()).resolve()
        except Exception:
            hqr_home = Path.home().joinpath(".hqr").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``hqr status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``hqr status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hqr setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.hqr/.env`` via ``hqr_cli.config``.
    """
    print()
    ...
    try:
        from hqr_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("hqr_cli.config not available; set LINE_* vars manually in ~/.hqr/.env")
        return
