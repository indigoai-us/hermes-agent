...
hqr_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `hqr config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_hqr_home = get_hqr_home()`` at import time (module
    singleton), so monkeypatching HQR_HOME after import doesn't move it.
    ...
    this only matters for tests that swap HQR_HOME.)
    """
    import cli
