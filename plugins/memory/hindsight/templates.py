...
the ``hqr`` integration, and applies a chosen manifest to the user's bank
...
def fetch_hqr_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``hqr`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "hqr" in (e.get("integrations") or [])]
