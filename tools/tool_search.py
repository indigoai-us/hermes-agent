"""Progressive tool disclosure ("tool search") for HQ Runtime Agent.
...
``tool_call`` — and surfaced on demand. Core HQ Runtime tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._HQR_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_HQR_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_HQR_CORE_TOOLS``. Core tools are never deferred
