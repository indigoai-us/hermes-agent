...
HQR_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env HQR_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``hqr config get terminal.backend`` still says local.
    """
    hqr_home = tmp_path / ".hqr"
    hqr_home.mkdir()
    (hqr_home / "config.yaml").write_text(
    ...
    (hqr_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
