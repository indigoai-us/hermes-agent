...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hqr -c.",
    ...
    "Ctrl+Z suspends HQ Runtime to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hqr -c resumes your most recent CLI session. hqr -c \"project name\" resumes by title.",
    "hqr -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hqr -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hqr chat -t web,terminal enables only specific toolsets for a focused session.",
    "hqr chat -s github-pr-workflow preloads a skill at launch.",
    "hqr chat -q \"query\" runs a single non-interactive query and exits.",
    "hqr chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "hqr chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hqr --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hqr chat --source telegram tags the session for filtering in hqr sessions list.",
    "hqr -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "hqr doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hqr dump outputs a compact setup summary — great for bug reports.",
    "hqr config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hqr config edit opens config.yaml in your default editor.",
    "hqr config check scans for missing or stale configuration options.",
    "hqr sessions browse opens an interactive session picker with search.",
    "hqr sessions stats shows session counts by platform and database size.",
    "hqr sessions prune --older-than 30 cleans up old sessions.",
    "hqr skills search react --source skills-sh searches the skills.sh public directory.",
    "hqr skills check scans installed hub skills for upstream updates.",
    "hqr skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hqr skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hqr mcp add github --command npx adds MCP servers from the command line.",
    "hqr mcp serve runs HQ Runtime itself as an MCP server for other agents.",
    "hqr auth add lets you add multiple API keys for credential pool rotation.",
    "hqr completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hqr logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hqr backup creates a zip backup of your entire HQ Runtime home directory.",
    "hqr profile create coder creates an isolated profile that becomes its own command.",
    "hqr profile create work --clone copies your current config and keys to a new profile.",
    "hqr update syncs new bundled skills to ALL profiles automatically.",
    "hqr gateway install sets up HQ Runtime as a system service (systemd/launchd).",
    "hqr memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hqr webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hqr tools disables unused tools, hqr skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hqr models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hqr -w).",
    ...
    "HQR_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call HQ Runtime tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'hqr profile create coder' creates the 'coder' command.",
    "hqr profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, HQ Runtime prints a resume command with session ID and stats.",
    "hqr sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hqr -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "hqr skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .hqr/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hqr cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "HQ Runtime runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "hqr gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to HQ Runtime via a local macOS server.",
    ...
    "Context files (.hqr.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "HQ Runtime loads project context from .hqr.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "hqr mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "hqr mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "hqr mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — hqr tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.hqr/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hqr chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hqr plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hqr acp runs HQ Runtime as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "HQR_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "hqr auth add nous or hqr auth add openai-codex sets up OAuth-based providers.",
    ...
    "hqr status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.hqr/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make HQ Runtime your own.",
    ...
    "Before a gateway session reset, HQ Runtime auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.hqr/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope HQ Runtime state via HQR_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "HQR_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HQR_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.hqr/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hqr profile create ls' would be rejected.",
    "hqr profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hqr claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "HQR_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hqr tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.hqr/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.hqr/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — HQ Runtime auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of HQ Runtime config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while HQ Runtime is working.',
    ...
    '/reload reloads ~/.hqr/.env into the running session — pick up new API keys without restarting.',
    ...
    "HQR_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hqr/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hqr/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'hqr curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hqr curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hqr curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'hqr auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'HQR_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "HQR_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hqr dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hqr dashboard embeds the full HQ Runtime TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.hqr/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hqr/dashboard-plugins/ — no npm build required.',
    ...
    'HQR_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'HQR_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'HQR_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'HQR_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hqr/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hqr/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hqr -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hqr chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hqr chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hqr chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hqr chat --source tool tags programmatic chats so they don't clutter hqr sessions list.",
    'hqr dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hqr sessions rename <ID> "new title" renames any past session; hqr sessions delete <ID> removes one.',
    'hqr import restores a full HQ Runtime backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'hqr fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hqr pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hqr setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hqr status --deep runs the full health sweep across every component; plain hqr status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'HQR_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HQR_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hqr/plugins/ — trust-gated by design.',
    "HQR_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'HQR_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'HQR_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'HQR_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'HQR_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'HQR_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'HQR_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'HQR_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'HQR_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'HQR_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'HQR_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes HQ Runtime block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from hqr sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hqr/dashboard-plugins/.',
]
