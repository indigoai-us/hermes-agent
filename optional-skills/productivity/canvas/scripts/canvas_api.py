#!/usr/bin/env python3
"""Canvas LMS API CLI for HQ Runtime Agent.
...
def _check_config():
    ...
    if missing:
        hqr_env = os.path.join(
            os.environ.get("HQR_HOME", os.path.expanduser("~/.hqr")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {hqr_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for HQ Runtime Agent"
    )
