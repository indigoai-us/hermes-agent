...
is typically only in `~/.hqr/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike HQ Runtime,
...
`hqr-custom-providers` skill, pitfall 2). HQ Runtime hits this because it forwards
