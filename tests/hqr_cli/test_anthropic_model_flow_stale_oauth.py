"""Tests for Bug #12905 fix — stale OAuth token detection in hqr model flow.

Bug 3: `hqr model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.hqr/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("hqr_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from hqr_cli.main import _model_flow_anthropic
        cfg = {}
