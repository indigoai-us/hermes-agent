...
class PlatformEntry:
    ...
    # status displays (``hqr setup``, ``hqr status``, the dashboard
    ...
    # Env vars this platform needs (for ``hqr setup`` display).
    ...
    # built-ins).  Used by ``hqr gateway setup`` to auto-enable the
    ...
    # hard-casing in HQ Runtime core.
    #
    # Signature:
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved HQR_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `hqr`
        # invocation -- including plain `hqr chat`, which never touches any
        ...
        # `hqr setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``hqr setup``/``gateway status``, channel
