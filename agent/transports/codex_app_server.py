...
projection into HQ Runtime' display, approval bridging, transcript projection into
...
"codex_app_server"`. HQ Runtime' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 HQ Runtime secret — gateway
        ...
        spawn_env = hqr_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("HQR_KANBAN_TASK"):
            kanban_db = spawn_env.get("HQR_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "HQR_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("HQR_HOME", os.path.expanduser("~/.hqr")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "hqr",
        client_title: str = "HQ Runtime Agent",
