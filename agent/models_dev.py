...
Data resolution order:
  ...
  2. Disk cache (~/.hqr/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: HQ Runtime ↔ models.dev
# ---------------------------------------------------------------------------

# HQ Runtime provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; HQ Runtime' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → HQ Runtime ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_hqr_ids(mdev_id: str) -> List[str]:
    """Return the HQ Runtime provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for hqr_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(hqr_id)
...
def _get_cache_path() -> Path:
    ...
    return get_hqr_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from hqr_constants import get_hqr_home
    return get_hqr_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``hqr config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the HQ Runtime provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by HQ Runtime id.
    for hqr_id in _models_dev_to_hqr_ids(provider_key):
        if hqr_id != provider_key:
            candidates.append(hqr_id)
...
def _get_provider_models(
    ...
    """Resolve a HQ Runtime provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``hqr model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from hqr_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal HQ Runtime agent traffic.
...
def get_provider_info(
    ...
    Accepts either a HQ Runtime provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve HQ Runtime ID → models.dev ID
...
def get_model_info(
    ...
    Accepts HQ Runtime or models.dev provider ID.  Tries exact match then
