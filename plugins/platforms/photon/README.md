...
This plugin connects HQ Runtime Agent to iMessage (and other Spectrum
...
TypeScript-only, HQ Runtime runs it inside a small supervised Node sidecar and
...
hqr photon setup --phone +15551234567

# Start the gateway
hqr gateway start
```

`hqr photon setup` does, in order:

1. **Device login** (RFC 8628, `client_id=photon-cli`) — opens
   `https://app.photon.codes/` for approval and stores the bearer token.
2. **Find or create** the `HQ Runtime Agent` project on the Photon dashboard.
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.hqr/.env` so the
...
There is no separate `login` command; like every other HQ Runtime channel,
...
Run `hqr photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.hqr/.env` (the same place every other
...
Management metadata lives in `~/.hqr/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `hqr photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, HQ Runtime
  ...
  `.pluginPayloadAttachment` images immediately after the URL; HQ Runtime coalesces
...
- **Native polls are supported.** HQ Runtime posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `hqr send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<hqr-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `hqr photon status`, a DM and a group roundtrip,
