...
version: 1.1.3
author: xdevplatform + openclaw + HQ Runtime Agent
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by HQ Runtime tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official HQ Runtime Docker layout, `/opt/data` is `HQR_HOME`, but HQ Runtime tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for HQ Runtime-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, HQ Runtime tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the HQ Runtime subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- HQ Runtime adaptation: reformatted for HQ Runtime skill conventions; safety guardrails preserved verbatim.
