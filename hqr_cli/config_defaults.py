"""Default configuration data for HQ Runtime Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from hqr_cli/config.py. Must not import from hqr_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hqr-cli"],
    # SQLite journal mode used by every HQ Runtime database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running HQ Runtime server processes.
    ...
    "session": {
        # Per-terminal `hqr -c`: each CLI session drops a breadcrumb file
        # under $HQR_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # an interactive `hqr gateway restart` must never block for hours
        ...
        # the HQ Runtime-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps HQ Runtime
        ...
        # identity slot (SOUL.md). Empty by default. The HQR_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, HQ Runtime adds a coding operating brief
        ...
        # `hqr status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``HQR_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             HQR_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force HQR_HOME/home when it exists (old strict
        ...
        # skipped. When empty, HQ Runtime auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), HQ Runtime sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when HQ Runtime
        ...
        #  "/home/user/.hqr/cache/documents:/output"]
        ...
        # bundled HQ Runtime image, which drops to the `hqr` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, HQ Runtime sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and HQ Runtime should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``hqr chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.hqr/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: hqr sweeps the checkpoint base at startup
        ...
        # ``hqr checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .hqr.md, or .cursorrules before HQ Runtime applies
    ...
    # Single-query (``hqr -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, HQ Runtime truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so HQ Runtime' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); hqr =
                                      # HQ Runtime' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. HQ Runtime' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before HQ Runtime does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``hqr kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``hqr kanban decompose`` and the kanban
        ...
        # ``hqr profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `hqr model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hqr` (and `hqr chat`) launches by default:
        ...
        # REPL and `--tui` (or HQR_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hqr --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hqr -c` muscle memory.  Default off so existing
        # users aren't surprised.  HQR_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `hqr --tui` drops a one-time hint
        ...
        # model — see hqr_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to HQR_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/hqr`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `hqr pets`. Disabled until
        ...
        "pet": {
            ...
            # get_hqr_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``HQR_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``HQR_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``HQR_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``HQR_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``HQR_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/hqr``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; HQ Runtime appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.hqr/cache/piper-voices/
    ...
    # "Hey HQ Runtime" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey hqr",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.hqr/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.hqr/cache/delegation/
    ...
    # response. If not, HQ Runtime feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before HQ Runtime auto-pauses the goal and
    ...
    "moa": {
        ...
        # <hqr_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "skills": {
        ...
        # with ``hqr skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``hqr skills trust`` / ``hqr skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${HQR_SKILL_DIR} and ${HQR_SESSION_ID} in SKILL.md
        ...
        # ~/.hqr/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.hqr/.curator_backups/blobs/. Enables `hqr curator ledger`
        # and single-mutation `hqr curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `hqr curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `hqr curator run --consolidate` overrides
        ...
        # normally restored on every `hqr update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `hqr curator purge` deletes
        ...
        # skipped), snapshot ~/.hqr/skills/ into
        # ~/.hqr/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `hqr curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for hqr-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *HQ Runtime Agent*" header.
    ...
    # dangerous command. -q runs export HQR_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; HQR_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace HQ Runtime' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.hqr/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or HQR_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .hqr config) always require human
        ...
        # longer trigger the startup banner. Add via `hqr doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hqr_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow HQ Runtime to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $HQR_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via HQR_CRON_MAX_PARALLEL env var.
        ...
        # HQR_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # HQR_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `hqr -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `hqr` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `hqr kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     hqr-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core HQ Runtime tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.hqr/logs/.
    ...
    # update model picker lists without shipping a hqr-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `hqr model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the HQ Runtime provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal HQR_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.hqr/sessions/sessions.json entirely.
        ...
        # the HQR_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.hqr.gateway` that defenses 1-2 don't
        ...
        # `hqr update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``HQR_GATEWAY_MAX_STARTS`` / ``HQR_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.hqr/.env,
        ...
        # behavior: files must live under the HQ Runtime cache, under
        ...
        # user. Bridged to HQR_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the HQ Runtime
        # cache (~/.hqr/cache/{documents,images,audio,video,screenshots})
        ...
        # to HQR_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # HQR_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to HQR_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.hqr/state.db.
    ...
    "sessions": {
        ...
        # the default of ``hqr sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.hqr/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `hqr sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `hqr update` prints a one-line notice with
        ...
        # ~/.hqr/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to HQR_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # HQR_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `hqr sessions export`. Checked
    ...
    # ``hqr doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `hqr doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``hqr update`` behaviour.
    "updates": {
        ...
        #     DBs) into <HQR_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``hqr backup``-style zip
        #     of HQR_HOME into <HQR_HOME>/backups/, restorable with
        #     ``hqr import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `hqr update` does with uncommitted local changes to the
    ...
    # `hqr tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.hqr/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.hqr/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.hqr/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, HQ Runtime may use this encrypted
            ...
            # ~/.hqr/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.hqr/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. HQ Runtime disables it for our users unless they
        # explicitly opt in here. When false (default), HQ Runtime sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; HQ Runtime also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each HQ Runtime computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". HQ Runtime passes the
    ...
    #
    # Configure with `hqr egress setup`.  Disabled by default — the rest of
    # HQ Runtime works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.hqr/bin/ on
    ...
    # HQ Runtime Desktop (Electron app) launch options. These only affect
    # `hqr desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the HQR_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the HQR_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials HQ Runtime mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'hqr model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the HQ Runtime auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey HQ Runtime' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use HQ Runtime, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @hqr:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HQR_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #hqr)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: hqr-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'hqr-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote HQ Runtime API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote HQ Runtime API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote HQ Runtime API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # HQR_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to HQR_TOOL_PROGRESS_MODE for backward
    ...
    # HQR_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "HQR_PREFILL_MESSAGES_FILE": {
