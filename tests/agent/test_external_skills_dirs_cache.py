...
@pytest.fixture
def hqr_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.hqr/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".hqr"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(hqr_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = hqr_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different HQR_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".hqr"
