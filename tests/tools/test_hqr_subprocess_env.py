"""Tests for hqr_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_HQR_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
_TIER1_SAMPLE = {
    ...
    "HQR_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return hqr_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "HQR_KANBAN_TASK": "t_parent",
                "HQR_KANBAN_RUN_ID": "123",
                "HQR_KANBAN_DB": "/tmp/parent-kanban.db",
                "HQR_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
