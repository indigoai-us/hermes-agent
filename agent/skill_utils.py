...
from hqr_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.hqr/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    HQR_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``HQR_KANBAN_TASK`` / ``HQR_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("HQR_KANBAN_TASK") or os.getenv("HQR_KANBAN_BOARD"):
        ...
        # The HQ Runtime Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``hqr_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``HQR_PLATFORM`` or
            ``HQR_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("HQR_PLATFORM")
        or get_session_env("HQR_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``hqr config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``hqr`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.hqr/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``hqr`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against HQR_HOME, not cwd
        if not p.is_absolute():
            p = (hqr_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.hqr/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.hqr/skills`` at the git root) are
...
#   <root>/.hqr/skills/   — HQ Runtime-native location
...
# a one-line "run `hqr skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when HQR_HOME itself lives inside a git checkout:
    ``<root>/.hqr/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``hqr skills trust``. Returns None when there is nothing to notify
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.hqr/skills/`` (including via symlinks) or configured
    ...
    # ~/.hqr/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: HQ Runtime can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    hqr = metadata.get("hqr") or {}
    if not isinstance(hqr, dict):
        hqr = {}
    return {
        "fallback_for_toolsets": hqr.get("fallback_for_toolsets", []),
        "requires_toolsets": hqr.get("requires_toolsets", []),
        "fallback_for_tools": hqr.get("fallback_for_tools", []),
        "requires_tools": hqr.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes HQ Runtime metadata, VCS, virtualenv/dependency, cache, and skill
