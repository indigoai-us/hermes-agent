...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=hqr"
...
def _load_supermemory_config(hqr_home: str) -> dict:
    config = _default_config()
    config_path = Path(hqr_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, hqr_home: str) -> None:
    config_path = Path(hqr_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "hqr"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes HQ Runtime writes into the "HQ Runtime" Space in the Supermemory
        ...
        merged = {"sm_source": "hqr", **(metadata or {})}
...
def _resolve_container_tag_for_setup(hqr_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(hqr_home)
...
def _probe_supermemory_connection(api_key: str, hqr_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(hqr_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(hqr_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `hqr memory setup`.
        # All other options are documented for $HQR_HOME/supermemory.json
    ...
    def save_config(self, values, hqr_home):
        ...
        _save_supermemory_config(sanitized, hqr_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, hqr_home)
    ...
    def post_setup(self, hqr_home: str, config: dict) -> None:
        ...
        from hqr_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(hqr_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from hqr_constants import get_hqr_home
        self._hqr_home = kwargs.get("hqr_home") or str(get_hqr_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._hqr_home)
