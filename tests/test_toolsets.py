...
class TestToolsetConsistency:
    ...
    def test_hqr_platforms_share_core_tools(self):
        """All hqr-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on hqr-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["hqr-cli", "hqr-telegram", "hqr-discord", "hqr-whatsapp", "hqr-slack", "hqr-signal", "hqr-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hqr_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hqr-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("hqr-cli")
        second = resolve_toolset("hqr-cli")
        ...
        assert (
            "hqr-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("hqr-cli", include_registry=False)
        second = resolve_toolset("hqr-cli", include_registry=False)
        assert first == second
