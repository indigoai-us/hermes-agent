...
def _restore_tool_modules():
    original_hqr_home = os.environ.get("HQR_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hqr_home is None:
            os.environ.pop("HQR_HOME", None)
        else:
            os.environ["HQR_HOME"] = original_hqr_home
        _reset_modules(("tools", "hqr_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    hqr_cli = types.ModuleType("hqr_cli")
    hqr_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["hqr_cli"] = hqr_cli
    hqr_home = tmp_path / "hqr-home"
    os.environ["HQR_HOME"] = str(hqr_home)
    sys.modules["hqr_cli.config"] = types.SimpleNamespace(
        get_hqr_home=lambda: hqr_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="hqr-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": hqr_home / "modal_snapshots.json",
