...
The ``hqr update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m hqr_cli.main gateway run`` — image name
    ``pythonw``, not ``hqr.exe`` — so the ``taskkill /IM hqr.exe`` guard
    ...
    # The hqr.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM hqr.exe' in text
