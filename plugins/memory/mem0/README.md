...
```bash
hqr memory setup    # select "mem0"
```

Or manually:
```bash
hqr config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.hqr/.env
```

## Config

Behavioral settings live in `$HQR_HOME/mem0.json` (set them via `hqr memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.hqr/.env`.
...
| `user_id` | `hqr-user` | User identifier on Mem0 |
| `agent_id` | `hqr` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   hqr memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   hqr memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.hqr/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.hqr/.env
   ```
   or in `$HQR_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh HQ Runtime session and call `mem0_search` — it connects to your server.
...
```bash
hqr memory setup mem0 --mode oss \
...
```bash
hqr memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$HQR_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.hqr/mem0_qdrant"}}
  }
}
```
...
```bash
hqr memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
hqr memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$HQR_HOME/mem0.json`).
