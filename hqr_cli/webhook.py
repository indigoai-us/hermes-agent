"""hqr webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    hqr webhook subscribe <name> [options]
    hqr webhook list
    hqr webhook remove <name>
    hqr webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.hqr/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _hqr_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: hqr gateway run
"""
...
def webhook_command(args):
    """Entry point for 'hqr webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: hqr webhook {subscribe|list|remove|test}")
        print("Run 'hqr webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (hqr gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: hqr webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from hqr webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (hqr gateway run)")
