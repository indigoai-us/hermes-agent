...
Companion skill for the `unreal-engine` entry in the HQ Runtime MCP catalog. The
...
Two halves, in this order: the editor side must be up before HQ Runtime connects.
...
### One-time, HQ Runtime side

    hqr mcp install unreal-engine
...
`~/.hqr/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for HQ Runtime — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. HQ Runtime connects
...
2. Start the HQ Runtime session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through HQ Runtime
they appear as:

| HQ Runtime tool | Purpose |
|---|---|
...
then happens at `hqr mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — HQ Runtime runs batched calls
...
- **Start order matters.** Editor + server up first, then the HQ Runtime
...
- **Timeouts on long operations.** HQ Runtime' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.hqr/config.yaml` for
