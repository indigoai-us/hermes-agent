...
Write plugins for the HQ Runtime desktop app: statusbar items, layout panes,
...
system (`~/.hqr/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$HQR_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$HQR_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The HQ Runtime desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$HQR_HOME/desktop-plugins/` (usually
  `~/.hqr/desktop-plugins/`).

## How to Run

1. Create `$HQR_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.hqr/...` by default, or `~/.hqr/profiles/<profile>/...` under a
...
The ONLY import surface is `@hqr/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from HQ Runtime (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.hqr/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@hqr/plugin-sdk`, `react`, and
