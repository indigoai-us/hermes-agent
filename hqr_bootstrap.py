"""Windows UTF-8 bootstrap for HQ Runtime entry points.
...
should be imported at the very top of every HQ Runtime entry point
(``hqr``, ``hqr-agent``, ``hqr-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``hqr_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``hqr_bootstrap``, never ``hqr_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing HQ Runtime modules.

    HQ Runtime ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real HQ Runtime source root
    ...
    """
    root = src_root or os.environ.get("HQR_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``HQR_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("HQR_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import hqr_bootstrap``
# (or ``from hqr_bootstrap import apply_windows_utf8_bootstrap``) at
