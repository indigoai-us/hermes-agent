...
_FORCE_SYNC_ENV = "HQR_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.hqr") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.hqr to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``HQR_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hqr_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.hqr/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(hqr_home=hqr_home)

    def _sync_back_transaction(self, hqr_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="hqr-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.hqr/skills/a.md`` →
        ``~/.hqr/skills/a.md``, a new remote file at
        ``/root/.hqr/skills/b.md`` maps to ``~/.hqr/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
