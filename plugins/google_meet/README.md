...
Let the hqr agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where hqr runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hqr meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hqr meet` CLI |
...
| `cli.py` | `hqr meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HQR_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `hqr meet node {run,list,approve,remove,status,ping}` |
...
hqr meet install                                      # pip + Chromium
hqr meet setup                                        # preflight
hqr meet auth                                         # optional
hqr meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hqr meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hqr/.env
hqr meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hqr meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hqr meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hqr/.env
hqr meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, hqr will **not** switch your system audio input automatically — the
...
hqr plugins enable google_meet
hqr meet node run --display-name my-mac --host 0.0.0.0 --port 18789
