...
Optional HQ Runtime observability plugin that configures exporters and maps
HQ Runtime-specific observer hooks to NeMo Relay marks and ATIF state. HQ Runtime core
...
not replace HQ Runtime Agent's planner, tools, memory, model provider routing, or
CLI UX. HQ Runtime core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, HQ Runtime Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by HQ Runtime core.
- Add HQ Runtime session, turn, approval, and subagent mark events.
...
```bash
hqr plugins enable observability/nemo_relay
```

The `HQR_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `HQR_HOME` that the
agent run will use:

```bash
env HQR_HOME=/tmp/hqr-nemo-relay-test \
  hqr plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`HQR_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`HQR_HOME` is the HQ Runtime profile/config home used by both
`hqr plugins enable ...` and the later `hqr chat ...` run. If unset,
HQ Runtime uses the user's default home, usually `~/.hqr`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export HQR_HOME=/tmp/hqr-nemo-relay-test
hqr plugins enable observability/nemo_relay
hqr chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `hqr` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run hqr plugins enable observability/nemo_relay
uv run hqr chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, HQ Runtime installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/hqr_agent-*.whl
hqr plugins enable observability/nemo_relay
```
...
behavior; it does not control HQ Runtime shared client metrics.
...
`hqr.session.segment` / `hqr.session.segment_reason`
...
The plugin can configure exporters directly from `HQR_NEMO_RELAY_*`
...
```bash
export HQR_NEMO_RELAY_ATOF_ENABLED=1
export HQR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export HQR_NEMO_RELAY_ATIF_ENABLED=1
export HQR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `HQR_NEMO_RELAY_ATOF_FILENAME`
- `HQR_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `HQR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `HQR_NEMO_RELAY_ATIF_AGENT_NAME`
- `HQR_NEMO_RELAY_ATIF_AGENT_VERSION`
- `HQR_NEMO_RELAY_ATIF_MODEL_NAME`
- `HQR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point HQ Runtime at it:

```bash
export HQR_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "HQ Runtime Agent"
agent_version = "local"
```

When `HQR_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`HQR_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`HQR_NEMO_RELAY_ATIF_*` fallback setup is also skipped so HQ Runtime does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
HQ Runtime keeps the direct env-var fallbacks active for that run.

HQ Runtime core routes provider and tool execution through NeMo Relay managed APIs
...
HQ Runtime uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with HQ Runtime-owned
...
with this HQ Runtime-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. HQ Runtime rejects
...
HQ Runtime activates these plugins before registering its managed LLM and tool
...
For the full generic HQ Runtime middleware contract, see
...
with HQ Runtime and a local Ollama model served through the OpenAI-compatible API.

```bash
export HQR_HOME=/tmp/hqr-nemo-relay-docs/hqr-home
mkdir -p "$HQR_HOME"

cat > "$HQR_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent HQ Runtime session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HQR_NEMO_RELAY_ATOF_ENABLED=1
export HQR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hqr-nemo-relay-docs/subagent/atof
export HQR_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HQR_NEMO_RELAY_ATOF_MODE=overwrite
export HQR_NEMO_RELAY_ATIF_ENABLED=1
export HQR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hqr-nemo-relay-docs/subagent/atif
export HQR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HQR_NEMO_RELAY_ATIF_AGENT_NAME='HQ Runtime Agent E2E'
export HQR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export HQR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"hqr.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "HQ Runtime Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. HQ Runtime dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/hqr-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/hqr-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/hqr-nemo-relay-docs/workdir/beta.txt
cd /tmp/hqr-nemo-relay-docs/workdir

export HQR_NEMO_RELAY_ATOF_ENABLED=1
export HQR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hqr-nemo-relay-docs/parallel/atof
export HQR_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HQR_NEMO_RELAY_ATOF_MODE=overwrite
export HQR_NEMO_RELAY_ATIF_ENABLED=1
export HQR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hqr-nemo-relay-docs/parallel/atif
export HQR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export HQR_NEMO_RELAY_ATIF_AGENT_NAME='HQ Runtime Agent E2E'
export HQR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- HQ Runtime sessions map to `agent` scopes.
- HQ Runtime core managed provider calls map to `llm` scope start/end events.
- HQ Runtime core managed tool calls map to `tool` scope start/end events.
...
HQ Runtime core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a HQ Runtime-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, HQ Runtime calls the
...
Enable it for HQ Runtime:

```bash
export HQR_NEMO_RELAY_PLUGINS_TOML=/tmp/hqr-middleware-test/plugins.toml
```

Execution follows these boundaries with or without an adaptive component:

```text
HQ Runtime provider call
  -> nemo_relay.llm.execute(...)
    -> HQ Runtime provider adapter callback(...)

HQ Runtime tool call
  -> nemo_relay.tools.execute(...)
    -> HQ Runtime authorization and dispatch callback(...)
```
...
middleware for a local HQ Runtime run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export HQR_HOME=/tmp/hqr-middleware-test/hqr-home
mkdir -p "$HQR_HOME" /tmp/hqr-middleware-test/nemo-relay
...
cat > /tmp/hqr-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/hqr-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/hqr-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "HQ Runtime Middleware E2E"
agent_version = "local"
...
export HQR_NEMO_RELAY_PLUGINS_TOML=/tmp/hqr-middleware-test/nemo-relay/plugins.toml
...
```json
{
  ...
  "session_id": "middleware-demo-session",
  "agent": {
    "name": "HQ Runtime Middleware E2E",
    "version": "local",
