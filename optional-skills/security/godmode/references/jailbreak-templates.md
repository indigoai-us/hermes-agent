...
## 5. GODMODE FAST — HQ Runtime 4 405B (Zero Refusal)

**Model:** `nousresearch/hermes-4-405b`
**Strategy:** HQ Runtime models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with HQ Runtime Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.hqr/config.yaml`:
