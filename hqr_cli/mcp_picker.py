"""MCP picker — interactive `hqr mcp picker` (also the default `hqr mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``hqr mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `hqr plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from hqr_cli.cli_output import prompt_yes_no
from hqr_cli.curses_ui import curses_single_select
from hqr_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new HQ Runtime session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from hqr_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `hqr mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: hqr mcp install <name>    Picker: hqr mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their HQ Runtime is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer HQ Runtime — run `hqr update` "
...
def show_catalog() -> None:
    """`hqr mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`hqr mcp picker` (and default `hqr mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`hqr mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `hqr mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
