...
``~/.hqr/skills/.curator_ledger.jsonl`` describing who changed what, with
before/after file manifests whose contents are stored content-addressed
(sha256-deduped) under ``~/.hqr/.curator_backups/blobs/``.
...
def ledger_path() -> Path:
    return get_hqr_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_hqr_home() / ".curator_backups" / "blobs"
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """All paths in an entry must live under HQR_HOME. Defense in depth —
