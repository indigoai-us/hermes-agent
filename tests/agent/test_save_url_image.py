...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate HQR_HOME under tmp_path."""
    monkeypatch.setenv("HQR_HOME", str(tmp_path / ".hqr"))
    (tmp_path / ".hqr").mkdir()

    # Force the constants/image cache helpers to re-read HQR_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("hqr_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hqr_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under HQR_HOME — gateway cleanup
