...
def hqr_home(tmp_path, monkeypatch):
    ...
    # /etc/hqr on the dev/CI box can't influence the test.
    monkeypatch.setenv("HQR_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(hqr_home, monkeypatch):
...
def test_env_expansion_in_user_config(hqr_home, monkeypatch):
    from hqr_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_hqr_dotenv(hqr_home=str(home))
