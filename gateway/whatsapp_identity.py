...
def expand_whatsapp_aliases(identifier: str) -> Set[str]:
    ...
    bridge's ``$HQR_HOME/whatsapp/session/lid-mapping-*.json`` files,
    ...
    session_dir = get_hqr_dir("platforms/whatsapp/session", "whatsapp/session")
...
def canonical_whatsapp_identifier(identifier: str) -> str:
    ...
    same session-key identity HQ Runtime itself uses.

    Plugins that need per-sender behaviour (role-based routing,
    authorisation, per-contact policy) should use this so their
    bookkeeping lines up with HQ Runtime' session bookkeeping even when
