...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.hqr/.env`` into the gateway process via
    ``hqr_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m hqr_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "hqr_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m hqr_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=hqr_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from hqr_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind HQR_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_hqr_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("HQR_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("HQR_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_hqr_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off HQR_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from hqr_constants import set_hqr_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from hqr_constants import reset_hqr_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``hqr journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from hqr_cli.skills_hub import do_install
        ...
        if action == "browse":
            from hqr_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from hqr_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `hqr mcp` offers, so
    ...
    try:
        ...
        from hqr_cli import mcp_catalog
...
# (hqr_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes HQR_HOME via set_hqr_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses hqr_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from hqr_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from hqr_cli.config import load_config, save_config, save_env_value
        from hqr_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from hqr_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    the SAME interactive MCP OAuth machinery ``hqr mcp login`` uses
    ...
    try:
        ...
        hqr_home = str(get_hqr_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(hqr_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``hqr plugins`` / the dashboard, so the three surfaces
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native HQ Runtime plugin.
