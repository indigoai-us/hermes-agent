...
def hqr_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves HQR_HOME.
...
@pytest.fixture()
def server(hqr_home, monkeypatch):
    ...
    # Pin config resolution to the isolated HQR_HOME. Sibling test
    ...
    # module-level ``_hqr_home = get_hqr_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_hqr_home", hqr_home)
...
def test_moa_bare_returns_usage(server, session, hqr_home):
    _write_moa_config(hqr_home, """
moa:
