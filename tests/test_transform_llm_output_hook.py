...
from hqr_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(hqr_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <hqr_home>/plugins/<name> and opt it in."""
    plugin_dir = hqr_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HQR_HOME", str(tmp_path / "hqr_empty"))
