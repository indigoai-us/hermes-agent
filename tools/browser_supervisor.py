...
One ``CDPSupervisor`` runs per HQ Runtime ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach HQ Runtime logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "hqr-dialog-bridge.invalid"
...
(() => {
  if (window.__hqrDialogBridgeInstalled) return;
  window.__hqrDialogBridgeInstalled = true;
  const ENDPOINT = "http://hqr-dialog-bridge.invalid/";
