...
Every HQ Runtime request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to HQ Runtime Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=hqr-agent",
        "client=hqr-client-v<__version__>",
    ]

The version is sourced live from ``hqr_cli.__version__`` so it auto-aligns
...
``hqr_cli.__version__`` is the canonical source of truth.
"""
...
def _hqr_version() -> str:
    """Return the current HQ Runtime release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``hqr_cli`` cannot be imported (should
...
def hqr_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=hqr-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=hqr-client-v{_hqr_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a HQ Runtime session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    HQ Runtime conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.hqr/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific HQ Runtime
    ...
    """
    tags = ["product=hqr-agent", hqr_client_tag()]
