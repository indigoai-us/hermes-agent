"""
Top-level argparse construction for the hqr CLI.
...
# argparse runs (it sets ``HQR_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``hqr_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    hqr                        Start interactive chat
    hqr chat -q "Hello"        Single query mode
    hqr --tui                  Launch the modern TUI (or set display.interface: tui)
    hqr --cli                  Force the classic REPL (overrides display.interface: tui)
    hqr -c                     Resume the most recent session
    hqr -c "my project"        Resume a session by name (latest in lineage)
    hqr --resume <session_id>  Resume a specific session by ID
    hqr --resume latest        Resume the most recent session (same as -c)
    hqr --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    hqr setup                  Run setup wizard
    hqr logout                 Clear stored authentication
    hqr auth add <provider>    Add a pooled credential
    hqr auth list              List pooled credentials
    hqr auth remove <p> <t>    Remove pooled credential by index, id, or label
    hqr auth reset <provider>  Clear exhaustion status for a provider
    hqr model                  Select default model
    hqr fallback [list]        Show fallback provider chain
    hqr fallback add           Add a fallback provider (same picker as `hqr model`)
    hqr fallback remove        Remove a fallback provider from the chain
    hqr config                 View configuration
    hqr config edit            Edit config in $EDITOR
    hqr config set model gpt-4 Set a config value
    hqr gateway                Run messaging gateway
    hqr -s hqr-agent-dev,github-auth
    hqr -w                     Start in isolated git worktree
    hqr gateway install        Install gateway background service
    hqr sessions list          List past sessions
    hqr sessions browse        Interactive session picker
    hqr sessions rename ID T   Rename/title a session
    hqr logs                   View agent.log (last 50 lines)
    hqr logs -f                Follow agent.log in real time
    hqr logs errors            View errors.log
    hqr logs --since 1h        Lines from the last hour
    hqr debug share             Upload debug report for support
    hqr console                Open the safe HQ Runtime command console
    hqr update                 Update to latest version
    hqr dashboard              Start web UI dashboard (port 9119)
    hqr dashboard --stop       Stop running dashboard processes
    hqr dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="hqr",
        description="HQ Runtime Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `hqr chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via HQR_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `hqr setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to HQR_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.hqr/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with HQ Runtime Agent",
    )
    ...
    # top-level parser: when the user writes `hqr -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/hqr_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also HQR_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.hqr/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from HQ Runtime itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
