...
- Give it a title like "hqr-agent-<machine-name>"
...
uv run python "${HQR_HOME:-$HOME/.hqr}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _hqr_env="${HQR_HOME:-$HOME/.hqr}/.env"; [ -f "$_hqr_env" ] && grep -q "^GITHUB_TOKEN=" "$_hqr_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hqr_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${HQR_HOME:-$HOME/.hqr}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
