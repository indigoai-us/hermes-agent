...
class TestSpawnGatewayRestartReapsOrphans:
    ...
    @patch("hqr_cli.web_server._gateway_subcommand", return_value=["gateway", "restart"])
    @patch("hqr_cli.web_server._spawn_hqr_action")
    @patch("hqr_cli.web_server._ACTION_PROCS", {})
    def test_reap_called_before_spawn(self, mock_spawn, mock_subcmd):
        ...
        from hqr_cli.web_server import _spawn_gateway_restart

        with patch(
            "hqr_cli.gateway._reap_unsupervised_gateway_orphans"
        ) as mock_reap:
