...
| **P11.2** | **Slack** | **Arm the reload from every auth-error site + store-backed bolt authorize + startup-race re-read.** P11.1 armed the reactive reload only from the attachment-download classifier, but the token expires on the PRIMARY paths: slack_bolt `AsyncSingleTeamAuthorization` (which authenticates a per-request client built from bolt's *captured* token, so a reloaded token never reached it) and `gateway.channel_directory`. dbooth (v2.22) logged `token_expired` for hours with a VALID on-disk token and never armed. P11.2: (1) centralizes arming in `_SlackBotTokenStore.note_auth_error` — any `invalid_auth`/`token_revoked`/`token_expired` seen on any client (via a `_ReloadableAsyncWebClient.api_call` override) or the bolt authorize path arms the reload; (2) replaces bolt's static token with a store-backed `authorize` callable that reads the live token, caches auth.test by token value, and arms on error; (3) a one-shot post-connect token-file re-read (T+5s) closes the startup race where the env-seeded token predates the refresher's first write. | `plugins/platforms/slack/adapter.py` (`note_auth_error`, `_ReloadableAsyncWebClient.api_call`, `_handle_store_auth_error`, `_make_store_authorize`, `_schedule_post_connect_token_reread`) | Inherits `platforms.slack.bot_token_reload_enabled`; degraded reporting unconditional |
...
- P11 / P11.1 / P11.2: `tests/gateway/test_slack_bot_token_reload.py`,
  `tests/gateway/test_slack_bot_token_reload_provider.py`,
  `tests/gateway/test_slack_bot_token_reload_p112.py`
