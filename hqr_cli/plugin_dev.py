"""Runtime-backed validation behind ``hqr plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary HQR_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="hqr-plugin-doctor-")
    stack = ExitStack()
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "HQR_HOME": str(home),
                "HQR_BUNDLED_PLUGINS": str(bundled),
                "HQR_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    modules_before = {
        ...
        if name == "hqr_plugins" or name.startswith("hqr_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"HQ Runtime discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "hqr_plugins" or name.startswith("hqr_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_hqr_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".hqr" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from hqr_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this HQ Runtime supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — HQ Runtime never auto-installs plugin dependencies; "
    ...
    if schema:
        from hqr_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through HQ Runtime' real scanner and registration path."""
    try:
