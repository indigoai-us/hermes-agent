"""Tests for hqr_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_hqr_home):
    """Drop the example-dashboard fixture into the per-test HQR_HOME
    ...
    ``HQR_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, hqr-achievements, model providers
    ...
    from hqr_cli import web_server

    user_plugins_dir = get_hqr_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `hqr plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down HQR_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known HQ Runtime vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints HQR_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import hqr_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("HQR_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("HQR_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hqr_home):
        """Create a TestClient and isolate the state DB under the test HQR_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import hqr_state
        from hqr_constants import get_hqr_home
        from hqr_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(hqr_state, "DEFAULT_DB_PATH", get_hqr_home() / "state.db")
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import hqr_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `hqr update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `hqr update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(hqr_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = hqr_state.SessionDB.__init__
        ...
        # the class on hqr_state covers every open the helper makes.
        monkeypatch.setattr(hqr_state.SessionDB, "__init__", counting_init)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        HQR_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from hqr_constants import get_hqr_home
        from hqr_cli.config import load_env
        ...
        config_path = get_hqr_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from hqr_constants import get_hqr_home
        from hqr_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide HQR_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from hqr_constants import get_hqr_home

        path = get_hqr_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_hqr_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["hqr"]["workspace"] == "myws"
        assert cfg["hosts"]["hqr"]["peerName"] == "eri"
        assert cfg["hosts"]["hqr"]["environment"] == "local"
        assert cfg["hosts"]["hqr"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["hqr"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from hqr_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_hqr_returns_docker_guidance_without_spawning(self, monkeypatch):
        import hqr_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn hqr update")
        ...
        monkeypatch.setattr(web_server, "_spawn_hqr_action", fail_spawn)
        web_server._ACTION_PROCS.pop("hqr-update", None)
        web_server._ACTION_RESULTS.pop("hqr-update", None)

        resp = self.client.post("/api/hqr/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "hqr-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/hermes-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/hqr-update/status")
    ...
    def test_update_hqr_spawns_with_action_id(self, monkeypatch):
        import hqr_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_hqr_action", fake_spawn)
        web_server._ACTION_PROCS.pop("hqr-update", None)
        web_server._ACTION_RESULTS.pop("hqr-update", None)
        ...
        assert calls == [
            (["update"], "hqr-update", {"HQR_ACTION_ID": "a" * 32})
        ]

    def test_update_hqr_reuses_running_action(self, monkeypatch):
        import hqr_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["hqr-update"] = Proc()
        web_server._ACTION_IDS["hqr-update"] = "b" * 32

        try:
            resp = self.client.post("/api/hqr/update")
        finally:
            web_server._ACTION_PROCS.pop("hqr-update", None)
            web_server._ACTION_IDS.pop("hqr-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/hqr_cli/test_setup_hidden_env.py.
        """
        from hqr_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a HQ Runtime provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from hqr_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from hqr_cli.config import save_env_value
        from hqr_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "hqr_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/HqrSetupBot/hqr_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/HqrSetupBot/hqr_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "hqr_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_hqr_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from hqr_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from hqr_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``hqr model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from hqr_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from hqr_cli import profiles as profiles_mod
        from hqr_cli.config import custom_endpoint_key_env
        from hqr_constants import get_hqr_home

        default_home = get_hqr_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from hqr_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from hqr_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from hqr_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from hqr_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from hqr_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_hqr_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from hqr_constants import get_hqr_home
        import hqr_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import hqr_cli.tools_config as tools_config
        from hqr_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import hqr_cli.tools_config as tools_config
        from hqr_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "hqr_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        assert "hqr@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from hqr_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from hqr_cli.web_server import _denormalize_config_from_web
        from hqr_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from hqr_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.hqr/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_hqr_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(hqr_home, name="ocean"):
        themes_dir = hqr_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="hqr-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hqr_home):
        ...
        monkeypatch.setattr(
            hqr_state, "DEFAULT_DB_PATH", get_hqr_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``hqr_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in hqr_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hqr_home):
        ...
        # Pin the SessionDB to the isolated HQR_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_hqr_home, _install_example_plugin):
        ...
        installs it into the per-test ``HQR_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into HQR_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, hqr-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (hqr-achievements is loaded by default).
        resp = self.client.get("/api/plugins/hqr-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        HQR_HOME override scopes a request to another profile."""
        from hqr_constants import (
            reset_hqr_home_override,
            set_hqr_home_override,
        )
        ...
        monkeypatch.setenv("HQR_HOME", str(launch_home))
        from hqr_cli import web_server
        token = set_hqr_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets HQR_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the hqr root's plugins/ directory."""
        root = tmp_path / "hqr-root"
        ...
        monkeypatch.setenv("HQR_HOME", str(profile_home))
        from hqr_cli import web_server
        plugins = web_server._discover_dashboard_plugins()
...
# or ``sh -c 'printf …'``) instead of the real ``hqr --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import hqr_cli.main as main_mod

        command = f"hqr-review-python{Path(sys.executable).suffix}"
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from hqr_cli.pty_bridge import PtyUnavailableError
        ...
        import hqr_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from hqr_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import hqr_cli.main as cli_main
    import hqr_cli.web_server as ws
    ...
    gateway_url = env.get("HQR_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hqr_home, _install_example_plugin):
        ...
        the per-test ``HQR_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_hqr_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("HQR_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under HQR_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("HQR_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/hqr"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/hqr/ds-assets/bg.png)" in prefixed.text
        assert "url(/hqr/fonts-terminal/x.woff2)" in prefixed.text
