...
``hqr pause`` writes a sentinel file at ``$HQR_HOME/ESTOP``;
``hqr resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "HQ Runtime is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.hqr/ESTOP``.
...
def _hqr_home() -> Path:
    """Resolve the active HQR_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active HQR_HOME."""
    return _hqr_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on HQR_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ HQ Runtime is paused ({reason}). New work is on hold; "
            "run `hqr resume` to pick things back up."
        )
    return (
        "⏸️ HQ Runtime is paused. New work is on hold; "
        "run `hqr resume` to pick things back up."
    )
