...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into HQ Runtime with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.hqr/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of HQ Runtime
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``hqr tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``hqr setup`` for
