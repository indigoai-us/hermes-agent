...
thinking phase.  HQ Runtime's default cloud-stream stale detector
(``HQR_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``HQR_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_non_reasoning_model_keeps_default(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("HQR_API_CALL_STALE_TIMEOUT", raising=False)
