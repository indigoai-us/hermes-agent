...
2. An event-loop heartbeat file at ``<HQR_HOME>/state/gateway.heartbeat`` so
...
def _process_hqr_home() -> Path:
    """HQR_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<HQR_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_hqr_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from hqr_logging import drain_log_queue
