...
def _make_cli(**kwargs):
    """Create a HqrCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports hqr_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``HqrCLI.run()``'s local ``KeyBindings`` instance, so they are not
