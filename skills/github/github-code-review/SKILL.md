...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from HQ Runtime Agent\",
    \"comments\": [
...
```bash
source "${HQR_HOME:-$HOME/.hqr}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by HQ Runtime Agent. Code looks clean — good test coverage, no security concerns."
