...
(/opt/hqr/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable HQR_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_hqr_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable HQR_HOME."""
    ...
    monkeypatch.setattr(
        "hqr_constants.get_hqr_home", lambda: hqr_home
    )
    ...
    expected = hqr_home / "scripts" / "whatsapp-bridge"
