...
binary download are exercised manually by `hqr secrets bitwarden
...
def hqr_home(tmp_path, monkeypatch):
    """Point HQ Runtime at an isolated home directory."""
    home = tmp_path / ".hqr"
    home.mkdir()
    monkeypatch.setenv("HQR_HOME", str(home))
    # Some modules cache get_hqr_home; clear if needed.
    import hqr_constants
    if hasattr(hqr_constants, "_HQR_HOME_CACHE"):
        hqr_constants._HQR_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(hqr_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from hqr_cli.env_loader import _apply_external_secret_sources
