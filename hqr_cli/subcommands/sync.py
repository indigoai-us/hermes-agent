"""``hqr sync`` subcommand parser — Skill Sync.

Cloned from ``hqr_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    hqr sync status                 show gate/opt-in/head state
    hqr sync pull                   pull and materialize opted-in skills
    hqr sync push                   push opted-in skills
    hqr sync now                    reconcile: pull then push
    hqr sync enable <skill>         opt a skill into sync
    hqr sync disable <skill>        opt a skill out of sync
    hqr sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    hqr sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  hqr sync status            what is synced, and from where\n"
            "  hqr sync enable my-skill   include a skill in your sync\n"
            "  hqr sync now               pull, then push\n"
            "  hqr sync propose my-skill  share a skill with your team\n"
        ),
