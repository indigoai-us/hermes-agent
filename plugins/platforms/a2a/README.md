# A2A — Agent-to-Agent protocol for HQ Runtime
...
peer (another HQ Runtime, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
hqr gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, HQ Runtime serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.hqr/a2a_audit.jsonl`.
- Conversations persist to `~/.hqr/a2a_conversations/` — they survive context
