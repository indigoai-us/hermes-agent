...
├── cli.py                # Interactive CLI (HqrCLI)
├── hqr_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── hqr_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.hqr/config.yaml` (settings), `~/.hqr/.env` (API keys) — both under `$HQR_HOME` when it is set.
...
`_HQR_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_hqr_home()` for paths,
never hardcode `~/.hqr`. For custom/local-only tools, write a plugin in
`~/.hqr/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `hqr_cli/commands.py`
...
- Tests auto-redirect `HQR_HOME` to temp dirs — never touch real `~/.hqr/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/hqr_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_hqr_home()` from `hqr_constants` for all paths (profile-safe)
