...
it is automatically downloaded from GitHub releases to $HQR_HOME/bin/tirith.
...
def _get_hqr_home() -> str:
    """Return the HQ Runtime home directory, respecting HQR_HOME env var."""
    return str(get_hqr_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_hqr_home(), ".tirith-install-failed")
...
def _hqr_bin_dir() -> str:
    """Return $HQR_HOME/bin, creating it if needed."""
    d = os.path.join(_get_hqr_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HQR_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_hqr_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HQR_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HQR_HOME/bin/tirith
    ...
    hqr_bin = os.path.join(_hqr_bin_dir(), "tirith")
    if os.path.isfile(hqr_bin) and os.access(hqr_bin, os.X_OK):
        _resolved_path = hqr_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return hqr_bin
