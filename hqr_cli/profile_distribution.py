"""Profile distributions — shareable, packaged HQ Runtime profiles via git.

A distribution is a HQ Runtime profile published as a git repository (or
...
* ``hqr profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``hqr skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``hqr profile``, not a parallel tree):

    hqr profile install <source> [--name N] [--alias] [--force] [--yes]
    hqr profile update  <name>  [--force-config] [--yes]
    hqr profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "hqr-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            hqr_requires=str(data.get("hqr_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.hqr_requires:
            out["hqr_requires"] = self.hqr_requires
        if self.author:
...
def check_hqr_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires HQ Runtime {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this HQ Runtime distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``hqr profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a HQ Runtime profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a HQ Runtime distribution."
        )

    # Version check up-front so we fail fast
    check_hqr_requires(manifest.hqr_requires, hqr_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.hqr).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="hqr_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `hqr profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`hqr profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="hqr_dist_update_") as tmp:
        plan = plan_install(
