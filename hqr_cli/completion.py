"""Shell completion script generation for hqr CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_hqr_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# HQ Runtime Agent bash completion
# Add to ~/.bashrc:
#   eval "$(hqr completion bash)"

_hqr_profiles() {{
    local profiles_dir="$HOME/.hqr/profiles"
...
_hqr_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_hqr_profiles)" -- "$cur"))
        return
    fi
...
complete -F _hqr_completion hqr
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef hqr
# HQ Runtime Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(hqr completion zsh)"

_hqr_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.hqr/profiles" ]]; then
        profiles+=($HOME/.hqr/profiles/*(N/:t))
    fi
...
_hqr() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_hqr_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# HQ Runtime Agent fish completion",
        "# Add to your config:",
        "#   hqr completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hqr_profiles",
        "    echo default",
        "    if test -d $HOME/.hqr/profiles",
        "        for d in $HOME/.hqr/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c hqr -f -s p -l profile"
        " -d 'Profile name' -xa '(__hqr_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__hqr_profiles)' -d 'Profile name'"
                )
