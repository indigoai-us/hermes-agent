...
version: 1.0.0
author: HQ Runtime Agent (adapted from gsd-build/get-shit-done)
license: MIT
...
If `gsd-spike` shows up as a sibling skill (installed via `npx get-shit-done-cc --hqr`), prefer **`gsd-spike`** when the user wants the full GSD workflow: persistent `.planning/spikes/` state, MANIFEST tracking across sessions, Given/When/Then verdict format, and commit patterns that integrate with the rest of GSD. This skill is the lightweight standalone version for users who don't have (or don't want) the full system.
...
Use HQ Runtime tools for the research step:
