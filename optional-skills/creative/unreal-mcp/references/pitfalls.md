...
### 1. Start order: editor first, HQ Runtime second

HQ Runtime probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW HQ Runtime session.
...
restart the editor, restart the HQ Runtime session.
...
server fails to bind (Output Log) or HQ Runtime' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.hqr/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for HQ Runtime

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. HQ Runtime' connection
lives in `~/.hqr/config.yaml` via `hqr mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms HQ Runtime — just don't
mistake it for the HQ Runtime setup step.
...
explicitly warns against overlapping calls. HQ Runtime executes same-turn tool
...
### 9. Timeouts: HQ Runtime gives up before Unreal does

HQ Runtime' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If HQ Runtime runs elsewhere (SSH backend,
