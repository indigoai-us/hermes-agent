...
the tool registry or launching HQ Runtime (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``hqr-cli`` to the non-existent ``hqr``. ``resolve_toolset('hqr')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``hqr-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``hqr`` instead of ``hqr-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `hqr tools` to reconfigure."
        )
    return warnings
