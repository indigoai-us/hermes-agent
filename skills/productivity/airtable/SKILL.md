...
4. Store the token in `${HQR_HOME:-~/.hqr}/.env` (or via `hqr setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for HQ Runtime. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for HQ Runtime
...
- **`AIRTABLE_API_KEY` flows from `${HQR_HOME:-~/.hqr}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
