...
- OpenViking server running and reachable from HQ Runtime

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
HQ Runtime can identify older servers that expose the legacy status-only health
...
Then configure HQ Runtime:

```bash
hqr memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into HQ Runtime, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
hqr config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.hqr/.env`; for a named profile use
`~/.hqr/profiles/<profile>/.env`.
...
# OPENVIKING_AGENT=hqr
```

## Config

OpenViking's server config is separate from HQ Runtime:
...
HQ Runtime-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `hqr` | HQ Runtime peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, HQ Runtime lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
HQ Runtime sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
HQ Runtime built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| HQ Runtime action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because HQ Runtime
...
file URIs, such as
`viking://user/peers/hqr/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/hqr/memories/preferences/mem_abc123.md`. Files
