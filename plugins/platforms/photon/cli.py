"""
``hqr photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other HQ Runtime
...
# Writable sidecar runtime dir (mirrors to HQR_HOME on immutable
...
# wiring imports this module for `hqr --help`).
...
# when called later (gateway start, hqr status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `hqr photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'HQ Runtime Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``hqr photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "HQ Runtime Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.hqr/.env,
    ...
    try:
        from hqr_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  hqr gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `hqr photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`hqr photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.hqr/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `hqr photon telemetry on` / `hqr photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.hqr/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  hqr gateway restart")
    return 0
...
#
# `hqr gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``hqr photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `hqr gateway setup` wizard."""
