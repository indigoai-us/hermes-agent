...
Pure logic: walks a HQ Runtime config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `hqr doctor` and a future `hqr migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a HQ Runtime config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a HQ Runtime config and return retirement issues.
...
def apply_migration(
    ...
    from hqr_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `hqr migrate xai` reports nothing to migrate
