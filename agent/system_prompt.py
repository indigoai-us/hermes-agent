...
upstream prefix cache warm.  See ``hqr-agent-dev``'s
...
from hqr_constants import get_default_hqr_root, get_hqr_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- hqr-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``HQR_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("HQR_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the HQR_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from hqr_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from hqr_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound HQR_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  HQ Runtime never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want HQR_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the HQR_HOME ContextVar
    ...
    # Pointer to the hqr-agent skill + docs for user questions about HQ Runtime itself.
    stable_parts.append(HQR_AGENT_HELP_GUIDANCE)
    ...
    # HQR_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base HQ Runtime, any interactive coding surface in a code
    ...
    # `hqr -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['hqr-bots']).
    ...
    # Active-profile hint — names the HQ Runtime profile the agent is running
    # under so it doesn't conflate ~/.hqr/skills/ (default profile) with
    # ~/.hqr/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient HQR_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_hqr_root(): get_hqr_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_hqr_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_hqr_root())
    else:
        _home_str = _root_str = str(get_hqr_home())
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_hqr_root()), which in ambient
        # profile mode is NOT get_hqr_home().
    ...
    if not agent.skip_context_files:
        ...
        # (developing HQ Runtime). Every other surface (desktop chat panel,
    ...
    from hqr_time import get_timezone as _hqr_tz, now as _hqr_now
    now = _hqr_now()
