...
Author widget apps for the HQ Runtime TUI (`hqr --tui`): glanceable ambient
...
- The TUI must be in use (`hqr --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.hqr/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.hqr/logs/tui_gateway_crash.log` if a widget never appears.
