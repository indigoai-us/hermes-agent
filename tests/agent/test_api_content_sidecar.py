...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("hqr_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated HQR_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="hqr_api_content_")
    os.makedirs(os.path.join(test_home, ".hqr"))
    prev_home = os.environ.get("HQR_HOME")
    os.environ["HQR_HOME"] = os.path.join(test_home, ".hqr")
