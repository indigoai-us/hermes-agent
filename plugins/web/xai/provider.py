...
``{title, url, description, position}`` rows every other HQ Runtime web
provider produces.
...
prefers HQ Runtime-managed xAI Grok OAuth (via ``hqr auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.hqr/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``hqr tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `hqr auth` to sign in with "
