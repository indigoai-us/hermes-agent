...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.hqr/hqr-agent/utils.py)
...
2. ``hqr update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``hqr_cli/main.py`` (the ``hqr update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``hqr_constants`` after the
