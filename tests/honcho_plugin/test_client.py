...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "hqr"
        assert config.workspace_id == "hqr"
...
class TestFromGlobalConfig:
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"hqr": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="hqr-agent"
        ):
            result = config.resolve_session_name("/home/user/hqr-agent/subdir")
        assert result == "hqr-agent"


class TestResolveConfigPath:
    def test_prefers_hqr_home_when_exists(self, tmp_path):
        hqr_home = tmp_path / "hqr"
        hqr_home.mkdir()
        local_cfg = hqr_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: HQR_HOME points at ~/.hqr/profiles/<name>, so
        # _get_default_hqr_home() must resolve back to ~/.hqr — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".hqr"
        ...
        assert _get_default_hqr_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "hqr_coder"
        assert profile_host_key("default") == "hqr"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HQR_HONCHO_HOST": "hqr.coder"}):
            assert resolve_active_host() == "hqr.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("HQR_HONCHO_HOST", None)
            # Temporarily remove hqr_cli.profiles to simulate import failure
            saved = sys.modules.get("hqr_cli.profiles")
            sys.modules["hqr_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "hqr"
            finally:
                if saved is not None:
                    sys.modules["hqr_cli.profiles"] = saved
                else:
                    sys.modules.pop("hqr_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="hqr_coder")
        assert config.host == "hqr_coder"
        assert config.workspace_id == "hqr"  # shared workspace
        assert config.ai_peer == "hqr_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"hqr": {"enabled": True, "aiPeer": "hqr"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (hqr.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="hqr_work",
            workspace_id="hqr",
            raw={"hosts": {"hqr.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_hqr_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HQR_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("hqr_cli.profiles.get_active_profile_name", return_value="default"), \
