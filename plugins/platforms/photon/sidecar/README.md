...
Small Node helper that bridges HQ Runtime Agent to Photon's Spectrum SDK
(`spectrum-ts`).  HQ Runtime is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Hqr-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and HQ Runtime can receive inbound messages
...
The HQ Runtime plugin's `hqr photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. HQ Runtime is Python, so replies go through this sidecar
