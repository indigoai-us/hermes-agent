"""
Gateway subcommand for hqr CLI.

Handles: hqr gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_hqr_home is imported lazily at call sites to avoid ImportError
# when hqr_constants is cached from a pre-update version during `hqr update`.
...
from hqr_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "hqr CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `hqr update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``hqr gateway status`` from falsely counting the ``hqr`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``hqr gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_hqr_home().resolve())
    # Forward slashes on both sides of the HQR_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"hqr_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. HQR_HOME
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "hqr_home=" in command_lc
            and f"hqr_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``hqr update`` hung exactly there
            ...
            # watchdog probe.
            from hqr_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``hqr update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            HQ Runtime profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to HQ Runtime profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from hqr_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "hqr_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m hqr_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``hqr update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting HQ Runtime's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose HQR_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``hqr update`` users
    # with no running gateway until they re-invoke ``hqr gateway``
    ...
    from hqr_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (HQR_HOME,
    ...
    if sys.platform == "win32":
        try:
            from hqr_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / HQR_HOME) the windowless
        # base interpreter needs to import hqr_cli.  Empty on POSIX, where
...
def _hqr_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``HQR_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("HQR_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_hqr_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HQR_HOME``.

    Under ``sudo``, ``HQR_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hqr_home` falls back to ``/root/.hqr`` — the wrong
    profile. The unit file pins ``HQR_HOME`` for the actual gateway
    ...
    unit_home = (_hqr_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("HQR_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("HQR_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["HQR_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}hqr gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}hqr gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from hqr_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: hqr gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `hqr gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``hqr gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from hqr_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (hqr_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Hqr_Gateway /
            # Hqr_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `hqr gateway install` deployment.
            from hqr_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Hqr_Gateway"
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (HQR_HOME-scoped).
...
from hqr_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our HQ Runtime Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``hqr gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``HQR_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("HQR_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from hqr_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "hqr-gateway"
SERVICE_DESCRIPTION = "HQ Runtime Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HQR_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (HQR_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from hqr_constants import get_default_hqr_root

    home = get_hqr_home().resolve()
    default = get_default_hqr_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary HQR_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hqr_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when HQR_HOME is a named profile.

    For ``~/.hqr/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hqr_home: Optional explicit HQR_HOME path. Defaults to the current
            ``get_hqr_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional HQ Runtime root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_hqr_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from hqr_constants import get_default_hqr_root

    home = Path(hqr_home or str(get_hqr_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_hqr_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(hqr_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".hqr"
    try:
        Path(hqr_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(hqr_home, default_root=target_root)
    except ValueError:
        return _profile_arg(hqr_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this HQR_HOME.

    Default ``~/.hqr`` returns ``hqr-gateway`` (backward compatible).
    Profile ``~/.hqr/profiles/coder`` returns ``hqr-gateway-coder``.
    Any other HQR_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older HQ Runtime installs that predate the
# hqr-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (hqr-gateway-*.service) and unrelated third-party
# "hqr" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("hqr.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "hqr_cli.main gateway",
    "hqr_cli/main.py gateway",
    "gateway/run.py",
    " hqr gateway ",
    "/hqr gateway ",
)
...
def _find_legacy_hqr_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy HQ Runtime gateway units.

    Detects unit files installed by older HQ Runtime versions that used a
    different service name (e.g. ``hqr.service`` before the rename to
    ``hqr-gateway.service``). When both a legacy unit and the current
    ``hqr-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``hqr-gateway-coder.service`` and unrelated third-party
      ``hqr-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``hqr.service`` running an unrelated
...
def has_legacy_hqr_units() -> bool:
    """Return True when any legacy HQ Runtime gateway unit files exist."""
    return bool(_find_legacy_hqr_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy HQ Runtime gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_hqr_units()
    if not legacy:
        return
    print_warning("Legacy HQ Runtime gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current hqr-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    hqr gateway migrate-legacy")


def remove_legacy_hqr_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy HQ Runtime gateway unit files.

    Iterates over whatever ``_find_legacy_hqr_units()`` returns — which is
    ...
    legacy = _find_legacy_hqr_units()
    if not legacy:
        print("No legacy HQ Runtime gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy HQ Runtime gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: hqr gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo hqr gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    hqr gateway uninstall")
    print_info("    sudo hqr gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: hqr gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``hqr setup`` (end of wizard)
    and ``hqr import``: if this host supports a service manager and no
    ...
    ``hqr gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   hqr gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: hqr gateway")
        return False
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from hqr_cli import gateway_windows
        ...
        print_info("  You can retry manually: hqr gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode HQ Runtime often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.hqr`` → ``ai.hqr.gateway.plist`` (backward compatible).
    Profile ``~/.hqr/profiles/coder`` → ``ai.hqr.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.hqr.gateway-{suffix}" if suffix else "ai.hqr.gateway"
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # hqr_constants cached from before this symbol existed. See
            # _reload_hqr_constants() in hqr_cli/managed_uv.py.
            from hqr_cli.managed_uv import _reload_hqr_constants

            venv_python_path = _reload_hqr_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hqr/hqr-agent  -> /home/alice/.hqr/hqr-agent
      /opt/hqr                 -> /opt/hqr  (kept as-is)
...
def _hqr_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HQR_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hqr_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hqr                    → /home/alice/.hqr
      /root/.hqr/profiles/coder     → /home/alice/.hqr/profiles/coder
      /opt/custom-hqr               → /opt/custom-hqr  (kept as-is)
    """
    current_hqr_raw = os.environ.get("HQR_HOME", "").strip()
    current_hqr = (
        Path(current_hqr_raw).expanduser()
        if current_hqr_raw
        else get_hqr_home()
    )
    ...
    # different HQR_HOME into the generated service unit.
    current_default = Path.home() / ".hqr"
    target_default = Path(target_home_dir) / ".hqr"

    # Default ~/.hqr → remap to target user's default
    if current_hqr == current_default:
        return str(target_default)

    # Profile or subdir of ~/.hqr → preserve the relative structure
    try:
        relative = current_hqr.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.hqr) — keep as-is
        return str(current_hqr)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    hqr_node = hqr_home / "node" / "bin"
    if _is_dir(hqr_node):
        candidates.append(str(hqr_node))
    hqr_nm = hqr_home / "node_modules" / ".bin"
    if _is_dir(hqr_nm):
        candidates.append(str(hqr_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m hqr_cli.main``, so module
    ...
    dir, or a clone that ``hqr update`` later relocates/removes — the path
    ...
    ``HQR_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if HQR_HOME
...
def _systemd_watchdog_seconds(hqr_home: str | Path | None = None) -> int:
    ...
    if hqr_home is not None:
        ...
        override_token = set_hqr_home_override(hqr_home)
        reset_home_override = reset_hqr_home_override
    try:
...
def _systemd_watchdog_service_fields(
    hqr_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(hqr_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], hqr_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The HQ Runtime-managed Node under ``$HQR_HOME/node`` goes first when it
    ...
    *hqr_root* is the HQ Runtime home the unit will run against. System units
    ...
    """
    from hqr_constants import (
        hqr_managed_node_tree_present,
        iter_hqr_node_dirs,
    )

    managed_node_present = hqr_managed_node_tree_present(hqr_root)
    for directory in iter_hqr_node_dirs(hqr_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target HQ Runtime home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's HQ Runtime home is known — probing here would stat the calling
    ...
    if system:
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(hqr_home) if hqr_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m hqr_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    hqr_home = str(get_hqr_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        hqr_home
    )
    profile_arg = _profile_arg(hqr_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m hqr_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="HQR_HOME={hqr_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned HQR_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo hqr gateway … --system``, HQR_HOME is often stripped
    # and falls back to ``/root/.hqr``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / HQR_HOME comparisons
    ...
    _sync_hqr_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir HQR_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved HQR_HOME
    ...
    exported a throwaway ``HQR_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``hqr gateway restart``
    with ``HQR_HOME=/tmp/hqr-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="HQR_HOME=..."`` lines and launchd
    ``<key>HQR_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'HQR_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>HQR_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp HQR_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: HQR_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported HQR_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single HQR_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only HQR_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp HQR_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hqr_test`` path into
    # ``Environment="HQR_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/hqr_test"' in new_unit
        or "/hqr_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # HQR_HOME into the unit (manual E2E homes like /tmp/hqr-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current HQ Runtime install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo hqr gateway uninstall --system")
    print_info("         hqr gateway install")
    print_info("         hqr gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("HQR_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("HQR_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (hqr.service from pre-rename installs)
    # before installing the new hqr-gateway.service. If both remain, they
    ...
    if has_legacy_hqr_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_hqr_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin HQR_HOME; adopt it before any
    ...
    # ``sudo hqr gateway install --system --force`` would bake /root/.hqr
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}hqr gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}hqr gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}hqr gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # HQR_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `hqr gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `hqr gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}hqr gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}hqr gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}hqr gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}hqr gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.hqr.gateway-{suffix}" if suffix else "ai.hqr.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``hqr gateway start/stop/status`` call).
...
#      background process (the `nohup hqr gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_hqr_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_hqr_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m hqr_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active HQR_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "hqr_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``hqr update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup hqr gateway run --replace`
    ...
    working.
    """
    from hqr_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_hqr_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from hqr_constants import display_hqr_home as _dhh
    ...
    print(
        f"  Try manually: nohup hqr gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `hqr gateway start`)
    ...
    nofile_block = ""
    try:
        from hqr_cli.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.hqr/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See hqr-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_hqr_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current HQ Runtime install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  hqr gateway status             # Check status")
    from hqr_constants import display_hqr_home as _dhh
...
def launchd_stop():
    ...
    # `hqr gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `hqr update`
...
def launchd_status(deep: bool = False):
    ...
    # HQ Runtime PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the HQ Runtime PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current HQ Runtime install")
    else:
        print("⚠ Service definition is stale relative to the current HQ Runtime install")
        print("  Run: hqr gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: hqr gateway stop")
            else:
    ...
    if deep:
        log_file = get_hqr_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``HQR_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``hqr gateway run`` is always a misconfiguration, so we
    ...
    try:
        from hqr_constants import get_default_hqr_root
        default_root = get_default_hqr_root()
    ...
    try:
        ...
        from hqr_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``hqr gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``hqr gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other HQR_HOME roots.
    """
    ...
    print("  Use 'hqr gateway restart' to replace it,")
    print("  or 'hqr gateway stop' first.")
    print("  Or use 'hqr gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("HQR_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the HQ Runtime gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hqr' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hqr/docker/entrypoint.sh before the HQ Runtime command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$HQR_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set HQR_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `hqr gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set HQR_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("HQR_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `hqr gateway restart` which already
    ...
    # `hqr gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ HQ Runtime Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # HQR_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HQR_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``HQR_GATEWAY_MAX_STARTS`` /
    # ``HQR_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("HQR_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("HQR_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``hqr gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. hqr) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where HQ Runtime delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. HQ Runtime connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: hqr pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and HQ Runtime will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``hqr setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run hqr under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.hqr/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_hqr_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from hqr_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from hqr_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'hqr pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: hqr pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'hqr gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. HQ Runtime will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. HQ Runtime will store the returned account_id/token in ~/.hqr/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `hqr gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_hqr_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `hqr pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `hqr pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "HqrAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``hqr_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from hqr_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.hqr/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.hqr/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_hqr_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: hqr gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: hqr gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: hqr gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo hqr gateway install --system"
                        )
                    print_info("  Or run in foreground:  hqr gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: hqr gateway run")
                print_info(
                    "  For persistence:   tmux new -s hqr 'hqr gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: hqr gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup hqr gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'hqr gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`hqr_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from hqr_cli.service_manager import (
    ...
    if profile is None:
        ...
        # HQR_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``hqr gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``hqr gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``hqr gateway run`` are unaffected.
      2. ``HQR_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``hqr gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``HQR_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("HQR_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("HQR_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${HQR_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if HQR_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or HQR_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-hqr service's
    # pattern in docker/s6-rc.d/main-hqr/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: hqr gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: hqr gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hqr 'hqr gateway run'"
                )
                print()
            ...
            print()
            print(
                "  hqr gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hqr 'hqr gateway run'         # persistent via tmux"
            )
            print(
                "  nohup hqr gateway run > ~/.hqr/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from hqr_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  hqr profile create <name>     # creates the s6 service slot")
                print("  hqr -p <name> gateway start   # bring it up via s6")
                print("  hqr status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: hqr gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: hqr gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: hqr gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  hqr profile delete <name>     # tears down the s6 service slot")
                print("  hqr -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `hqr -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: hqr gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HQR_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `hqr gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HQR_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `hqr gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: hqr gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  hqr gateway install")
                else:
                    print("To install as a service:")
                    print("  hqr gateway install")
                    print("  sudo hqr gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  hqr gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup hqr gateway run > ~/.hqr/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hqr 'hqr gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  hqr gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  hqr gateway install  # Install as user service")
                    print(
                        "  sudo hqr gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy HQ Runtime gateway unit files from
        # pre-rename installs (e.g. hqr.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_hqr_units(interactive=not yes, dry_run=dry_run)
