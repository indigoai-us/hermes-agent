...
- Bare `hqr setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `hqr setup --quick` runs the narrower "fill in missing items" flow.
- `hqr setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("hqr_cli.setup.ensure_hqr_home", {}),
        ("hqr_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("hqr_cli.config.is_managed", {"return_value": False}),
        ("hqr_cli.setup.load_config", {"return_value": {}}),
        ("hqr_cli.setup.save_config", {}),
        ("hqr_cli.setup.get_env_value", {"return_value": None}),
        ("hqr_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("hqr_cli.setup._print_setup_summary", {}),
        ("hqr_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("hqr_cli.setup.load_config", {"return_value": {}}),
        ("hqr_cli.setup.save_config", {}),
        ("hqr_cli.auth.get_active_provider", {"return_value": None}),
        ("hqr_cli.setup.get_env_value", {"return_value": None}),
        ("hqr_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `hqr setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="hqr_cli.setup.prompt_choice",
                quick="hqr_cli.setup._run_quick_setup",
                model="hqr_cli.setup.setup_model_provider",
                terminal="hqr_cli.setup.setup_terminal_backend",
                agent="hqr_cli.setup.setup_agent_settings",
                gateway="hqr_cli.setup.setup_gateway",
                tools="hqr_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("hqr_cli.setup.prompt_choice", {"return_value": 0}),
                first="hqr_cli.setup._run_first_time_quick_setup",
            )
            from hqr_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "hqr_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["hqr", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
