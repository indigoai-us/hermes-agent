...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *HQ Runtime Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to HQR_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/hqr), this function
    mirrors the bridge source to a writable HQR_HOME location and returns that
    ...
    # Try HQR_HOME location first
    hqr_home = get_hqr_home()
    hqr_home_bridge = hqr_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to HQR_HOME if needed
    if hqr_home_bridge.exists():
        return hqr_home_bridge

    # Mirror the bridge source to HQR_HOME
    try:
        hqr_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            hqr_home_bridge,
            dirs_exist_ok=False,
