"""Tests for SIGHUP protection and stdout mirroring in ``hqr update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``hqr_cli/main.py``.  These exist so
that ``hqr update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("HQR_ACTION_ID", "a" * 32)

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== hqr-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("HQR_ACTION_ID", "not-safe\nforged")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import hqr_cli.config as _cfg
        if hasattr(_cfg, "_HQR_HOME_CACHE"):
            _cfg._HQR_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "hqr update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_hqr_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "hqr_cli.config.get_hqr_home", _boom, raising=True
        )
