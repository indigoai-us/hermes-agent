...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `hqr backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - hqr_home (str): The active HQR_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.hqr``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "hqr").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'hqr memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], hqr_home: str) -> None:
        ...
        Called by 'hqr memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``hqr_home`` is the active HQR_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE HQR_HOME.

        ``hqr backup`` only walks HQR_HOME, so any provider state kept
        ...
        of the archive, and ``hqr import`` restores them to their original
