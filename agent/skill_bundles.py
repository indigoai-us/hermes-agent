...
Storage
-------
Bundles live in ``~/.hqr/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``HQR_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``hqr bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under HQR_HOME.

    Honors ``HQR_BUNDLES_DIR`` for tests; falls back to
    ``<HQR_HOME>/skill-bundles``.
    """
    override = os.environ.get("HQR_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_hqr_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `hqr bundles` CLI subcommand.
# ---------------------------------------------------------------------------
