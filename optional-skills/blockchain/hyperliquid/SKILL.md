...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), HQ Runtime Agent
license: MIT
...
The script reads `${HQR_HOME:-~/.hqr}/.env` for two optional defaults:
...
Helper script: `~/.hqr/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.hqr/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${HQR_HOME:-~/.hqr}/.env`.
...
```bash
python ~/.hqr/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.hqr/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
