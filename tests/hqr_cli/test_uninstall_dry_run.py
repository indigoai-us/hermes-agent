...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_hqr_home", lambda: hqr_home)
    monkeypatch.setattr(uninstall, "_is_default_hqr_home", lambda home: False)
    ...
    assert str(hqr_home) in output
    assert project_root.exists()
    assert hqr_home.exists()
