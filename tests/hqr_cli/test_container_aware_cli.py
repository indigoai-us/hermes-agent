...
def container_env(tmp_path, monkeypatch):
    """Set up a fake HQR_HOME with .container-mode file."""
    hqr_home = tmp_path / ".hqr"
    hqr_home.mkdir()
    monkeypatch.setenv("HQR_HOME", str(hqr_home))
    monkeypatch.delenv("HQR_DEV", raising=False)

    container_mode = hqr_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=hqr-agent\n"
        "exec_user=hqr\n"
        "hqr_bin=/data/current-package/bin/hqr\n"
    )
    return hqr_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("hqr_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "hqr-agent"
    assert info["exec_user"] == "hqr"
    assert info["hqr_bin"] == "/data/current-package/bin/hqr"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from hqr_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "hqr"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "hqr-agent" in cmd
    assert "/data/current-package/bin/hqr" in cmd
    assert "chat" in cmd
