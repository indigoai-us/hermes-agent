...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. HQ Runtime' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
