...
Extracted verbatim from hqr_state.py so the SessionDB mixin modules
(hqr_state_search / hqr_state_schema / hqr_state_portability) can
reference them without importing hqr_state (which would be a cycle).
hqr_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``hqr sessions optimize-storage``. A legacy DB sits at
...
# `hqr sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `hqr db optimize`. This is
