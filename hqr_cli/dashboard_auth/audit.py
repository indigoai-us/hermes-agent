...
Profile-aware location: ``$HQR_HOME/logs/dashboard-auth.log``.
...
from ``hqr_constants`` or other hqr_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$HQR_HOME/logs/dashboard-auth.log``.

    Uses ``hqr_constants.get_hqr_home()`` (a leaf module — no import
    cycle) so profile overrides and the native-Windows ``%LOCALAPPDATA%``
    fallback are honored.
    """
    from hqr_constants import get_hqr_home

    return get_hqr_home() / "logs" / "dashboard-auth.log"
