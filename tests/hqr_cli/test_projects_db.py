"""Tests for the per-profile Projects store (hqr_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="HQ Runtime Agent", folders=["/tmp/hqr"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "hqr-agent"
    assert proj.name == "HQ Runtime Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/hqr"
    assert [f.path for f in proj.folders] == ["/tmp/hqr"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "hqr-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' HQR_HOME.
