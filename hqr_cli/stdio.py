...
on Japanese locales, etc.).  HQ Runtime's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``hqr_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. HQ Runtime is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``HQR_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("HQR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the HQ Runtime-managed Git install directories so
    ...
    # already-running shells don't see the change — which means hqr
    ...
    # install without HQ Runtime-managed PortableGit).
    ...
    # stdin is re-configured for completeness; HQ Runtime's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and HQ Runtime's
    ``hqr config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching HQ Runtime (User env var in Windows Settings, or
    export in a PowerShell profile) and HQ Runtime picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known HQ Runtime-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\hqr\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``hqr`` from right after install) retain their old PATH.

    Any subprocess HQ Runtime spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known HQ Runtime-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "hqr", "git", "cmd"),
        os.path.join(local_appdata, "hqr", "git", "bin"),
        os.path.join(local_appdata, "hqr", "git", "usr", "bin"),
        # HQ Runtime venv Scripts directory — host of the hqr.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes hqr, but harmless to include.
        os.path.join(local_appdata, "hqr", "hqr-agent", "venv", "Scripts"),
