...
<url>``) is spawned as a direct child of the HQ Runtime process. HQ Runtime's own
...
HQ Runtime process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next HQ Runtime startup's opt-in
