    ...
    with patch.dict(
        "sys.modules",
        {
            "hqr_constants": MagicMock(
                get_hqr_home=MagicMock(return_value="/tmp/hqr_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'hqr-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'hqr-release' patched"
    }
