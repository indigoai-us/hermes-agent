...
Three cookies in play:
  ...
  - hqr_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-hqr_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "hqr_session_at"
SESSION_RT_COOKIE = "hqr_session_rt"
...
SESSION_PROVIDER_COOKIE = "hqr_session_provider"
PKCE_COOKIE = "hqr_session_pkce"
...
SSO_ATTEMPT_COOKIE = "hqr_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /hqr`` we want ``Path=/hqr`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (hqr #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/hqr``)
