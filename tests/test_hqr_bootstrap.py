"""Tests for hqr_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every HQ Runtime entry point
(hqr, hqr-agent, hqr-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every HQ Runtime entry point imports hqr_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported hqr_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("hqr_bootstrap", None)
    import hqr_bootstrap  # noqa: WPS433
    return hqr_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the HQ Runtime CLI
...
class TestEntryPointsImportBootstrap:
    """Every HQ Runtime entry point must import hqr_bootstrap as its
    ...
    # Entry points that invoke HQ Runtime as a process.  Each one must
    # import hqr_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "hqr_cli/main.py",   # hqr CLI (console_script)
        "run_agent.py",          # hqr-agent (console_script)
        "acp_adapter/entry.py",  # hqr-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import hqr_bootstrap' and that
        ...
        half-finished ``hqr update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hqr_bootstrap`` as a top-level module) leaves hqr
        ...
        """
        # Resolve relative to the hqr-agent repo root.  Tests live
        # at tests/test_hqr_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hqr start even when hqr_bootstrap hasn't been
        ...
        assert first_import_name == "hqr_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'hqr_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import hqr_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing HQ Runtime's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("HQR_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["HQR_PYTHON_SRC_ROOT"] = env
            elif "HQR_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["HQR_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hqr")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HQR_PYTHON_SRC_ROOT", None)
            else:
                os.environ["HQR_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/hqr", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # HQ Runtime root.  The guard must relocate HQ Runtime to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/hqr"])
        assert result[0] == "/opt/hqr"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the HQ Runtime root.
        assert result.index("/opt/hqr") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HQR_PYTHON_SRC_ROOT"] = "/env/hqr"
            hb.harden_import_path()
            assert sys.path[0] == "/env/hqr"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HQR_PYTHON_SRC_ROOT", None)
