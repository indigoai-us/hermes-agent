"""Install and remove the Linux desktop entry (``hqr.desktop``).

``hqr desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "hqr.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``hqr.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``hqr desktop``.

    Prefer the real ``hqr`` executable (argv[0] or PATH). When HQ Runtime
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from hqr_cli.relaunch import resolve_hqr_bin

    bin_path = resolve_hqr_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "hqr_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=HQ Runtime\n"
        "GenericName=HQ Runtime Desktop\n"
        "Comment=Launch HQ Runtime Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=HQ Runtime\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the HQ Runtime desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "hqr"
