...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``hqr logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # HQ Runtime only initiates compaction in "hqr" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping HQ Runtime preflight compression for codex app-server "
                "(mode=%s); HQ Runtime will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by HQ Runtime (via _emit_status), so it always
