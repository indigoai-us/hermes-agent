...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with hqr-agent)
2. User plugins: ``$HQR_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``hqr_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HQR_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HQR_HOME/plugins/model-providers/`` if it exists."""
    try:
        from hqr_constants import get_hqr_home

        d = get_hqr_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HQR_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_hqr_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``hqr_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."hqr_agent.plugins"]
        acme-inference = "acme_hqr_plugin:register"
    ...
    * **Provider targets only.** The ``hqr_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$HQR_HOME``) keep their
    ...
    try:
        from hqr_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HQR_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``hqr_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$HQR_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with hqr-agent.
    ...
    # 2. User plugins — under $HQR_HOME/plugins/model-providers/<name>/.
