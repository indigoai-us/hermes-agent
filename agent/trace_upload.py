"""Upload a HQ Runtime session transcript to Hugging Face as an agent trace.

HQ Runtime stores sessions in its own SQLite store (``hqr_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``hqr trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through HQ Runtime' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "hqr-traces"
_HQR_VERSION = "hqr-agent"
...
# ---------------------------------------------------------------------------
# Conversion: HQ Runtime OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses HQ Runtime' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render HQ Runtime conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.hqr/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `hqr trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/hqr-traces`` dataset. Returns a
