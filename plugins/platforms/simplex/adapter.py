"""SimpleX Chat platform adapter (HQ Runtime plugin).
...
This adapter ships as a HQ Runtime platform plugin under
``plugins/platforms/simplex/``. The HQ Runtime plugin loader scans the
...
discoverable and ``hqr setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside HQ Runtime — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("HQR_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hqr setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.hqr/.env`` via
    ``hqr_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "hqr_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.hqr/.env"
        )
        return
