...
# xAI's public API is OpenAI-compatible for the endpoints HQ Runtime commonly
# uses. The Responses endpoint is included because HQ Runtime' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via HQ Runtime-managed OAuth credentials."""

    auth_hint = "hqr auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`hqr auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`hqr auth reset xai-oauth` or re-authenticate with "
                    "`hqr auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `hqr auth add xai-oauth --type oauth`."
            )
