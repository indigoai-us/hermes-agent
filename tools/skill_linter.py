...
Design contract (matches the HQ Runtime "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    hqr_meta = meta.get("hqr") if isinstance(meta, dict) else None
    if not isinstance(hqr_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.hqr.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in hqr_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.hqr.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("hqr", "agent", "hqr agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'HQ Runtime Agent' (proper caps) "
