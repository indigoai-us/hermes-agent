"""``hqr slack ...`` CLI subcommands.

Today only ``hqr slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ hqr slack manifest > slack-manifest.json
    # or:
    $ hqr slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of HQ Runtime. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a HQ Runtime deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps HQ Runtime on Slack's older Assistant messaging
    ...
    """
    from hqr_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with HQ Runtime in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with HQ Runtime in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your HQ Runtime agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hqr_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$HQR_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "HQ Runtime")
    ...
    """
    name = getattr(args, "name", None) or "HQ Runtime"
    description = getattr(args, "description", None) or "Your HQ Runtime agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "hqr slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"hqr slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "hqr slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "hqr slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your HQ Runtime app\n"
