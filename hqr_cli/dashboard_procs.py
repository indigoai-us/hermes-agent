"""Dashboard process-hygiene helpers — extracted from ``hqr_cli/main.py``.
...
``_detect_concurrent_hqr_instances``) are lifted verbatim. References to
helpers that STAY in ``hqr_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``hqr_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``hqr_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``hqr_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``hqr dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``hqr update`` replaces files on disk, the running
    ...
    ``hqr-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the HQ Runtime Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``hqr serve`` as
    ...
    environment variable ``HQR_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`hqr serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "hqr.*dashboard"`) keeps us consistent
            # with `hqr_cli.gateway._scan_gateway_pids` and avoids the
...
def _hqr_home_for_pid(pid: int) -> str | None:
    """Best-effort ``HQR_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("HQR_HOME")
        if home:
            return home
    except Exception:
        pass
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by HQ Runtime Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``hqr update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], hqr_home: str | None = None
) -> str:
    """Stable owner key: ``HQR_HOME`` when known, else ``--profile`` / ``-p``.

    ``HQR_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.hqr`` roots) keep a resolved ``home:`` key so
    ...
    if hqr_home:
        try:
            home_path = Path(hqr_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(hqr_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(hqr_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``hqr update``.

    Each candidate is ``(pid, argv, hqr_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``HQR_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``HQR_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``hqr update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, hqr_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, hqr_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``hqr dashboard`` / ``hqr serve`` processes.

    Called at the end of ``hqr update`` (default ``reason``) and also
    from ``hqr dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``hqr update`` path), a detected
    ``hqr-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``hqr-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``hqr update``'s systemd
    fleet-restart loop, which restarts ``hqr-serve*`` units before this
    ...
    # When the HQ Runtime Desktop Electron app spawns this dashboard as a
    # backend child, it sets HQR_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("HQR_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `hqr update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot HQR_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _hqr_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. hqr update's
    ...
    #    remote backend (hqr serve) under Restart=on-failure never comes
...
def _detect_concurrent_hqr_instances(
    ...
    The HQ Runtime Desktop Electron app spawns ``hqr.EXE`` as a backend child,
    so during ``hqr update`` the user-invoked process and the desktop's
    ...
    shims (``hqr.exe`` / ``hqr-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``hqr update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``hqr.exe``) is a separate process from the Python
    ...
    for shim in _m()._hqr_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated hqr.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``hqr update``
    ...
    #      hqr.exe sitting *under* a non-HQ Runtime parent (e.g. a HQ Runtime
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        hqr serve --host 127.0.0.1 --port 0
        hqr serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "hqr" not in cmd and "hqr_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (HQR_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("HQR_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``hqr serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``hqr serve`` on a Mac
...
def _hqr_home_dir() -> Path:
    """Resolved HQ Runtime home (HQR_HOME override or ~/.hqr)."""
    override = os.environ.get("HQR_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    # only check the suffix so a relocated HQR_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{hqr_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _hqr_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local hqr serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``hqr serve`` backends with no parent.
    ...
    running under HQR_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never HQR_DESKTOP_CHILD_PID entries
