# Themes / Skins — Author a HQ Runtime Color Theme

Author a HQ Runtime **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`hqr_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.hqr/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for HQ Runtime itself.
...
- Write access to the HQ Runtime home dir — `~/.hqr` by default, or `$HQR_HOME`
  / the active profile's dir. Skins live in `<hqr-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `hqr config set`).
...
3. `write_file` it to `<hqr-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `hqr config set display.skin default` (or they can `/skin default`).
...
```
hqr skin set <key> <hex>      # e.g. hqr skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `hqr skin set background "#08201f"` changes
only the background; `hqr skin use <name>` / `hqr skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.hqr`** when a profile is active — resolve the real home
  from `$HQR_HOME` first, falling back to `~/.hqr`.
...
- **Never hand-edit `config.yaml` to activate.** Use `hqr config set
...
- **You apply it, not the user.** `hqr config set display.skin <name>` is
...
- `read_file` the written `<hqr-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `hqr config get display.skin` and confirm it reports `<name>`.
