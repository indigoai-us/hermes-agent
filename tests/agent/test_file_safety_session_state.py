...
manipulation. HQ Runtime keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point HQR_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_hqr_home /
    get_default_hqr_root) instead of monkeypatching private helpers —
    ...
    HQR_HOME=<root>/profiles/<name> makes get_default_hqr_root()
