...
- Hosted: API key from [app.supermemory.ai/integrations?connect=hqr](http://app.supermemory.ai/integrations?connect=hqr)
...
```bash
hqr memory setup    # select "supermemory"
```

Or manually:

```bash
hqr config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.hqr/.env
```
...
Before running `hqr memory setup`, add the local endpoint to
`$HQR_HOME/supermemory.json`:
...
Then run `hqr memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$HQR_HOME/supermemory.json`
...
| `container_tag` | `hqr` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hqr-{identity}` → `hqr-coder`). |
...
`https://api.supermemory.ai`. HQ Runtime resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: hqr`, and document writes stamp
`metadata.sm_source: hqr`. This is a **functional routing key, not telemetry**:
it groups HQ Runtime-written memories into a dedicated "HQ Runtime" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per HQ Runtime profile:

```json
{
  "container_tag": "hqr-{identity}"
}
```

For a profile named `coder`, this resolves to `hqr-coder`. The default profile resolves to `hqr-default`. Without `{identity}`, all profiles share the same container.
