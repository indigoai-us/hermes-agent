...
from hqr_constants import get_process_hqr_home
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory HQ Runtime was already launched from (for example ``hqr-agent``
    while the process cwd is ``~/.hqr/hqr-agent``), passing it through
    unchanged makes the wrapper run ``cd hqr-agent`` *inside* the project
    ...
    # Common recovery for config values like ``hqr-agent`` when HQ Runtime was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./hqr-agent``; use the current directory instead.
...
# HQ Runtime-internal env vars that should NOT leak into terminal subprocesses.
_HQR_PROVIDER_ENV_FORCE_PREFIX = "_HQR_FORCE_"

# HQ Runtime-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a HQ Runtime inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from hqr_cli.config import OPTIONAL_ENV_VARS
    ...
    # HQ Runtime-managed inference credential — Claude subscription auth is not a
    # working HQ Runtime provider path.  Stripping it broke agent-spawned
...
_HQR_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the HQ Runtime venv path instead of the project's own
# ``.venv`` — silently clobbering the HQ Runtime environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# HQ Runtime venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the HQ Runtime venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). HQ Runtime itself treats PYTHONHOME as contamination in its own
...
# _strip_hqr_owned_pythonpath() which removes only HQ Runtime-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hqr_internal_secret(key: str) -> bool:
    """Return True for HQ Runtime-internal secrets injected under *dynamic* names.

    ``_HQR_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "HQ Runtime-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`hqr_subprocess_env` helper all call it, so the
...
def _inject_context_hqr_home(env: dict) -> None:
    """Bridge the context-local HQ Runtime home override into subprocess env."""
    try:
        from hqr_constants import get_hqr_home_override

        value = get_hqr_home_override()
        if value:
            env["HQR_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (HQR_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter HQ Runtime-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_HQR_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_hqr_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _HQR_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_HQR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_HQR_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_hqr_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_hqr_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _HQR_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_hqr_home(sanitized)

    from hqr_constants import apply_subprocess_home_env
    ...
    # the separate HQ Runtime runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_hqr_owned_pythonpath_and_runtime_markers(sanitized)
...
# legitimate child HQ Runtime spawns needs them, and they are the highest-value
...
# narrow subset of _HQR_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in hqr_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_hqr_internal_secret, but _ID has no secret suffix, so it must be
...
def hqr_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_HQR_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child HQ Runtime spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HQR_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and HQ Runtime-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_hqr_internal_secret`.
    for key in list(env):
        if key.startswith(_HQR_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_hqr_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _HQR_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_hqr_home(env)
    from hqr_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_hqr_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose HQR_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's HQR_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`hqr_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``HQR_HOME`` / subprocess ``HOME`` contract) and the
    HQ Runtime secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_hqr_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``HQR_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local HQ Runtime home override into ``HQR_HOME`` and apply the
      subprocess HOME contract (``hqr_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``HQR_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs HQR_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("HQR_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale HQR_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\hqr\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\hqr\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hqr_portable_git = os.path.join(_local_appdata, "hqr", "git") if _local_appdata else ""
    if _hqr_portable_git:
        for candidate in (
            os.path.join(_hqr_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_hqr_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # HQR_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\hqr\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "HQR_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. HQ Runtime Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set HQR_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart HQ Runtime. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``hqr`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_HQR_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_hqr_bin_dir() -> str | None:
    """Return the directory holding the ``hqr`` console-script, or None.
    ...
    hqr install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``hqr``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``hqr`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``hqr`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("hqr")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``hqr`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _HQR_BIN_DIR
    if _HQR_BIN_DIR is not _SENTINEL:
        return _HQR_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("hqr")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "hqr" or base.startswith("hqr."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "hqr.exe" if _IS_WINDOWS else "hqr"
    ...
    _HQR_BIN_DIR = candidate
    return candidate


def _prepend_hqr_bin_dir(existing_path: str) -> str:
    """Prepend the hqr install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing HQ Runtime-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes HQ Runtime installs for
    itself, so on a machine where HQ Runtime provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$HQR_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$HQR_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_hqr_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_hqr_managed_node``, a first browser install).
    """
    try:
        from hqr_constants import get_hqr_home, iter_hqr_node_dirs

        candidates = [*iter_hqr_node_dirs(), get_hqr_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    HQ Runtime-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  HQ Runtime
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_HQR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_HQR_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_hqr_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_hqr_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _HQR_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the hqr install dir is reachable so plugins can shell out
        # to bare ``hqr`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_hqr_bin_dir(new_path)

    _inject_context_hqr_home(run_env)
    ...
    _strip_hqr_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_hqr_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by HQ Runtime launchers.

    ``gateway_windows._preserve_hqr_home_path`` maps a physical path under
    the resolved HQR_HOME back onto the configured HQR_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of HQR_HOME as HQ Runtime-owned.
    ...
    # lexically the same way get_default_hqr_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of HQR_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\hqr\hqr-agent
    # -> C:\...\hqr-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as HQ Runtime-owned.  Fail-closed: a
...
# --- HQ Runtime venv / repo-root detection (module-level, computed once) ---

#: The HQ Runtime repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import hqr_cli``,
#: etc.  Subprocesses that are NOT the HQ Runtime backend don't need it and it
#: can shadow local packages.
_hqr_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that HQ Runtime launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders HQ Runtime-owned paths under
#: the configured HQR_HOME spelling (which may be a junction to another
#: drive — see ``hqr_cli/gateway_windows.py::_preserve_hqr_home_path``).
...
_hqr_repo_root_aliases: tuple[Path, ...] = _build_hqr_repo_root_aliases(
    _hqr_repo_root,
...
#: subprocess spawn.
_hqr_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<HQ Runtime repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _hqr_repo_root_aliases):
        return None
...
def _get_hqr_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the HQ Runtime runtime.
    ...
    """
    global _hqr_site_packages
    if _hqr_site_packages is not None:
        result = list(_hqr_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _hqr_site_packages = list(result)
...
def _strip_hqr_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip HQ Runtime-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_hqr_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_hqr_owned_pythonpath(env: dict) -> None:
    """Remove HQ Runtime-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the HQ Runtime repo root and the HQ Runtime venv's
    ...
    user entries, so only entries proven HQ Runtime-owned are removed:
    ...
    hqr_site_packages = _get_hqr_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is HQ Runtime-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: HQ Runtime venv site-packages ---
        ...
        # resolved and unresolved (HQR_HOME/junction) spellings count as
        # HQ Runtime-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _hqr_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped HQ Runtime-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, HQ Runtime trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``HQR_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under HQR_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_hqr_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "hqr_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._hqr_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_hqr_pgid", None)
                    if pgid is None:
                        raise
