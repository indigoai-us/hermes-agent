...
three virtual model IDs so the ``hqr tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `hqr tools` → Image "
                    "Generation → OpenAI to configure, or `hqr setup` "
                    "to add the key."
                ),
