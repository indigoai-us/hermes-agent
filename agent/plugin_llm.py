...
Plugins built on HQ Runtime Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing HQ Runtime
...
:class:`~hqr_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors HQ Runtime' main config
...
handles every provider, fallback chain, and per-task override HQ Runtime
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`hqr_cli.plugins.PluginContext`
