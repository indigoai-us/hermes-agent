...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `hqr update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `hqr update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (hqr_cli/dingtalk_auth.py)
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``hqr update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `hqr update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `hqr update` lazy-refresh
