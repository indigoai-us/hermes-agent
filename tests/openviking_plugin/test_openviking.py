...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/hqr/.overview.md") == "viking://user/hqr"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/hqr/memories/profile.md") == "viking://user/hqr/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_hqr_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HQR_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp HQR_HOME
        hqr_home = tmp_path / "hqr_test"
        hqr_home.mkdir(exist_ok=True)
        config_yaml = hqr_home / "config.yaml"
...
class TestOpenVikingRead:
    def test_overview_read_normalizes_uri_and_unwraps_result(self):
        ...
        result = json.loads(provider._tool_read({"uri": "viking://user/hqr/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/hqr/.overview.md"
        assert result["resolved_uri"] == "viking://user/hqr"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/hqr/memories/a.md",
            "viking://user/hqr/memories/b.md",
            "viking://user/hqr/memories/c.md",
            "viking://user/hqr/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/hqr/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/hqr"}),
        ]
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/hqr/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "hqr")
        ...
        assert records["reads"] == ["viking://user/peers/hqr/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "hqr" for headers in normalized_headers)
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/hqr/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/hqr/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/hqr"}))

        assert result["path"] == "viking://user/hqr"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/hqr/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/hqr/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/hqr"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="hqr"):
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/peers/hqr/memories/"
        )
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting hqr for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this HQ Runtime run" not in message, message
