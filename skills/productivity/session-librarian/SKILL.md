...
| List/filter by metadata (age, source, cost, tokens, workspace) | `hqr sessions list` / `stats` via terminal |
| Rename | `hqr sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `hqr sessions archive <filters>` |
| Delete (destructive) | `hqr sessions delete` / `hqr sessions prune <filters>` |
| Export before deleting anything valuable | `hqr sessions export --session-id <id> --format md` |
...
`hqr sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `hqr sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `hqr sessions list`
