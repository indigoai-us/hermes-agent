...
# HQ Runtime WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: HQR_TINI_SHIM_TARGET overrides the `/init` path (and
# HQR_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${HQR_TINI_SHIM_TARGET:-/init}"
WRAPPER="${HQR_TINI_SHIM_WRAPPER:-/opt/hqr/docker/main-wrapper.sh}"
