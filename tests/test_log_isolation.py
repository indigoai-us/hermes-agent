"""The test suite must never write into the operator's real HQ Runtime logs.

`hqr_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<HQR_HOME>/logs/agent.log`.

If HQR_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets HQR_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(hqr_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_hqr_home_is_sandboxed_before_imports(self):
        # Deliberately NOT os.environ: by test time the per-test `_isolate_env`
        # fixture has sandboxed HQR_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import HQR_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set HQR_HOME before test modules import"
        assert Path(home).resolve() != _real_hqr_home().resolve(), (
            f"HQR_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("hqr_cli.main")

        real_logs = str(_real_hqr_home() / "logs")
        offenders = [p for p in _all_file_destinations() if p.startswith(real_logs)]

        assert offenders == [], (
            "the test session is writing into the operator's real HQ Runtime logs:\n  "
            + "\n  ".join(offenders)
        )
