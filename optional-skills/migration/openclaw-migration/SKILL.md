...
description: Import an OpenClaw setup (memories, skills) into HQ Runtime.
version: 1.0.0
author: HQ Runtime Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hqr:
    tags: [Migration, OpenClaw, HQ Runtime, Memory, Persona, Import]
    related_skills: [hqr-agent]
---

# OpenClaw -> HQ Runtime Migration

Use this skill when a user wants to move their OpenClaw setup into HQ Runtime Agent with minimal manual cleanup.
...
```bash
hqr claw migrate              # Full interactive migration
hqr claw migrate --dry-run    # Preview what would be migrated
hqr claw migrate --preset user-data   # Migrate without secrets
hqr claw migrate --overwrite  # Overwrite existing conflicts
hqr claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hqr setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hqr.py` to:

- import `SOUL.md` into the HQ Runtime home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into HQ Runtime memory entries
- merge OpenClaw command approval patterns into HQ Runtime `command_allowlist`
- migrate HQ Runtime-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to HQ Runtime working-directory configuration
- copy OpenClaw skills into `~/.hqr/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen HQ Runtime workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hqr/tts/`
- archive non-secret docs that do not have a direct HQ Runtime destination
...
- `scripts/openclaw_to_hqr.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py`

Do not guess a shorter path like `~/.hqr/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hqr/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of HQ Runtime-compatible secrets, currently:
...
HQ Runtime CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in HQ Runtime. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a HQ Runtime workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing HQ Runtime skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.hqr/skills/migration/openclaw-migration/scripts/openclaw_to_hqr.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of HQ Runtime unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty HQ Runtime targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean HQ Runtime destination. Unsupported auth blobs must still be reported as skipped.
...
- HQ Runtime persona state imported
- HQ Runtime memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.hqr/skills/openclaw-imports/`
