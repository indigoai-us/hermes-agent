...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("HQR_YOLO_MODE", ""))
...
# HQ Runtime session id (observability identity, distinct from the gateway
...
# os.environ["HQR_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export HQR_INTERACTIVE.
_hqr_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "hqr_interactive",
    default=None,
)


def set_hqr_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["HQR_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``HQR_INTERACTIVE`` env var for legacy callers.
    """
    return _hqr_interactive_ctx.set("1" if interactive else "")


def reset_hqr_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_hqr_interactive_context`."""
    _hqr_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``HQR_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hqr_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("HQR_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the HQ Runtime session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HQR_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("HQR_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("HQR_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("HQR_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("HQR_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``hqr chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``HQR_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("HQR_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("HQR_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set HQR_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HQR_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HQR_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("HQR_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``HQR_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $HQR_HOME, or by the resolved absolute
# active profile home path such as /home/hqr/.hqr/config.yaml. The
# resolved-absolute form is folded into the ~/.hqr/ patterns at detection
...
# go stale when HQR_HOME is set after this module is imported, e.g. under the
...
# ~/.hqr/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HQR_ENV_PATH; matches the HQR_HOME override form as
# well as ~/.hqr/.
_HQR_CONFIG_PATH = (
    r'(?:~\/\.hqr/|'
    r'(?:\$home|\$\{home\})/\.hqr/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_hqr_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by HQ Runtime: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY HQ Runtime
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hqr[^\n]*\.env\b', "access to HQ Runtime secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `hqr` and `gateway` (e.g. `hqr -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhqr\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart hqr gateway (kills running agents)"),
    (r'\bhqr\s+update\b', "hqr update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `hqr gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `hqr ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hqr-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hqr-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hqr|gateway|cli\.py)\b', "kill hqr/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill hqr` but not
    # `kill -9 $(pgrep -f hqr)` because the substitution is opaque
    ...
    # the `hqr gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.hqr.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(hqr|ai\.hqr)\b', "stop/restart hqr launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/HQ Runtime file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.hqr/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a HQ Runtime-managed security file (~/.hqr/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HQR_CONFIG_PATH}|{_HQR_ENV_PATH})', "in-place edit of HQ Runtime config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HQR_CONFIG_PATH}|{_HQR_ENV_PATH})', "in-place edit of HQ Runtime config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HQR_CONFIG_PATH}|{_HQR_ENV_PATH})', "in-place edit of HQ Runtime config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.hqr/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / HQR_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) HQ Runtime home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hqr), so folding the user home
    # first would eat the prefix the HQ Runtime-home fold needs.
    command = _rewrite_resolved_hqr_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / HQR_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.hqr``); the matched
...
def _rewrite_resolved_hqr_home(command: str) -> str:
    """Rewrite the resolved absolute HQ Runtime home prefix to ``~/.hqr/``.

    Resolves the active ``HQR_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.hqr/`` so the static ``_HQR_CONFIG_PATH`` / ``_HQR_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/hqr/.hqr/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$HQR_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from hqr_constants import get_hqr_home
        home = get_hqr_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.hqr")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``hqr send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one HQ Runtime ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"hqr-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from HQ Runtime YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["HQR_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "HQR_SPINNER_PAUSE" in os.environ:
            del os.environ["HQR_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses HQ Runtime approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``HQR_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            HQR_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export HQR_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set HQR_INTERACTIVE or "
                "HQR_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set HQR_INTERACTIVE or "
            "HQR_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("HQR_EXEC_ASK"):
        ...
        # can see (HQR_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``HQR_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from hqr_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from hqr_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("HQR_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # HQR_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from hqr_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (HQR_EXEC_ASK) still takes this path even when INTERACTIVE
