...
- ``hqr_cli.providers``        -- canonical provider identity + overlays
- ``hqr_cli.model_normalize``  -- per-provider name formatting
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``hqr model`` wizard — not a
    ...
    while ``hqr model`` still live-probed the full ``/v1/models`` list.
...
_HQR_MODEL_WARNING = (
    "Nous Research HQ Runtime 3 & 4 models are NOT agentic and are not designed "
    "for use with HQ Runtime Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research HQ Runtime 3 / HQ Runtime 4 chat families.
# The previous substring check (`"hqr" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hqr-brain:qwen3-14b-ctx16k`` that just
# happen to carry "hqr" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   hqr-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HQR_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hqr[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_hqr_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous HQ Runtime 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_HQR_NON_AGENTIC_RE.search(model_name))


def _check_hqr_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous HQ Runtime 3/4 chat model."""
    if is_nous_hqr_non_agentic(model_name):
        return _HQR_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``hqr config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from hqr_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``hqr_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'hqr model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'hqr doctor' — config issues detected:"
        ...
        from hqr_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from hqr_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from hqr_cli.config import load_config
            from hqr_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from hqr_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from hqr_cli.providers import nous_api_mode
    ...
    # hqr_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from hqr_cli.models import normalize_opencode_base_url
    ...
    hqr_warn = _check_hqr_model_warning(new_model)
    if hqr_warn:
        warnings.append(hqr_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from hqr_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: HQ Runtime provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from hqr_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from hqr_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from hqr_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from hqr_cli.providers import HQR_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from hqr_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: HQ Runtime-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for hqr_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(hqr_id)
        if (
            _alias_target
            and _alias_target != hqr_id
        ...
        try:
            ...
            pass
        if _canonical != hqr_id:
            continue
        if hqr_id.lower() in seen:
            continue
        if hqr_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(hqr_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from hqr_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(hqr_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hqr_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        hqr_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(hqr_id)
            seen.add(hqr_id.lower())

    # --- Section 2: HQ Runtime-only providers (HQR_OVERLAYS) ---
    _mdev_to_hqr = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in HQR_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        hqr_slug = _mdev_to_hqr.get(pid, pid)
        if hqr_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or hqr_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or hqr_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(hqr_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(hqr_slug)
            seen.add(pid.lower())
            seen.add(hqr_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from hqr_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against hqr_id / mdev_id (section 1), pid / hqr_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from hqr_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by hqr provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://hermes-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a HQ Runtime release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from hqr_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from hqr_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check HQ Runtime-mapped providers ---
    from hqr_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hqr_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hqr_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # same hqr_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if hqr_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (HQR_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers HQ Runtime may not route yet.
        ...
        if not has_creds:
            try:
                from hqr_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hqr_id)
                )
        ...
        # /model picker sees the SAME list `hqr model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(hqr_id)
        if not model_ids:
            model_ids = curated.get(hqr_id, [])
            if hqr_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(hqr_id, model_ids)
    ...
    # --- 2. Check HQ Runtime-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hqr_cli.providers import HQR_OVERLAYS
    from hqr_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → HQ Runtime provider ID.
    # HQR_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use HQ Runtime IDs ("copilot").
    _mdev_to_hqr = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in HQR_OVERLAYS.items():
        ...
        # Resolve HQ Runtime slug — e.g. "github-copilot" → "copilot"
        hqr_slug = _mdev_to_hqr.get(pid, pid)
        if hqr_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or hqr_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(hqr_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or hqr_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(hqr_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(hqr_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", hqr_slug, exc)
        ...
        # configured.
        if not has_creds and hqr_slug == "anthropic":
            try:
                ...
                if (hqr_creds and hqr_creds.get("accessToken")) or \
        ...
        if hqr_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(hqr_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(hqr_slug)
                model_ids = _ids if _ids else (curated.get(hqr_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(hqr_slug, []) or curated.get(pid, [])
        elif hqr_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `hqr model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from hqr_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(hqr_slug, []) or curated.get(pid, [])
                if hqr_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(hqr_slug, model_ids)
        total = len(model_ids)
        if hqr_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": hqr_slug,
            "name": get_label(hqr_slug),
            "is_current": hqr_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hqr",
        ...
        seen_slugs.add(hqr_slug.lower())
        _record_builtin_endpoint(hqr_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HQR_OVERLAYS (keeps /model in sync
    # with `hqr model`).
    try:
        from hqr_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # HQ Runtime writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # HQ Runtime's own writer uses (mirrors section-4 grouping).
            ...
            # ``hqr model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``hqr model``. Pin with
            ...
            if _discovery_allowed:
                try:
                    from hqr_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. HQ Runtime's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, hqr_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``hqr model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from hqr_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`hqr_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from hqr_cli.models import fetch_openrouter_models
