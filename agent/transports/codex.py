...
# Wire-name used when HQ Runtime keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through HQ Runtime's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "hqr_web_search"
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if params.get("is_xai_responses", False):
            ...
            # out at high. max/ultra are HQ Runtime ladder aliases for "this
        ...
        if (params.get("provider") or "").strip().lower() == "actual":
            ...
            # line 1 column 1"). Clamp HQ Runtime' wider set to the supported one.
            _effort_clamp.update({"xhigh": "high", "ultra": "max"})
        reasoning_effort = _effort_clamp.get(reasoning_effort, reasoning_effort)

        response_tools = _responses_tools(tools)

        # xAI server-side web search vs HQ Runtime web providers.
        #
        ...
        #    keep HQ Runtime dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``hqr_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so HQ Runtime dispatches
