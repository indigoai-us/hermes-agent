"""Unified removal contract for every credential source HQ Runtime reads from.

HQ Runtime seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.hqr/.env
    claude_code   — ~/.claude/.credentials.json
    hqr_pkce   — ~/.hqr/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``hqr auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), hqr_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.hqr/.env  → clear it
    ...
    """
    from hqr_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in hqr_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to HQ Runtime.",
            f"  The pool entry is now suppressed — HQ Runtime will ignore "
            f"{env_var} until you run `hqr auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so HQ Runtime stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `hqr auth add anthropic` to re-enable if needed.",
    ])


def _remove_hqr_pkce(provider: str, removed) -> RemovalResult:
    """~/.hqr/.anthropic_oauth.json is ours — delete it outright."""
    from hqr_constants import get_hqr_home

    result = RemovalResult()
    oauth_file = get_hqr_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared HQ Runtime Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `hqr auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `hqr auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``hqr auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `hqr model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the HQ Runtime auth store is not enough — _seed_from_singletons() would
    ...
    via ``hqr auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `hqr auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `hqr auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    HQ Runtime stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `hqr auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="hqr_pkce",
        remove_fn=_remove_hqr_pkce,
        description="~/.hqr/.anthropic_oauth.json",
    ))
    register(RemovalStep(
