...
Set via `hqr model` (picker) or `hqr setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `hqr auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `hqr auth add openai-codex` |
| qwen-oauth | OAuth | `hqr auth add qwen-oauth` |
| minimax-oauth | OAuth | `hqr auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`hqr auth`).
Fallback chain when the primary fails: `hqr fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`hqr_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   hqr config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
