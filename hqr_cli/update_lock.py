"""Cross-process mutual exclusion for in-flight HQ Runtime updates.

Three different surfaces can start an update of the same install tree:

* ``hqr update`` from a terminal,
* the dashboard's Update button (``POST /api/hqr/update`` →
  ``_spawn_hqr_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``hqr-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``hqr update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <HQR_HOME>/.hqr-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``hqr update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("HQ Runtime is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``hqr-setup`` binary under ``~/.hqr`` is only refreshed by a full
  installer run (``copy_self_to_hqr_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("HQ Runtime is still running") on every
  GUI update forever, with no HQ Runtime process actually running.
"""
...
MARKER_NAME = ".hqr-update-in-progress"

# Set by an orchestrating updater (the Tauri `hqr-setup --update` flow) to
# its own pid before spawning `hqr update` as a child stage. The parent
...
HANDOFF_PID_ENV = "HQR_UPDATE_HANDOFF_PID"
...
# to show "HQ Runtime is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* HQ Runtime home (never the context-local profile override):
    the Rust updater resolves ``$HQR_HOME`` or the platform default, and the
    ...
    return get_process_hqr_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``hqr update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``hqr-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another HQ Runtime update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `hqr update` as a stage): we run under ITS claim
