"""Tests for IRC gateway configuration via `hqr setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh HQ Runtime install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new HQ Runtime install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#hqr")
            monkeypatch.setenv("IRC_NICKNAME", "hqr-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `hqr setup gateway` experience with IRC present."""
