"""Shared fixtures for the hqr-agent test suite.
...
2. **Isolated HQR_HOME.** HQR_HOME points to a per-test tempdir so
   code reading ``~/.hqr/*`` via ``get_hqr_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hqr"`` instead of the canonical
   ``get_hqr_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No HQR_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox HQR_HOME before ANY test module is imported ──────────────────
# `hqr_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_hqr_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.hqr/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes HQR_HOME, but fixtures run
...
# the REAL HQ Runtime root — capture it BEFORE the sandbox rewires HQR_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.hqr (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("HQR_KANBAN_HOME", "").strip()
_PRE_SANDBOX_HQR_HOME = os.environ.get("HQR_HOME", "")


def _hqr_home_points_at_production(value: str) -> bool:
    """True when a pre-set HQR_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    HQR_HOME=~/.hqr``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``hqr_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.hqr`` — the escape vector that landed pytest
    ...
    HQR_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".hqr").resolve()
    except Exception:
        return True
...
if _hqr_home_points_at_production(os.environ.get("HQR_HOME", "")):
    _SESSION_HQR_HOME = tempfile.mkdtemp(prefix="hqr-test-home-")
    os.environ["HQR_HOME"] = _SESSION_HQR_HOME
    atexit.register(shutil.rmtree, _SESSION_HQR_HOME, True)
...
# like a real CLI") — which used to disarm hqr_state's live-DB guard in
# the child at the same moment the child lost the HQR_HOME redirect.
# HQR_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# hqr_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export HQR_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["HQR_TEST_ISOLATION"] = os.environ.get("HQR_HOME", "") or "1"

#: HQR_HOME as it stood when conftest was imported - i.e. before any test
...
HQR_HOME_AT_CONFTEST_IMPORT = os.environ.get("HQR_HOME", "")
...
# HQR_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_HQR_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``hqr_cli.voice.speak_text`` — real synthesis,
    ...
    "HQR_MAX_ITERATIONS",
    "HQR_SESSION_PLATFORM",
    "HQR_SESSION_CHAT_ID",
    "HQR_SESSION_CHAT_NAME",
    "HQR_SESSION_CHAT_TYPE",
    "HQR_SESSION_THREAD_ID",
    "HQR_SESSION_SOURCE",
    "HQR_SESSION_KEY",
    "HQR_GATEWAY_SESSION",
    "HQR_CRON_SESSION",
    ...
    "HQR_TIMEZONE",
    "HQR_REDACT_SECRETS",
    "HQR_BACKGROUND_NOTIFICATIONS",
    "HQR_EXEC_ASK",
    "HQR_HOME_MODE",
    "HQR_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.hqr/kanban.db instead of the per-test HQR_HOME.
    "HQR_KANBAN_DB",
    "HQR_KANBAN_BOARD",
    "HQR_KANBAN_HOME",
    "HQR_KANBAN_WORKSPACES_ROOT",
    "HQR_KANBAN_LOGS_ROOT",
    "HQR_KANBAN_TASK",
    "HQR_KANBAN_WORKSPACE",
    "HQR_KANBAN_RUN_ID",
    "HQR_KANBAN_CLAIM_LOCK",
    "HQR_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "hqr" host.
    "HQR_HONCHO_HOST",
    ...
    "HQR_DASHBOARD_OAUTH_CLIENT_ID",
    "HQR_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and HQR_HOME to per-test tempdirs so code that
    reads ``~/.hqr/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral HQR_* vars that could change test semantics.
    for name in _HQR_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("HQR_HONCHO_HOST", "hqr")

    # 3. Redirect HQR_HOME to a per-test tempdir. Code that reads
    #    ``~/.hqr/*`` via ``get_hqr_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.hqr/*`` via
    #    ``Path.home() / ".hqr"`` instead of ``get_hqr_home()``
    #    is a bug to fix at the callsite.
    fake_hqr_home = tmp_path / "hqr_test"
    fake_hqr_home.mkdir()
    (fake_hqr_home / "sessions").mkdir()
    (fake_hqr_home / "cron").mkdir()
    (fake_hqr_home / "memories").mkdir()
    (fake_hqr_home / "skills").mkdir()
    monkeypatch.setenv("HQR_HOME", str(fake_hqr_home))
    ...
    # hqr_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("HQR_TEST_ISOLATION", str(fake_hqr_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("HQR_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. hqr_state computes ``DEFAULT_DB_PATH = get_hqr_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from hqr_state import ...``) that
    ...
    hqr_state_mod = sys.modules.get("hqr_state")
    if hqr_state_mod is not None and hasattr(hqr_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            hqr_state_mod, "DEFAULT_DB_PATH", fake_hqr_home / "state.db"
        )
    ...
    monkeypatch.setenv("HQR_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.hqr/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import hqr_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path HQR_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_hqr_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from hqr_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.hqr. This autouse
...
# allow-list because test-level fixtures legitimately move HQR_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session HQR_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``HQR_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) HQ Runtime root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_HQR_HOME and not _hqr_home_points_at_production(
        _PRE_SANDBOX_HQR_HOME
    ):
        # HQR_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_hqr_root().resolve()
    # No pre-existing HQR_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".hqr").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.hqr`` captured at import time. Hermetic tests that legitimately
    move HQR_HOME to sibling tempdirs are unaffected.

    Only patches when ``hqr_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("hqr_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import hqr_cli.kanban_db`
...
# ``hqr_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL HQ Runtime root. This fixture wires the
# test-side knobs:
...
#     installs where HQR_HOME is not ~/.hqr) into the guard's
...
# so subprocess children that import hqr_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("hqr_state")
    ...
    if _PRE_SANDBOX_HQR_HOME and not _hqr_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_HQR_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local HQ Runtime home override redirects every later
    # ``get_hqr_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from hqr_constants import get_hqr_home_override, set_hqr_home_override

        if get_hqr_home_override() is not None:
            set_hqr_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal hqr config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``hqr-gateway`` process
...
# correlated with a ``tests/hqr_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> hqr-gateway``
...
def _wal_is_usable() -> bool:
    """True when HQ Runtime will actually put a database into WAL mode here.

    HQ Runtime refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running HQ Runtime can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a HQ Runtime managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``hqr_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_hqr_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_hqr_home`` fixture
    redirects ``HQR_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.hqr/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``hqr_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["HQR_VOICE_TTS"] = "1"``. The
...
#      ``hqr_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``hqr_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``hqr_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``hqr_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# HQ Runtime runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.hqr``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where HQ Runtime falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — HQ Runtime uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart hqr-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting hqr/python patterns are also blocked.
    """
    ...
    def _matches_hqr_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _HQR_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_hqr_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch hqr-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m hqr_cli.main".
                if (
                    "hqr" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live hqr-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting hqr/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `hqr update` (or the
        # equivalent `python -m hqr_cli.main update`).  These commands
        ...
        if "update" in low and (
            # hqr update / hqr update --gateway / setsid bash -c ... hqr update
            ("hqr" in low and "update" in low.split())
            or
            # python -m hqr_cli.main update --gateway
            ("hqr_cli" in low and "update" in low.split())
            or
            # venv/bin/hqr update  (absolute path variant used in tests)
            (".venv/bin/hqr" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`hqr update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_HQR_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import hqr_cli.voice as _voice
    except Exception:
