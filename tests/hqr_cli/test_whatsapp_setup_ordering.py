...
Before the fix, ``hqr whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``hqr gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    hqr = home / ".hqr"
    hqr.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("HQR_HOME", str(hqr))
...
def _env_value(hqr_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from hqr_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("hqr_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``hqr whatsapp`` with an existing paired session and
