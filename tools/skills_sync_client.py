...
409), and is driven by:
  ...
  * the ``hqr sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``hqr_cli/``) so the
...
is toggled via ``hqr sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.hqr/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``hqr sync enable`` — it is
...
# namespace from hqr-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the HQR_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: HQR_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("HQR_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a HQ Runtime Cloud instance can be set
...
# precedence as base_url: the HQR_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   HQR_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   HQR_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   HQR_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``HQR_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a HQ Runtime Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("HQR_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``HQR_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``hqr sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "HQR_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``HQR_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``hqr sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    HQ Runtime Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("HQR_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.hqr/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.hqr/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (HQ Runtime Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``HQR_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.hqr/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``hqr sync device --name`` to set an explicit
    label."""
    ...
    # HQ Runtime Cloud (and any templated deployment) can seed the label
    # declaratively via HQR_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `hqr sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("HQR_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.hqr/skills/.sync_device_id, overwriting
...
# path under ~/.hqr/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.hqr/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this HQ Runtime speaks "
            f"{WIRE_VERSION} — update HQ Runtime to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (hqr sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.hqr/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "hqr skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (HQR_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``hqr sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.hqr/skills/_org/
...
#
# `hqr sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.hqr/skills/_org/<org_id>/``.
