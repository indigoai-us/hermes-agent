"""
Profile management for multiple isolated HQ Runtime instances.

Each profile is a fully independent HQR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.hqr/profiles/<name>/`` by default.

The "default" profile is ``~/.hqr`` itself — backward compatible,
zero migration needed.

Usage::

    hqr profile create coder          # fresh profile + bundled skills
    hqr profile create coder --clone  # also copy config, .env, SOUL.md, skills
    hqr profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    hqr -p coder chat                 # or via flag
    hqr profile use coder             # set as sticky default
    hqr profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See hqr_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `hqr profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `hqr
...
# `hqr skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         HQ Runtime infrastructure directories that only the default profile
         (``~/.hqr``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_hqr_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.hqr) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "hqr-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when HQR_HOME equals the
...
# into the export. The set below identifies the *known HQ Runtime profile
# artifacts* at the root of HQR_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``hqr_constants``.
...
_RESERVED_NAMES = frozenset({
    "hqr", "default", "test", "tmp", "root", "sudo",
})

# HQ Runtime subcommands that cannot be used as profile names/aliases
_HQR_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hqr root, NOT to the current HQR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HQR_HOME points outside
    ``~/.hqr``, profiles live under ``HQR_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_hqr_home() / "profiles"


def _get_default_hqr_home() -> Path:
    """Return the default (pre-profile) HQR_HOME path.

    In standard deployments this is ``~/.hqr``.
    In Docker/custom deployments where HQR_HOME is outside ``~/.hqr``
    (e.g. ``/opt/data``), returns HQR_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_hqr_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the HQ Runtime installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HQR_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_hqr_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, hqr subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _HQR_SUBCOMMANDS:
        return f"'{canon}' conflicts with a hqr subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hqr_exe = shutil.which("hqr") or "hqr"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(hqr_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running HQ Runtime. If we
    ...
    try:
        ...
        # not be migrated. The next `hqr doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``hqr -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``hqr profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``hqr -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``hqr profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``HQR_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing HQ Runtime config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_hqr_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, hqr_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own HQR_HOME. When
    ...
    The returned ``hqr_home`` is the path to pass to
    ``set_hqr_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_hqr_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``hqr update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.hqr)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.hqr/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `hqr -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this HQ Runtime profile.\n"
    ...
    if not soul_path.exists():
        try:
            from hqr_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `hqr update`'s
    ...
    # Cloned configs can be older than the running HQ Runtime (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `hqr profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `hqr -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HQR_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``hqr profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HQR_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_hqr_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running HQ Runtime *backends* bound to this profile.
    ...
    names it, so find it by inspection: a HQ Runtime backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``HQR_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `hqr -p <canon> profile
    ...
    hqr_markers = ("hqr_cli.main", "hqr-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('hqr')` resolution) spawns the
    ...
    # interpreter, not "hqr", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _HQR_CONSOLE_SCRIPT_NAMES = frozenset({"hqr", "hqr-agent", "hqr-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a HQ Runtime process: either an entrypoint marker in argv, a
            # resolved executable named `hqr`, or a python interpreter
            # directly exec'ing a `hqr`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_hqr = (
                any(marker in joined for marker in hqr_markers)
                or exe_name == "hqr"
                or exe_name.startswith("hqr")
            )
            if not is_hqr and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: hqr, hqr-agent,
                # hqr-acp) rather than a bare `startswith("hqr")` --
                ...
                # "hqr-notes.py" or "hqr-unrelated-tool" as the shim,
                ...
                is_hqr = script_stem in _HQR_CONSOLE_SCRIPT_NAMES
            if not is_hqr:
                continue
            ...
            # ...or by HQR_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("HQR_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.hqr).\n"
            "To remove everything, use: hqr uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_hqr_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``HQR_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from hqr_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set HQR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("HQR_HOME")
    try:
        os.environ["HQR_HOME"] = str(profile_dir)
        from hqr_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["HQR_HOME"] = old_home
        elif "HQR_HOME" in os.environ:
            del os.environ["HQR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.hqr/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: hqr profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HQR_HOME.

    Returns ``"default"`` if HQR_HOME is not set or points to ``~/.hqr``.
    Returns the profile name if HQR_HOME points into ``~/.hqr/profiles/<name>``.
    Returns ``"custom"`` if HQR_HOME is set to an unrecognized path.
    """
    from hqr_constants import get_hqr_home
    hqr_home = get_hqr_home()
    resolved = hqr_home.resolve()

    default_resolved = _get_default_hqr_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      HQR_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-HQ Runtime file the
      user may have lying alongside HQR_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``hqr sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``HQR_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.hqr itself — its parent is ~/ and its
        # directory name is ".hqr", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: hqr profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.hqr itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.hqr). "
            "Specify a different name: hqr profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="hqr_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"hqr_{old_name}"
    legacy_old_host = f"hqr.{old_name}"
    new_host = f"hqr_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_hqr_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("hqr_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HQR_HOME path string.

    Called early in the CLI entry point, before any hqr modules
    are imported, to set the HQR_HOME environment variable.

    When HQR_HOME is already set, the configured spelling IS the
    ...
    it to match HQ Runtime-owned PYTHONPATH entries written in the same
