...
Ledger path resolution (first wins):
  --ledger PATH
  $HQR_CITATION_LEDGER
  $HQR_HOME/cache/citations/ledger.json
"""
...
from _hqr_home import get_hqr_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("HQR_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_hqr_home() / "cache" / "citations" / "ledger.json"
