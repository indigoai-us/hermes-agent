...
this. Capabilities govern the *host API surfaces* HQ Runtime hands out (which
...
until the user re-consents (``hqr plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer HQ Runtime may declare ids this build doesn't know; they can
