"""Offline, non-destructive recovery for a damaged HQ Runtime session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``hqr_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``hqr_state._backup_db_file``.
    """
    from hqr_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every HQ Runtime process using this profile and retry. "
                "This includes the interactive `hqr` CLI session this "
                ...
                "HQ Runtime process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `hqr` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from hqr_cli.session_lost_and_found import (
