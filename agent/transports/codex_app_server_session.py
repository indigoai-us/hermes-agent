...
Owns one Codex thread per HQ Runtime session. Drives `turn/start`, consumes
...
# HQ Runtime' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_HQR_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse HQ Runtime/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per HQ Runtime session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _HQR_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("HQR_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="hqr",
            client_title="HQ Runtime Agent",
            client_version=_get_hqr_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        into HQ Runtime' messages shape.
        ...
        # supports rich content but HQ Runtime' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into HQ Runtime' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own hqr-tools callback we
            # auto-accept — the user already approved HQ Runtime' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO HQ Runtime
        approval-mode/timeout logic. The HQ Runtime-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; HQ Runtime approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map HQ Runtime approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    HQ Runtime returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the HQ Runtime approval mode/timeout resolution
    ...
    # "prompt expired"; the HQ Runtime-side messaging already distinguishes them.
    return "decline"
...
def _get_hqr_version() -> str:
    """Best-effort HQ Runtime version string for codex's userAgent line."""
    try:
