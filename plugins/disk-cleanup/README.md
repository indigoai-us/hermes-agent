...
Auto-tracks and cleans up ephemeral files created during HQ Runtime Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HQR_HOME` and `/tmp/hqr-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HQR_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under HQR_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `HQR_HOME` or `/tmp/hqr-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HQR_HOME/disk-cleanup/` is itself excluded
- `$HQR_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
