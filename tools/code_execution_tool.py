...
Lets the LLM write a Python script that calls HQ Runtime tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `hqr_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `hqr_tools.py` with file-based RPC stubs
...
# available on every platform HQ Runtime itself runs on.
...
# match a safe prefix, the operational HQR_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports HQ Runtime
# code can lose the DB-layer Kanban mutation guard while still inheriting
# HQR_HOME.
#
# NB: the broad "HQR_" prefix was deliberately removed (#27303) — it leaked
# HQR_*-named config that lacks a secret substring (e.g. HQR_BASE_URL,
# HQR_KANBAN_DB, HQR_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HQR_CHILD_ALLOWED below; HQR_RPC_SOCKET /
# HQR_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational HQR_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set hqr_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_HQR_CHILD_ALLOWED = frozenset({
    "HQR_HOME",
    "HQR_PROFILE",
    "HQR_CONFIG",
    "HQR_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational HQR_* vars (_HQR_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret HQR_* vars dropped by the tightened allowlist (#27303). The
    # broad "HQR_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like HQR_KANBAN_DB / HQR_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _HQR_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("HQR_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped HQR_* var.
            _dropped_hqr.append(k)
    if _dropped_hqr:
        logger.debug(
            "execute_code: dropped %d non-allowlisted HQR_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-HQ Runtime-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-HQ Runtime-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    hqr_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'hqr_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_hqr_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the hqr_tools.py stub module.
...
"""Auto-generated HQ Runtime tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["HQR_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("HQR_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated HQ Runtime tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HQR_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hqr_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated hqr_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/hqr_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_hqr_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/hqr_tools.py", tools_src)
        ...
        env_prefix = (
            f"HQR_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"HQR_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("HQR_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    to a subset of HQ Runtime tools.
    ...
    # --- Set up temp directory with hqr_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="hqr_sandbox_")
    ...
    # selector from HQR_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"hqr_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated hqr_tools module.
        ...
        tools_src = generate_hqr_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "hqr_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  HQR_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["HQR_RPC_SOCKET"] = rpc_endpoint
        child_env["HQR_RPC_TOKEN"] = rpc_token
        ...
        # HQR_TIMEZONE is an internal HQ Runtime setting and must not leak
        # into child processes.
        _tz_name = os.getenv("HQR_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("HQR_TIMEZONE", None)
        ...
        # ``hqr_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # HQ Runtime's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing HQ Runtime's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip HQ Runtime-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-HQ Runtime-
        ...
        _strip_hqr_owned_pythonpath(child_env)
        _hqr_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_hqr_python_environment(_child_python):
            _pp_parts.append(_hqr_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import hqr_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the HQ Runtime "
                "environment; hqr root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.hqr/.env')).
...
def _load_config() -> dict:
    ...
    ``hqr --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (hqr-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the HQ Runtime environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the hqr root from every
...
def _uses_hqr_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to HQ Runtime's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``hqr tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and hqr-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.hqr/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls HQ Runtime tools programmatically. "
        ...
        f"Available via `from hqr_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from hqr_tools import {import_str}` "
