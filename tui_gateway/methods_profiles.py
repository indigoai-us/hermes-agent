...
handlers delegate to the same `hqr_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List HQ Runtime profiles (name, path, model, description, skill count).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's hqr-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``hqr setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (hqr_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local HQR_HOME override — the same
        ...
        try:
            ...
            token = set_hqr_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from hqr_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from hqr_cli.config import load_config_readonly, read_user_config_raw
            from hqr_cli.web_routers.profiles import _write_profile_model
            from hqr_constants import (
                reset_hqr_home_override,
                set_hqr_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the HQR_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from hqr_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `hqr tools`
            ...
            # composites (hqr-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from hqr_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `hqr tools` / Settings is where
            ...
            description = ""
            try:
                from hqr_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from hqr_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from hqr_cli.skills_config import save_disabled_skills
