#!/usr/bin/env python3
"""Google Workspace API CLI for HQ Runtime Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing HQ Runtime-facing JSON contract and falls back to the Python client
...
from _hqr_home import get_hqr_home
...
def _gws_binary() -> str | None:
    override = os.getenv("HQR_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for HQ Runtime Agent")
