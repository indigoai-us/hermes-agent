...
This module is imported by ``hqr_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no hqr_cli.config,
...
``hqr_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(hqr_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect HQR_HOME?"""
    active_profile = os.path.join(hqr_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    hqr_home = os.environ.get("HQR_HOME", "").strip()
    if hqr_home:
        return hqr_home
    return os.path.join(os.path.expanduser("~"), ".hqr")
...
def container_mode_may_be_active() -> bool:
    ...
    hqr_home = os.environ.get("HQR_HOME", "").strip()
    if hqr_home:
        if os.path.exists(os.path.join(hqr_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(hqr_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(hqr_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".hqr")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect HQR_HOME.
    """
...
def print_fast_version_info() -> None:
    from hqr_cli import __release_date__, __version__

    print(f"HQ Runtime Agent v{__version__} ({__release_date__})")
    ...
    print("Run 'hqr version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``hqr --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the HQR_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("HQR_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
