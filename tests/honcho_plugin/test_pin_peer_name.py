...
Under a gateway (Telegram, Discord, Slack, ...) HQ Runtime passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HQR_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to HQ Runtime via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each HQ Runtime profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `hqr -p partner` pinned to a different person's peer).
