"""
HQ Runtime Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hqr desktop`` / ``hqr gui``)
     Built inside the agent checkout under ``$HQR_HOME/hqr-agent/``:
       ...
       - ``$HQR_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/HQ Runtime.app`` or ``~/Applications/HQ Runtime.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\HQ Runtime`` (NSIS per-user)
...
the app name ("HQ Runtime"), separate from ``$HQR_HOME``:
  - macOS:   ``~/Library/Application Support/HQ Runtime``
  - Windows: ``%APPDATA%\\HQ Runtime``
  - Linux:   ``$XDG_CONFIG_HOME/HQ Runtime`` (default ``~/.config/HQ Runtime``)
...
``hqr uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(hqr_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return hqr_home / "hqr-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "HQ Runtime"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "HQ Runtime"
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        base = Path(appdata) if appdata else (home / "AppData" / "Roaming")
        return base / "HQ Runtime"
...
def source_built_gui_artifacts(hqr_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``hqr desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``hqr-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(hqr_home)
    ...
    return [
        ...
        hqr_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "HQ Runtime" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/HQ Runtime.app"),
            home / "Applications" / "HQ Runtime.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\HQ Runtime).
            local_base / "Programs" / "HQ Runtime",
            # Older / alternate layout some builds used.
            local_base / "hqr-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "HQ Runtime")
    else:
        ...
        # ``uninstall_gui``.
        from hqr_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `hqr desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "HQ Runtime.desktop",
        ]
    return paths


def agent_is_installed(hqr_home: Path) -> bool:
    """Return True when a usable Python agent install exists under HQR_HOME.
...
def gui_is_installed(hqr_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(hqr_home):
        if p.exists():
            return True
...
def gui_install_summary(hqr_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = hqr_home if hqr_home is not None else get_hqr_home()
...
def uninstall_gui(hqr_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hqr-agent/hqr_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$HQR_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so HQ Runtime disappears from the launcher.
        try:
            from hqr_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove hqr' or "
            "'sudo dnf remove hqr'). AppImage builds are a single file you "
