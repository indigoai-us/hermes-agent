"""
Dump command for hqr CLI.

Outputs a compact, plain-text summary of the user's HQ Runtime setup
...
from hqr_cli.config import get_hqr_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.hqr/.env.
    ...
    exports. ``hqr debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.hqr_build_sha`` by the Dockerfile's
    ``HQR_GIT_SHA`` build-arg (see ``hqr_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``hqr dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from hqr_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(hqr_home: Path) -> int:
...
def _cron_summary(hqr_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = hqr_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"hqr_home:      {display_hqr_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.hqr/.env: a managed
        ...
        # A credential added via `hqr auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `hqr auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["hqr-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(hqr_home)}")
    lines.append(f"  skills:             {_count_skills(hqr_home)}")
