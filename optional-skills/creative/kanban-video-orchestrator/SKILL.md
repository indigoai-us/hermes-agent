...
short to a music video to an ASCII loop — in a HQ Runtime Kanban pipeline that
...
3. **Generates** a setup script that creates HQ Runtime profiles, project workspace, and the initial kanban task
...
For mapping role → which HQ Runtime skills + toolsets it loads, see
...
3. Creates each HQ Runtime profile via `hqr profile create --clone`
...
7. Fires the initial `hqr kanban create` task assigned to the director
...
```bash
hqr kanban watch --tenant <project-tenant>     # live events
hqr kanban list  --tenant <project-tenant>     # board snapshot
hqr dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `hqr kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${HQR_HOME:-~/.hqr}/.env` or the user's secret store.
