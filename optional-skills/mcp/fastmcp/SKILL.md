...
metadata:
  hqr:
    ...
    related_skills: [hqr-agent, mcporter]
prerequisites:
  commands: [python]
---
...
- smoke-test a server with the FastMCP CLI before wiring it into HQ Runtime or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to HQ Runtime. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hqr/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hqr/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is HQ Runtime integration, either:

- configure the server in `~/.hqr/config.yaml` using the `native-mcp` skill, or
...
### HQ Runtime cannot see the deployed server

The server-building part may be correct while the HQ Runtime config is not. Load the `native-mcp` skill and configure the server in `~/.hqr/config.yaml`, then restart HQ Runtime.
