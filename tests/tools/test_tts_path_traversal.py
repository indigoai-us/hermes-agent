...
def test_output_path_rejects_hqr_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    hqr_home = tmp_path / "hqr-home"
    hqr_home.mkdir()
    monkeypatch.setattr(file_safety, "_hqr_home_path", lambda: hqr_home)
    monkeypatch.setattr(file_safety, "_hqr_root_path", lambda: hqr_home)

    target = hqr_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
