"""Tests for the AI_AGENT / HQR_AGENT harness-attribution env vars.
...
HQ Runtime-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal HQ Runtime' id in the public agent-harness
...
environment does not inherit the HQ Runtime process env, and survives the
cross-session leak guard that strips ``HQR_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__HQR_CWD_testsession0__"
        env._snapshot_path = "/tmp/hqr-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'HQR_AGENT="${HQR_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT HQR=$HQR_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "HQR_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} HQR=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", HQR_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi HQR=false" in out.stdout
