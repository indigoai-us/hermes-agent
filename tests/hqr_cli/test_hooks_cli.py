"""Tests for the ``hqr hooks`` CLI subcommand."""
...
from hqr_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("HQR_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("HQR_ACCEPT_HOOKS", raising=False)
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `hqr hooks doctor` used to run every
