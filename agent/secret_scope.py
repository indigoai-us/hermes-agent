...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the HQR_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # HQ Runtime runtime / deployment
    "HQR_HOME", "HQR_PROFILE", "HQR_GATEWAY_LOCK_DIR",
    "HQR_MAX_ITERATIONS", "HQR_MAX_TOKENS", "HQR_API_TIMEOUT",
    "HQR_REDACT_SECRETS", "HQR_NOUS_TIMEOUT_SECONDS",
    "_HQR_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "HQR_KANBAN_DB", "HQR_KANBAN_WORKSPACES_ROOT", "HQR_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "HQR_KANBAN_",
    "HQR_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset HQ Runtime writes
    ...
    ``hqr_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical HQ Runtime parser: save_env_value
    ...
    from hqr_cli.config import _parse_env_value
...
def build_profile_secret_scope(hqr_home: Path) -> Dict[str, str]:
    ...
    try:
        from hqr_cli.env_loader import get_secret_source_values
