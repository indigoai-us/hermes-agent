...
``auth.json``.  They intentionally have no dependency on ``hqr_cli.auth`` so
...
# Sources HQ Runtime owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and HQ Runtime-owned OAuth/device-code state)
