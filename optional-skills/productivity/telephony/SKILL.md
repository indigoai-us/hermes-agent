...
This optional skill gives HQ Runtime practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${HQR_HOME:-~/.hqr}/.env`
...
It does **not** turn HQ Runtime into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to HQ Runtime memory
...
### 1) "I want HQ Runtime to own a real phone number"
Use **Twilio**.
...
- pairs well with HQ Runtime `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.hqr/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
hqr skills search telephony
hqr skills install official/productivity/telephony
```
...
Then save credentials into HQ Runtime:
...
3. Buy it and save it into `${HQR_HOME:-~/.hqr}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is HQ Runtime calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing HQ Runtime's existing `text_to_speech` support.

Use this when:
- you want the call to use HQ Runtime's configured TTS voice rather than Twilio `<Say>`
...
Recommended HQ Runtime TTS -> Twilio Play workflow:

1. Generate the audio with HQ Runtime `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask HQ Runtime to create the message audio with `text_to_speech`
...
Important note:
- HQ Runtime TTS is great for prerecorded outbound messages
...
- HQ Runtime STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to HQ Runtime memory.
...
- Do not store arbitrary third-party phone numbers in HQ Runtime memory.
...
3. persist that number to `${HQR_HOME:-~/.hqr}/.env`
