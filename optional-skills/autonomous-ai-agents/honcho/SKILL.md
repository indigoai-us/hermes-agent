---
name: honcho
description: Configure and troubleshoot Honcho memory for HQ Runtime.
version: 2.0.0
...
# Honcho Memory for HQ Runtime

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every HQ Runtime profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hqr#running-honcho-locally-with-hqr

### Verify

```bash
hqr honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this HQ Runtime profile's AI peer
...
Honcho models conversations as interactions between **peers**. HQ Runtime creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this HQ Runtime instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each HQ Runtime run |
| `global` | Single session across all directories |

Manual override: `hqr honcho map my-project-name`
...
Each HQ Runtime profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
hqr profile create coder --clone
# creates host block hqr.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `hqr.coder` host block in `honcho.json`
...
```bash
hqr honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for HQ Runtime when Honcho memory is active.
...
Config file: `$HQR_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `hqr honcho setup`. Ensure `memory.provider: honcho` is in `~/.hqr/config.yaml`.

### Memory not persisting across sessions
Check `hqr honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hqr profile create <name> --clone`. For existing profiles: `hqr honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hqr honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hqr honcho status` | Show resolved config, connection test, peer info for active profile |
| `hqr honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hqr honcho disable` | Disable Honcho for the active profile |
| `hqr honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hqr honcho peers` | Show peer identities across all profiles |
| `hqr honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hqr honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hqr honcho sessions` | List known directory-to-session-name mappings |
| `hqr honcho map <name>` | Map current working directory to a Honcho session name |
| `hqr honcho identity` | Seed AI peer identity or show both peer representations |
| `hqr honcho sync` | Create host blocks for all HQ Runtime profiles that don't have one yet |
| `hqr honcho migrate` | Step-by-step migration guide from OpenClaw native memory to HQ Runtime + Honcho |
| `hqr memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hqr memory status` | Show active memory provider and config |
| `hqr memory off` | Disable external memory provider |
