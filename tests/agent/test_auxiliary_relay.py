...
from hqr_cli.observability.shared_metrics_contract import MODEL_ROUTE_METRIC
from hqr_cli.observability.shared_metrics_subscriber import SharedMetricsSubscriber


@pytest.fixture()
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("HQR_HOME", str(tmp_path / "profile"))
...
def test_auxiliary_provider_fallback_records_one_terminal_model_route(
    ...
    assert snapshot[0]["resource"]["hqr_version"] == "test-version"
