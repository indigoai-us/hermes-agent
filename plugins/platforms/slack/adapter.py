...
# identify HqrAgent traffic — matching other HQ Runtime outbound surfaces
# that already set ``HqrAgent/<version>`` for platform-partner attribution.
try:
    from hqr_cli import __version__ as _HQR_VERSION
except Exception:
    _HQR_VERSION = "unknown"
_HQR_SLACK_USER_AGENT_PREFIX = f"HqrAgent/{_HQR_VERSION}"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from hqr_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `hqr slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `hqr gateway setup` "
                "or in the active profile's ~/.hqr/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `hqr gateway setup` "
                "or add it to your active profile's ~/.hqr/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `hqr gateway setup` "
                "or in the active profile's ~/.hqr/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `hqr gateway setup` "
                "or add it to your active profile's ~/.hqr/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_hqr_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_HQR_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # HQ Runtime has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by HQ Runtime can "
            ...
            # /model work directly without /hqr prefix). A single regex
            ...
            # manifest (see `hqr slack manifest`). In Socket Mode, Slack
            ...
            from hqr_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/hqr$")

            @self._app.command(_slash_pattern)
            async def handle_hqr_command(ack, command):
            ...
            self._app.action("hqr_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^hqr_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("hqr_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'hqr slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own HQ Runtime session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # HQ Runtime owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_hqr_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_hqr_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_hqr_no_thread_response"):
        ...
        force_process = bool(event.get("_hqr_force_process"))
        ...
        # the shape produced by peer HQ Runtime agents in Socket Mode on some
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "hqr_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "hqr_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def send_clarify(
        ...
        (unique ``hqr_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``hqr_clarify_other``).  A choice click resolves the clarify
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "hqr_confirm_once": "once",
            "hqr_confirm_always": "always",
            "hqr_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "hqr_approve_once": "once",
            "hqr_approve_session": "session",
            "hqr_approve_always": "always",
            "hqr_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "hqr_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/hqr <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/hqr
        ...
        if slash_name in {"hqr", ""}:
            # Legacy /hqr <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from hqr_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/hqr   ``).
        ...
        # "Running /cmd…" ack shown by handle_hqr_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/hqr <question>" must produce public replies so
...
# ``hqr_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_hqr_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``hqr_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under HQR_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from hqr_cli.slack_cli import _build_full_manifest
            from hqr_constants import get_hqr_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="HQ Runtime",
                bot_description="Your HQ Runtime agent on Slack",
            )
            target = Path(get_hqr_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `hqr slack manifest --write` anytime to refresh after "
                "HQ Runtime adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `hqr update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``hqr_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `hqr setup` to install Slack support.",
        # Interactive setup wizard — replaces hqr_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in hqr_cli/gateway.py.
