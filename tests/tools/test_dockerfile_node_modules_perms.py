...
Older images made /opt/hqr/ui-tui and /opt/hqr/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HQR_TUI_DIR=/opt/hqr/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hqr:hqr /opt/hqr/ui-tui" not in text
    assert "chown -R hqr:hqr /opt/hqr/node_modules" not in text
