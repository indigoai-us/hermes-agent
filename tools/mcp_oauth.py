...
This module provides the glue:
    - ``HqrTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "HQ Runtime Agent"
"""
...
from hqr_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__hqr_user_skipped__"
...
def _get_token_dir(hqr_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses HQR_HOME so each profile gets its own OAuth tokens.
    Layout: ``HQR_HOME/mcp-tokens/``
    """
    from hqr_constants import get_hqr_home

    base = Path(hqr_home) if hqr_home is not None else Path(get_hqr_home())
...
def _cached_redirect_port(storage: "HqrTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If HQ Runtime restarts and chooses a
...
def _cached_redirect_uri(storage: "HqrTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``hqr mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `hqr mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# HqrTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class HqrTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        HQR_HOME/mcp-tokens/<server_name>.json         -- tokens
        HQR_HOME/mcp-tokens/<server_name>.client.json   -- client info
        HQR_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, hqr_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._hqr_home = Path(hqr_home) if hqr_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._hqr_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._hqr_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._hqr_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # HQ Runtime records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._hqr_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to HQ Runtime.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `hqr mcp login <server>` later to "
...
HqrOAuthClientProvider: Any = None


def _get_hqr_oauth_provider_class() -> type | None:
    global HqrOAuthClientProvider
    if HqrOAuthClientProvider is not None:
        return HqrOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _HqrOAuthClientProvider(OAuthClientProvider):
    ...
    _HqrOAuthClientProvider.__name__ = "HqrOAuthClientProvider"
    _HqrOAuthClientProvider.__qualname__ = "HqrOAuthClientProvider"
    HqrOAuthClientProvider = _HqrOAuthClientProvider
    return HqrOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = HqrTokenStorage(server_name, hqr_home=hqr_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "HqrTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "HQ Runtime Agent" / "HQ Runtime" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "HQ Runtime Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. HQ Runtime is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: hqr mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. HQ Runtime auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older HQ Runtime is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = HqrTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`hqr mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_hqr_oauth_provider_class()
