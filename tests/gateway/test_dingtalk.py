...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "HQ Runtime"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^hqr"]}
        )
        assert adapter._message_matches_mention_patterns("hqr please help") is True
        assert adapter._message_matches_mention_patterns("please hqr help") is False
