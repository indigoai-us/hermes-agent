"""`hqr checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hqr/checkpoints/``.  Actions:

    hqr checkpoints               # same as `status`
    hqr checkpoints status        # total size, project count, breakdown
    hqr checkpoints list          # per-project checkpoint counts + workdir
    hqr checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    hqr checkpoints clear [-f]    # nuke the entire base (asks first)
    hqr checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    hqr checkpoints
    hqr checkpoints prune --retention-days 3 --max-size-mb 200
    hqr checkpoints clear -f
...
from hqr_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: hqr checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``hqr checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `hqr checkpoints` → status
