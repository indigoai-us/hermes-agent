"""Run the read-tool eval through the REAL HQ Runtime AIAgent.

For each task: fresh temp HQR_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    hqr_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".hqr"
    ...
    try:
        ...
        shutil.rmtree(hqr_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.hqr/.env; set +a  — then relaunch."
        )
