...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"hqr-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "hqr-agent-dev", "declared": True},
        },
    )
...
# `hqr curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `hqr curator pin <umbrella>` in the rename block so
