"""Coding-context awareness — base HQ Runtime, every interactive surface.

When the user runs HQ Runtime inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), HQ Runtime shifts into a **coding posture**. This module is the
...
from hqr_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("hqr.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "hqr", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _HQR_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``HQR_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether HQ Runtime should operate in its coding posture right now."""
