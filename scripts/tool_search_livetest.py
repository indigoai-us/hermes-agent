#!/usr/bin/env python3
"""Live test harness for HQ Runtime Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any hqr imports.
ORIGINAL_HOME = os.environ.get("HQR_HOME")
ORIGINAL_AUTH = Path.home() / ".hqr" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.hqr/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.hqr/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="hqr_ts_live_"))
    hqr_home = home_dir / ".hqr"
    hqr_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, hqr_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".hqr" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, hqr_home / ".env")
        ...
        load_hqr_dotenv(hqr_home=str(Path.home() / ".hqr"))
    ...
    (hqr_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return hqr_home
...
def reset_module_state():
    """Drop cached modules so the new HQR_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "hqr_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original HQR_HOME
    if ORIGINAL_HOME is not None:
        os.environ["HQR_HOME"] = ORIGINAL_HOME
    else:
