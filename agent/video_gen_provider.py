...
as ``kind: backend``) or ``~/.hqr/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``hqr tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$HQR_HOME/cache/videos/``, creating parents as needed."""
    from hqr_constants import get_hqr_home

    path = get_hqr_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$HQR_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$HQR_HOME/cache/videos/``.
