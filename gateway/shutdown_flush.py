...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<hqr_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active HQR_HOME."""
    from hqr_constants import get_hqr_home

    flush_dir = get_hqr_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<hqr_home>/pending_messages/``), so a runtime cap rotation no
