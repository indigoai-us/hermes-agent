...
class TestWriteDenyExactPaths:
    ...
    def test_hqr_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.hqr/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "hqr_root"
        ...
        # Sanity check: HQR_HOME does point to the profile dir, not the root.
        from hqr_constants import get_hqr_home, get_default_hqr_root
...
class TestWriteAllowed:
    ...
    def test_hqr_control_files_requested_writable(self):
