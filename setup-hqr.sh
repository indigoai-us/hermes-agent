...
# 5. Symlinks the 'hqr' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ HQ Runtime Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/hqr-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink hqr into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up hqr command..."

HQR_BIN="$SCRIPT_DIR/venv/bin/hqr"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HQR_BIN" "$COMMAND_LINK_DIR/hqr"
echo -e "${GREEN}✓${NC} Symlinked hqr → $COMMAND_LINK_DISPLAY_DIR/hqr"
...
# ============================================================================
# Seed bundled skills into ~/.hqr/skills/
# ============================================================================

HQR_SKILLS_DIR="${HQR_HOME:-$HOME/.hqr}/skills"
mkdir -p "$HQR_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.hqr/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$HQR_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  hqr status        # Check configuration"
if is_termux; then
    echo "  hqr gateway       # Run gateway in foreground"
else
    echo "  hqr gateway install # Install gateway service (messaging + cron)"
fi
echo "  hqr cron list     # View scheduled jobs"
echo "  hqr doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m hqr_cli.main setup
fi
