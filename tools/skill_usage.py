...
Tracks per-skill usage metadata in a sidecar JSON file (~/.hqr/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.hqr/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.hqr/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.hqr/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``hqr update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip HQ Runtime metadata, VCS, virtualenv/dependency, and cache dirs
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.hqr/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``hqr curator
    list-archived`` to help users pass a name to ``hqr curator restore``.
    """
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``hqr curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``hqr curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``hqr curator
...
def adopt_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    if is_bundled(skill_name):
        ...
        # claim HQ Runtime' own shipped skill was agent-authored and change nothing
...
def _emit_skill_lifecycle(
    ...
    *,
    ...
    try:
        from hqr_cli.lifecycle import has_hook, invoke_hook
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.hqr/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.hqr/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.hqr/skills/<skill>/SKILL.md) and category-nested
    (~/.hqr/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
