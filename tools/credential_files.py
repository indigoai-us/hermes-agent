...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.hqr",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``HQR_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside HQR_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because HQR_HOME is exactly where
    ...
    """
    hqr_home = _resolve_hqr_home()

    # Reject absolute paths — they bypass the HQR_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to HQR_HOME)",
            relative_path,
        )
        return False

    host_path = hqr_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape HQR_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, hqr_home)
    ...
    # inside HQR_HOME and therefore pass the containment check above.
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="hqr-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching hqr_constants.get_hqr_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_hqr_dir()`` for backward compatibility with old directory layouts.
    """
    from hqr_constants import get_hqr_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_hqr_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_hqr_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.hqr``,
    SSH ``<remote_home>/.hqr``, etc.) and whether translation is wanted.
