"""Tests for the unified provider catalog (hqr_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``hqr model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from hqr_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
