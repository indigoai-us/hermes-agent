"""Migrate HQ Runtime' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads HQ Runtime' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  HQ Runtime mcp_servers.<n>.command/args/env  → codex stdio transport
  HQ Runtime mcp_servers.<n>.url/headers       → codex streamable_http transport
  HQ Runtime mcp_servers.<n>.timeout           → codex tool_timeout_sec
  HQ Runtime mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  HQ Runtime-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. HQ Runtime' own
...
MIGRATION_MARKER = (
    "# managed by hqr-agent — `hqr codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end hqr-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in HQ Runtime config.")
...
# HQ Runtime keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # HQ Runtime' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, hqr_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one HQ Runtime MCP server config to the codex inline-table dict
    ...
    if not isinstance(hqr_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(hqr_cfg.get("command"))
    has_url = bool(hqr_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(hqr_cfg["command"])
        args = hqr_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = hqr_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(hqr_cfg["url"])
        headers = hqr_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # HQ Runtime' transport: sse hint is informational; codex auto-negotiates
        if hqr_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in hqr_cfg:
        try:
            out["tool_timeout_sec"] = float(hqr_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in hqr_cfg:
        try:
            out["startup_timeout_sec"] = float(hqr_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if hqr_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in hqr_cfg:
        ...
        elif key not in _KNOWN_HQR_KEYS:
            skipped.append(f"{key} (unknown HQ Runtime key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (HQR_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by HQ Runtime)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert HQ Runtime' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before HQ Runtime' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior HQ Runtime
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="hqr-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a HQR_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed hqr-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.hqr`` symlink that happens to live under ``/private/var/folders``
...
def _build_hqr_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches HQ Runtime' own
    ...
    so a hqr installed under /opt/, /usr/local/, or a venv all work.
    HQR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # HQR_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_hqr_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever HQR_HOME its launcher sets
    ...
    # — that would override the launcher's HQR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("HQR_HOME", tmp_path) would
    ...
    hqr_home = os.environ.get("HQR_HOME") or ""
    if hqr_home and _looks_like_test_tempdir(hqr_home):
        hqr_home = ""
    if hqr_home:
        env["HQR_HOME"] = hqr_home
    # PYTHONPATH passes through so a worktree-launched hqr finds the
    ...
    env["HQR_QUIET"] = "1"
    env["HQR_REDACT_SECRETS"] = env.get("HQR_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.hqr_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_hqr_tools: bool = True,
) -> MigrationReport:
    """Translate HQ Runtime mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hqr_config: full ~/.hqr/config.yaml dict
        ...
        expose_hqr_tools: when True (default), register HQ Runtime' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into HQ Runtime for tools
    ...
    hqr_servers = (hqr_config or {}).get("mcp_servers") or {}
    if not isinstance(hqr_servers, dict):
        report.errors.append(
            "mcp_servers in HQ Runtime config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in hqr_servers.items():
    ...
    # Inject HQ Runtime' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into HQ Runtime for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/hqr_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_hqr_tools:
        translated["hqr-tools"] = _build_hqr_tools_mcp_entry()
        if "hqr-tools" not in report.migrated:
            report.migrated.append("hqr-tools")
