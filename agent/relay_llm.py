"""Core NeMo Relay adapters for physical HQ Runtime provider attempts."""
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited HQ Runtime turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited HQ Runtime turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from HQ Runtime's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # HQ Runtime-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by HQ Runtime."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "HQ Runtime Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
