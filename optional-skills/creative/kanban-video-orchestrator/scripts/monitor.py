#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `hqr kanban list` and
...
def hqr_available() -> bool:
    return shutil.which("hqr") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed hqr CLI."""
    try:
        out = subprocess.run(
            ["hqr", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `hqr kanban list`
    out = subprocess.run(
        ["hqr", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not hqr_available():
        print("ERROR: 'hqr' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
