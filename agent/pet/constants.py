...
row taxonomy is inferred from the atlas shape so HQ Runtime can render both legacy
...
# rows (1536x1872), while older HQ Runtime/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are HQ Runtime' activity state names. They are not always identical to the
...
# Legacy HQ Runtime/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical HQ Runtime activity names -> accepted row-name aliases in descending
