"""Tests for hqr_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $HQR_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the HQ Runtime-managed node dir.
"""
...
def _make_hqr_node(hqr_home: Path) -> Path:
    """Create a fake $HQR_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = hqr_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    hqr_home = fake_home / ".hqr"
    _make_hqr_node(hqr_home)
    ...
    removed = uninstall.remove_node_symlinks(hqr_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    hqr_home = fake_home / ".hqr"
    node_bin = _make_hqr_node(hqr_home)
