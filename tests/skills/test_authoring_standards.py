...
def test_required_frontmatter_fields(p):
    ...
    hqr = (fm.get("metadata") or {}).get("hqr") or {}
    if not (hqr.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.hqr.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    hqr = (fm.get("metadata") or {}).get("hqr") or {}
    dangling = [
        rs for rs in (hqr.get("related_skills") or []) if rs not in _all_names()
    ]
