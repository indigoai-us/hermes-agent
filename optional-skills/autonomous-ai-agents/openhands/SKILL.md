...
version: 0.1.0
author: Tim Koepsel (xzessmedia), HQ Runtime Agent
license: MIT
platforms: [linux, macos]
metadata:
  hqr:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, hqr-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from HQ Runtime.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For HQ Runtime-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hqr-agent` (HQ Runtime subagents via `delegate_task`).
