...
The design is **HQ Runtime-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
hqr skills install official/security/unbroker
```

Then start a new HQ Runtime session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a HQ Runtime session:
...
- **PII handling.** Dossiers live under the HQ Runtime home directory (`0600`, optionally
