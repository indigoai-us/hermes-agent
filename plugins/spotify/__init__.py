...
``_check_spotify_available()`` — when the user has not run ``hqr auth
spotify``, the tools remain registered (so they appear in ``hqr
...
The Spotify auth flow (``hqr auth spotify``), CLI plumbing, and docs
