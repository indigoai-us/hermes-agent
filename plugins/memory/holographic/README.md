...
```bash
hqr memory setup    # select "holographic"
```

Or manually:
```bash
hqr config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.hqr-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$HQR_HOME/memory_store.db` | SQLite database path |
