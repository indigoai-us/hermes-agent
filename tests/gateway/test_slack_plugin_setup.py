...
``hqr_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``hqr_cli.config`` (get_env_value / save_env_value / remove_env_value)
and ``hqr_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch
those source modules.
"""
import hqr_cli.config as config_mod
import hqr_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved, removed, existing):
    ...
    # Manifest writing reaches out to hqr_cli.slack_cli + filesystem; stub it.
    import hqr_cli.slack_cli as slack_cli_mod
