"""Run a real HQ Runtime CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_hqr_executable(hqr_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "hqr",
        Path(".venv") / "Scripts" / "hqr.exe",
    ):
        candidate = hqr_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("hqr")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "HQ Runtime executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="HQ Runtime source checkout containing .venv/bin/hqr",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated HQR_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["hqr.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['hqr.client.active']}"
        )
    [model] = by_name["hqr.model_route.count"]
    expected_model = {
        "name": "hqr.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['hqr.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["hqr.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['hqr.task_run.started']}"
        )
    [terminal] = by_name["hqr.task_run.finished"]
    ...
    [tool] = by_name["hqr.tool_call.count"]
    ...
    lifecycle = by_name["hqr.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["hqr.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The HQ Runtime development environment requires jsonschema"
        ) from exc
    ...
    if metrics["hqr.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['hqr.client.active']}"
        )
    [model] = metrics["hqr.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['hqr.model_route.count']}"
        )
    [terminal] = metrics["hqr.task_run.finished"]
    ...
    [tool] = metrics["hqr.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["hqr.skill.lifecycle.count"]
    ...
    loads = metrics["hqr.skill.load.count"]
    if {
        (
...
def main() -> int:
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="hqr-relay-shared-metrics-"))
    home = root / "hqr-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["HQR_HOME"] = str(home)
        python_paths = [str(hqr_repo)]
    ...
    (root / "hqr.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "hqr.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"HQ Runtime exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("HQ Runtime model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("HQ Runtime did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("HQ Runtime did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from hqr_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "hqr.shared_metrics.v2.schema.json",
    )

    print("HQ Runtime -> NeMo Relay shared-metrics smoke test passed")
