"""ACP auth helpers — detect and advertise HQ Runtime authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "hqr-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active HQ Runtime runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if HQ Runtime can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for HQ Runtime.
    ...
    not have HQ Runtime provider credentials configured yet, so HQ Runtime always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate HQ Runtime using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure HQ Runtime provider",
            description=(
                "Open HQ Runtime' interactive model/provider setup in a terminal. "
                "Use this when HQ Runtime has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
