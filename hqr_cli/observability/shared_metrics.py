"""Durable aggregation and local export for HQ Runtime shared metrics."""
...
from hqr_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "hqr.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_hqr_home() / "telemetry" / "shared_metrics"
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across HQ Runtime processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "hqr_version": period_row["hqr_version"],
