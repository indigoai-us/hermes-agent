"""HQ Runtime update pipeline — extracted from ``hqr_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``hqr_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``hqr_cli.main`` reference — so existing call sites and test monkeypatches
that target ``hqr_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``hqr_cli.main``.
...
Imports are one-way: ``hqr_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``hqr_cli.main`` reference.

    Lets callers keep patching ``hqr_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``hqr_cli.main`` -> ``hqr_cli.update_cmd`` stays one-way
    at import time.
    """
    from hqr_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``hqr update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``hqr update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``hqr_cli.config_defaults``,
    ``hqr_cli.config``, and ``hqr_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``hqr_cli._subprocess_compat`` and
    ``hqr_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "hqr_cli.config_defaults",
        "hqr_cli.config",
        "hqr_cli.config_migrations",
        "hqr_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from hqr_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from hqr_cli.config import migrate_config

    return migrate_config(interactive=interactive, quiet=quiet)


# Critical files that HQ Runtime must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``hqr`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="hqr-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``hqr update`` can be driven by a
    ...
    """
    from hqr_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from hqr_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('hqr_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``hqr update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `hqr update`.
    ...
    print("  Preview now:  hqr curator run --dry-run")
    print("  Pause it:     hqr curator pause")
    print(
        "  Docs:         https://hermes-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `hqr update`.
    ...
    print("  Run when convenient:  hqr sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``hqr update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``hqr update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: hqr curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from hqr_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``hqr_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.hqr-update-staging"
    backup = f"{dst}.hqr-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `hqr update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``hqr_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("HQR_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== hqr-update completed {action_id} ===")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    """Update HQ Runtime Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `hqr update "
            f"--branch {branch}`, or update against main with `hqr update`."
        )
        _m().sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="hqr-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"hqr-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for hqr-agent itself
        ...
        # GitHub ZIPs extract to hqr-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"hqr-agent-{branch}")
        ...
        print(
            "  Re-run `hqr update` to retry; if the agent won't start, "
    ...
    from hqr_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `hqr update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: hqr skills list-modified  "
    ...
    try:
        from hqr_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from hqr_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_hqr_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_hqr_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "hqr-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if HQ Runtime behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes hqr completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but HQ Runtime couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but HQ Runtime couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if HQ Runtime behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but HQ Runtime couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but HQ Runtime couldn't drop "
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_hqr_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_hqr_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official HQ Runtime repository.")
        print("  This means you may miss updates from NousResearch/hqr-agent.")
        print()
        try:
            response = (
...
def _invalidate_update_cache():
    ...
    ``hqr update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from hqr_constants import get_default_hqr_root

    default_home = get_default_hqr_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "hqr.exe"
    lines = ["✗ Another hqr.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close HQ Runtime Desktop, exit any open `hqr` REPLs, and")
    lines.append("  stop the gateway (`hqr gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `hqr update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``hqr tools``."""
    try:
        from hqr_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active HQ Runtime Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``hqr tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("HQ Runtime Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} HQ Runtime Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``hqr update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `hqr update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `hqr update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in HQ Runtime' editable-install extras or
    ...
    try:
        from hqr_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from hqr_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$HQR_HOME/bin/uv``, but on Termux the official
    ...
    """
    from hqr_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `hqr update` is exactly the step
...
def _npm_lockfile_changed(hqr_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `hqr
    ...
    try:
        ...
        cache_file = hqr_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(hqr_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    hqr update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `hqr update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from hqr_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `hqr update`.")
            failed = []
            if any(
    ...
    # HQ Runtime profile using this checkout. Keep one per-checkout cache under the
    # shared HQ Runtime root rather than rerunning npm once per named profile.
    shared_hqr_root = get_default_hqr_root()
    ...
    # `hqr update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_hqr_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `hqr update`.")
        return list(labels)
    ...
    nixos_env = with_hqr_node_path(_m()._nixos_build_env())
    ...
    # `hqr update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_hqr_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.hqr/logs/update.log`` only, never the terminal.

    During ``hqr update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``hqr update --check``: fetch and report without installing.
    ...
    """
    from hqr_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``hqr update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from hqr_cli.config import format_docker_update_message
    ...
    from hqr_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from hqr_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``hqr update`` without requiring a reinstall.  The
    ...
    ``hqr`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/hqr.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v hqr'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/hqr, code at /usr/local/lib/hqr-agent).
    fhs_link = Path("/usr/local/bin/hqr")
    ...
    path_comment = (
        "# HQ Runtime Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``hqr-acp`` launcher next to the ``hqr`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``hqr update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``hqr-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report HQ Runtime as not installed even when it is.

    The shim simply delegates to the sibling ``hqr`` launcher with the
    ...
    No-op on Windows (install.ps1 copies ``hqr.exe`` + ``hqr-acp.exe``
    ...
    so ``hqr-acp.exe`` already resolves) and wherever a ``hqr-acp`` is
    already present next to the ``hqr`` command.  Unwritable directories
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        hqr_cmd = bin_dir / "hqr"
        acp_cmd = bin_dir / "hqr-acp"
        try:
            if not (hqr_cmd.is_file() or hqr_cmd.is_symlink()):
                continue
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# HQ Runtime Agent — ACP launcher (written by `hqr update`).\n"
            ...
            continue
        print(f"  ✓ Installed hqr-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of HQR_HOME under
      ``backups/`` (restorable via ``hqr import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from hqr_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from hqr_constants import
        # get_hqr_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from hqr_cli.config import get_hqr_home as _get_home
    ...
    try:
        from hqr_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from hqr_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_hqr_home so the user sees ~/.hqr/...
    try:
        from hqr_constants import get_hqr_home, display_hqr_home

        home = get_hqr_home()
        try:
            display_path = f"{display_hqr_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  hqr import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``hqr update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``hqr update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run hqr from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.hqr-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".hqr-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The hqr.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m hqr_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``hqr
    ...
    for proc in proc_iter:
        ...
        # the `-m hqr_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``hqr_cli.main`` used to import
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `hqr` launch will complete it")
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other HQ Runtime processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← HQ Runtime Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the HQ Runtime desktop app / other HQ Runtime terminals, then re-run:"
    )
    lines.append("    hqr update")
    lines.append("  (or use `hqr update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``hqr update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from hqr_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning hqr-setup, and the update-in-progress marker
    ...
    the update with "HQ Runtime is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a HQ Runtime backend (``hqr_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.hqr-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "hqr_cli.main" in argv_low and (
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.hqr-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    hqr.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating HQ Runtime...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m hqr_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: hqr gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``hqr gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from hqr_cli import gateway_windows
        from hqr_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``hqr-gateway*.service``/``hqr-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("hqr-serve")`` alone would also accept the
        # unrelated ``hqr-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "hqr-gateway.service"
            or unit.startswith("hqr-gateway-")
            or unit == "hqr-serve.service"
            or unit.startswith("hqr-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``hqr-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``hqr-serve*`` units (#83438) don't, so sending them
    ...
    (``hqr-gateway-helper`` is fine — profile units are
    ``hqr-gateway-<profile>`` — but ``hqr-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "hqr-gateway" or svc_name.startswith("hqr-gateway-")
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``hqr_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``hqr_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``hqr update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``hqr-setup.exe``) executes
    ``$HQR_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_hqr_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``hqr update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``hqr update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `hqr desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "hqr_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the HQ Runtime-managed Node on PATH: when
    # `hqr update` runs inside the desktop updater chain (Desktop →
    # hqr-setup → hqr update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from hqr_constants import with_hqr_node_path

    build_env = with_hqr_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (non-fatal; run `hqr desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating HQ Runtime Agent...")
    print()

    # On Windows, abort early if another hqr.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_hqr_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `hqr serve` backend) will keep .pyd
    ...
    # the hqr.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .hqr-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "HQ Runtime is still
    ...
    # process by design (a CLI `hqr update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all HQ Runtime windows/gateways and re-run: hqr update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running HQ Runtime gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in hqr_cli/config.py bricked
        # every user who ran ``hqr update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``hqr
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``hqr update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``hqr update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``hqr doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && hqr update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_hqr_home added to hqr_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``hqr`` launch finishes the install
        ...
        # symbols in hqr_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `hqr` again "
        ...
        if not import_ok:
            ...
            print("    Run `hqr update` again — if it persists, reinstall:")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_hqr_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.hqr/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit HQR_HOME so
        # it is not affected by sync_skills()'s module-level HQR_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's HQR_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from hqr_cli.profiles import backfill_profile_envs
        ...
        # ``hqr update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``hqr_cli.config`` and
        # ``hqr_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from hqr_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'hqr config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'hqr config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: hqr config migrate")
            else:
                print()
                print("Skipped. Run 'hqr config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from hqr_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `hqr curator status`. Self-stamps after printing so it
        ...
        # Self-heal the hqr-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve HQ Runtime on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("hqr-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``hqr update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from hqr_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `hqr update`
                # must stay fast; `hqr computer-use install --upgrade`
        ...
        # When running as ``hqr update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_hqr_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... hqr-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... hqr-gateway*)
            ...
            try:
                from hqr_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all hqr-gateway* units (default + profiles) plus
            # hqr-serve* units (the Desktop app's backend, #83438).
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: hqr gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # hqr-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `hqr update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: hqr gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: hqr -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy HQ Runtime gateway unit files are still installed.
        # When both hqr.service (from a pre-rename install) and the
        # current hqr-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `hqr update` surfaces the issue until the user migrates.
        try:
            from hqr_cli.gateway import (
                has_legacy_hqr_units,
                _find_legacy_hqr_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_hqr_units():
                print()
                print("⚠ Legacy HQ Runtime gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hqr_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (hqr.service) fight the current")
                print("  hqr-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    hqr gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes hqr-serve*,
        ...
        print("  hqr model              # Select provider and model")
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``hqr_cli.gateway`` no longer imports,
