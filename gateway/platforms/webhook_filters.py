...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.hqr to the active profile home."""
    ...
    if raw == "~/.hqr":
        return hqr_home
    if raw.startswith("~/.hqr/"):
        return hqr_home / raw.removeprefix("~/.hqr/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return hqr_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under HQR_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from hqr_constants import get_hqr_home

    scripts_root = (get_hqr_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.hqr" or raw_text.startswith("~/.hqr/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__hqr_ignore__") is True
        ):
            return False, None
