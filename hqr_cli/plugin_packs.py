...
A pack is a single YAML file (``hqr-pack.yaml``) that pins a set of
...
Format (canonical)::
    ...
    version: 1.0.0
    plugins:
      - name: hqr-media-studio          # bare community-index name…
...
* Capability consent is NEVER bulk-granted: after each plugin installs,
  ...
  as a normal ``hqr plugins install`` (#64228).
    ...
    for entry in pack.plugins:
        ...
        try:
            if index_entries is None:
                from hqr_cli.plugin_index import load_index

                index_entries, _src = load_index()
            from hqr_cli.plugin_index import resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `hqr skills install <id>`.[/dim]"
        )
    console.print(
...
def install_pack_plugins(
    ...
    *,
    ...
    single install (:func:`hqr_cli.plugins_cmd._run_capability_consent`).
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-hqr-pack") -> tuple[str, List[str]]:
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `hqr plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def cmd_pack_show(source: str) -> None:
    """``hqr plugins pack show <path-or-url>`` — dry-run review."""
    ...
    console.print("\n[dim]Dry run only. Install with `hqr plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``hqr plugins pack install <path-or-url>``.
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-hqr-pack") -> None:
    """``hqr plugins pack export [--enabled-only]`` — pack YAML on stdout."""
    from rich.console import Console
...
def pack_command(args) -> None:
    """Dispatch ``hqr plugins pack <action>``."""
    ...
    if action == "install":
        ...
        cmd_pack_export(
            ...
            name=getattr(args, "name", None) or "my-hqr-pack",
        )
    elif action == "show":
        ...
        Console().print(
            "[red]Error:[/red] Usage: hqr plugins pack {install|export|show}"
        )
        sys.exit(1)
