"""Tests for the top-level `./hqr` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hqr` should use `hqr_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hqr"
    called = []

    fake_main_module = types.ModuleType("hqr_cli.main")

    def fake_main():
        called.append("hqr_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "hqr_cli.main", fake_main_module)
