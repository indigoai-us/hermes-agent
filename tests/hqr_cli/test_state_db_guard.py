...
from hqr_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    HQR_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def hqr_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("hqr_cli.config") or mod == "hqr_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, hqr_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, hqr_home, capsys, monkeypatch):
